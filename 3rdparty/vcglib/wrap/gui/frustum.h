...
// Implementation
template <class T>
Point3<T> Frustum<T>::ViewPoint()
{
    return view_point;
}

template <class T>
T Frustum<T>::Resolution(float dist)
{
    return resolution * dist;
}

template <class T>
bool Frustum<T>::IsOutside(Point3<T> &point)
{
...
template <class T>
T Frustum<T>::Remoteness(Point3<T> &point, T radius)
{
    Point3<T> r = Project(point);
...
template <class T>
T Frustum<T>::IsOutside(Point3<T> &point, T radius)
{
    T dist = 0;
    for (int i = 0; i < 4; i++)
    {
...
template <class T>
T Frustum<T>::range(Point3<T> &point, T radius, T &closest, T &farthest)
{
    // 4 near 5 far
...
template <class T>
T Frustum<T>::Distance(Point3<T> &point, int plane)
{
    return vcg::Distance(planes[plane], point);
}

template <class T>
void Frustum<T>::GetView()
{
    View<T>::GetView();
    UpdateView();
}
template <class T>
void Frustum<T>::SetView(const float *_proj = NULL, const float *_modelview = NULL, const int *_viewport = NULL)
{
...
void Frustum<T>::UpdateView()
{
    ...
    resolution =
      ((ne + NE) / 2 - (nw + NW) / 2).Norm() / (View<T>::viewport[2] * ((ne + NE + nw + NW) / 4 - view_point).Norm());
}
