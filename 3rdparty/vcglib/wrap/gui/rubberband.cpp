...
Rubberband::Rubberband(Color4b c)
    : color(c), currentphase(RUBBER_BEGIN), qt_cursor(), start(0, 0, 0), end(0, 0, 0), have_to_pick(false), font()
{
...
void Rubberband::Render(QGLWidget* gla)
{
    if (have_to_pick)
    {
        ...
        if (picked)
        {  // we have not picked the background
            have_to_pick = false;
            switch (currentphase)
            {
                case RUBBER_BEGIN:
    ...
    if (currentphase == RUBBER_BEGIN)
        return;

    // Drawing of the current line
    glPushAttrib(GL_DEPTH_BUFFER_BIT | GL_ENABLE_BIT | GL_LINE_BIT | GL_POINT_BIT | GL_CURRENT_BIT | GL_LIGHTING_BIT |
                 GL_COLOR_BUFFER_BIT);
