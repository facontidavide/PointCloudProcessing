...
in SphereMode::Hit added a check on the sphere intersection, if no intersection, calculating distance could generate a
NAN exception
...
void PathMode::Draw(Trackball *tb)
{
    ...
    DrawUglyPathMode(tb, points, current_point, prev_point, next_point, old_hitpoint, wrap);
}
...
void AreaMode::Init(const std::vector<Point3f> &pts)
{
    ...
    for (unsigned int i = 0; i < onethird; i++)
    {
        ...
        if (pts_not_in_line)
        {
            plane.Init(pts[i % npts], pts[(i + (onethird)) % npts], pts[(i + (2 * onethird)) % npts]);
            break;
        }
    }
...
void PolarMode::Apply(Trackball *tb, Point3f new_point)
{
    ...
    if (endb < -top)
        endb = -top;
    tb->track.rot = Quaternionf(endb, Point3f(1, 0, 0)) * Quaternionf(enda, Point3f(0, 1, 0));
}
...
bool NavigatorWasdMode::IsAnimating(const Trackball *tb)
{
    ...
    if (tb->current_button & MOVEMENT_KEY_MASK)
        return true;
    if (current_speed != Point3f(0, 0, 0))
        return true;
    if (step_current > 0.0)
        return true;
    return false;
...
void NavigatorWasdMode::Animate(unsigned int msec, Trackball *tb)
{
    ...
    float sa = sin(-alpha);
    float ca = cos(-alpha);
    if (tb->current_button & Trackball::KEY_UP)
        acc += vcg::Point3f(sa, 0, ca) * (accY * _flipH);
    if (tb->current_button & Trackball::KEY_DOWN)
        acc -= vcg::Point3f(sa, 0, ca) * (accY * _flipH);
    if (tb->current_button & Trackball::KEY_LEFT)
        acc -= vcg::Point3f(-ca, 0, sa) * accX;
    if (tb->current_button & Trackball::KEY_RIGHT)
        acc += vcg::Point3f(-ca, 0, sa) * accX;
    if (tb->current_button & Trackball::KEY_PGUP)
        acc -= vcg::Point3f(0, 1, 0) * accZ;
    if (tb->current_button & Trackball::KEY_PGDOWN)
        acc += vcg::Point3f(0, 1, 0) * accZ;
    ...
    if (vel < topSpeedH * 0.05)
    {
        ...
        if (step_current < step_height * 0.06)
        {
            step_current = 0;
            step_x = 0.0f;
        }
    ...
    else
    {
        ...
        if (step_current < step_current_min)
            step_current = step_current_min;
    }

    current_speed *= pow(dumping, sec);
    if (current_speed.Norm() < topSpeedH * 0.005)
        current_speed.SetZero();  // full stop
...
void NavigatorWasdMode::Apply(Trackball *tb, Point3f new_point)
{
    ...
    tb->track.rot = Quaternionf(beta, Point3f(1, 0, 0)) * Quaternionf(alpha, Point3f(0, 1, 0));
