...
ActiveCoordinateFrame::ActiveCoordinateFrame(float size)
    ...
    , drawmoves(true)
    , drawrotations(true)
    , move_button(Trackball::BUTTON_RIGHT)
    , rotate_button(Trackball::BUTTON_LEFT)
    , x_modifier(Trackball::BUTTON_NONE)
    , y_modifier(Trackball::KEY_CTRL)
    , z_modifier(Trackball::KEY_SHIFT)
    , x_axis(1, 0, 0)
    , y_axis(0, 1, 0)
    , z_axis(0, 0, 1)
    , rot_snap_rad(0.0f)
...
void ActiveCoordinateFrame::Render(QGLWidget* glw)
{
    ...
    {
        glColor(xcolor);
        message = QString("move or rotate on X axis");
    }
    else if (current_mode == y_modifier)
    {
        glColor(ycolor);
        message = QString("move or rotate on Y axis");
    }
    else if (current_mode == z_modifier)
    {
        glColor(zcolor);
        message = QString("move or rotate on Z axis");
    }
    else if (rotating && drawrotations)
    {  // draw a rotation
        ...
        axis = axis * verse;
        message = QString("rotated %1 deg around %2").arg(((angle * 180.0) / M_PI), 5, 'f', 3).arg(axis_name);
        Quaternionf arc_rot;
    ...
    {  // draw a traslation
        ...
        else if (current_mode == movz)
        {
            ...
            assert(0);
        message =
          QString("moved %1 units along %2").arg(verse * manipulator->track.tra.Norm(), 5, 'f', 3).arg(axis_name);
