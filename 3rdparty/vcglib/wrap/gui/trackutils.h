...
/*!
  ...
  \warning Many of these functions shouldn't be here and are supposed to be moved to some appropriate place by the
  library administrator.
  \warning The \e DrawUgly series of functions is a \b TEMPORARY solution, used while waiting for the \e DrawBeautiful
  series...
*/
...
/*!
  @brief Project a window coordinate point on the plane perpedicular to view dir and passing through the manipulator
  center.
...
bool HitHyper(Point3f center, float radius, Point3f viewpoint, Plane3f vp, Point3f hitplane, Point3f &hit)
{
    ...
    if (delta > 0)
    {
        ...
        yval = c / xval;  //  alternatively it also could be the other part of the equation
...
Point3f HitSphere(Trackball *tb, const Point3f &p)
{
    ...
    Point3f hitPlane(0, 0, 0),  // intersection view plane with point touched
      hitSphere(0, 0, 0), hitSphere1(0, 0, 0), hitSphere2(0, 0, 0), hitHyper(0, 0, 0);
    ...
    // printf("Angle %f (%5.2f %5.2f %5.2f) (%5.2f %5.2f
...
/*!
 @brief Calculates the minimal distance between 2 lines.
  ...
  @return a std::pair made with the distance from P_s to Q_t and a boolean value, true if and only if P and Q are almost
 parallel.
*/
...
/*!
  ...
  @return a std::pair made with the distance from R_s to Q_t and a boolean value, true if and only if P and Q are almost
  parallel.
*/
...
//  @param R_s the point on R closest to Q.
//  @param Q_t the point on Q closest to R.
//  @return a std::pair made with the distance from R_s to Q_t and a boolean value, true if and only if P and Q are
//  almost parallel.
//*/
// std::pair< float, bool > SegmentSegmentDistance(const Segment3f & R, const Segment3f & Q, Point3f & R_s, Point3f &
// Q_t)
//{
...
/*!
  ...
  @return a std::pair made with the point on axis closest to the ray projection of point and a boolean true if and only
  if the ray doesn't diverges respect to the axis.
*/
...
/*!
  ...
  @return a std::pair made with p's projection on the.plane and a boolean true if and only if the ray doesn't diverges
...
/*!
  ...
  \warning this method is part of the \e DrawUgly series of functions, which is a \b TEMPORARY solution, used while
  waiting for the \e DrawBeautiful series...
*/
void prepare_attrib()
{
    float amb[4] = { .3f, .3f, .3f, 1.0f };
...
void DrawUglyLetter(Trackball *tb, std::vector<Point3f> ugly_letter)
{
    ...
    {
        glVertex(tb->camera.UnProject(center + (ugly_letter[i] * offset * 0.25) + Point3f(-offset, -offset, 0)));
    }
    glEnd();
    glPopAttrib();
    glPopMatrix();
...
*/
void DrawUglyPathMode(Trackball *tb, const std::vector<Point3f> &points, Point3f current_point, Point3f prev_point,
...
*/
void DrawUglyAreaMode(Trackball *tb, const std::vector<Point3f> &points, Point3f status, Point3f old_status,
                      Plane3f plane, const std::vector<Point3f> &path, Point3f rubberband_handle)
{
    glPushMatrix();
    glPushAttrib(GL_ALL_ATTRIB_BITS);
    // go to world coords
