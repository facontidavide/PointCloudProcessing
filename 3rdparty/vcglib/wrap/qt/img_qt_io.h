...
inline bool saveQtY(const Image<1, ScalarType, Safe> &image, const QString filename)
{
    ...
    {
        if (!success)
            throw ImageException("cannot save image");
    }
    return success;
}
...
inline bool savePGM(const Image<1, ScalarType, Safe> &image, const QString filename)
{
    ...
    if (Safe)
    {
        ...
        if (!image.attributes.hasRange(ScalarType(0.0), ScalarType(255.0)))
            throw ImageException("Invalid range attribute");
        if (!filename.endsWith(".pgm", Qt::CaseInsensitive))
            throw ImageException("filename is not .pgm");
    }
    using namespace std;
    ...
    if (file.is_open())
    {
        ...
        for (int y_coord = 0; y_coord < image.height(); ++y_coord)
            ...
            {
                unsigned char v =
                  static_cast<unsigned char>(valueAsInt(clampValue(image.getValue(x_coord, y_coord, 0))));
                file << v;
            }
...
template <typename ScalarType, bool Safe>
inline void open_normalize_range_and_SRGB_linearize_RGB(const QString filename,
...
template <typename ScalarType, bool Safe>
inline bool SRGB_compress_adjust_range_and_save_PGM(const Image<1, ScalarType, Safe> &linear_image,
...
template <typename ScalarType, bool Safe>
inline bool SRGB_compress_adjust_range_and_save_RGB(const Image<3, ScalarType, Safe> &linear_image,
                                                    const QString filename)
{
    Image<3, ScalarType, Safe> range_0_1_image;
    convert_linear_srgb_to_gamma_precompensated_srgb(linear_image, range_0_1_image);
...
template <typename ScalarType, bool Safe>
inline bool SRGB_compress_adjust_range_and_save_Y(const Image<1, ScalarType, Safe> &linear_image,
                                                  const QString filename)
{
    Image<1, ScalarType, Safe> range_0_1_image;
    convert_linear_srgb_to_gamma_precompensated_srgb(linear_image, range_0_1_image);
...
//
// template<typename ScalarType, bool Safe>
// inline bool SRGB_compress_adjust_range_and_save_Y(const Image<1,ScalarType,Safe> &linear_image,const QString
// filename)
//{
