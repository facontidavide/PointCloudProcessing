...
#define RAW_MAX_TOKEN_LINE_DESCRIPTOR 32
    ...
    {
        static const char *raw_error_msg[] = { "No errors",
                                               "Can't open file",
    ...
    static int Parseline(int tokennumber, int *tokenorder, char *rawline, float *linebuffer)
    {
        ...
        for (ii = 0; ii < (linelen - 1);)
        {
            ...
            if (isdigit(rawline[ii]))  // is a number
                ...
                change = false;
            else if ((rawline[ii] == '-') || (rawline[ii] == '+') || (rawline[ii] == '-') ||
                     (rawline[ii] == '.'))  // is + - .
                change = false;
            else if (rawline[ii] == 'e')  // is e... is perhaps an exponential ?
            {
                if ((ii > 0) ||
                    (ii < (linelen - 2)))          // if it's at the begin or the end of line, it's not an exponential
                    if (isdigit(rawline[ii - 1]))  // a number before it
                        if (isdigit(rawline[ii + 1]) || (rawline[ii + 1] == '+') ||
                            (rawline[ii + 1] == '-'))  // after it a number a plus or a minus
                            change = false;
            }
    ...
    static int fillHoles(MESH_TYPE &m, size_t rowCount, size_t colCount)
    {
        for (size_t i = 1; i < colCount - 1; ++i)
            for (size_t j = 1; j < rowCount - 1; ++j)
            {
                ...
                {
                    m.vert[ind].P() =
                      (m.vert[indL].P() + m.vert[indR].P() + m.vert[indT].P() + m.vert[indB].P()) * 0.25;
                }
            }
    ...
    */
    static int Open(MESH_TYPE &m, const char *filename, bool triangulate = false, int lineskip = 0,
                    const char *linedesc = "PX PY PZ")
    {
        ...
        m.vn = m.vert.size();
        if (triangulate)
            fillHoles(m, rownumber, colnumber);
