...
class ImporterDAE : public UtilDAE
{
  public:
    ...
    class ColladaVertex : public vcg::Vertex<ColladaTypes, vcg::vertex::Coord3f, /* 12b */
    ...
    class ColladaFace : public vcg::Face<ColladaTypes, vcg::face::VertexRef, /*12b */
  ...
  private:
    static int WedgeNormalAttribute(ColladaMesh& m, const QStringList face, const QStringList wn, const QDomNode wnsrc,
                                    const int meshfaceind, const int faceind, const int component)
    {
        int indnm = -1;
        if (!wnsrc.isNull())
        {
            ...
            m.face[meshfaceind].WN(component) =
              vcg::Point3f(wn.at(indnm * 3).toFloat(), wn.at(indnm * 3 + 1).toFloat(), wn.at(indnm * 3 + 2).toFloat());
        }
        return indnm;
    }

    static int WedgeTextureAttribute(ColladaMesh& m, const QStringList face, int ind_txt, const QStringList wt,
                                     const QDomNode wtsrc, const int meshfaceind, const int faceind,
    ...
    static int WedgeTextureAttribute(typename ColladaMesh::FaceType::TexCoordType& WT, const QStringList faceIndexList,
                                     int ind_txt, const QStringList wt, const QDomNode wtsrc, const int faceind,
                                     const int stride = 2)
    {
        int indtx = -1;
        if (!wtsrc.isNull())
                                   ...
                                   const int meshfaceind, const int faceind, const int component)
    {
        ...
        if (!wcsrc.isNull())
        {
            ...
            m.face[meshfaceind].WC(component) = vcg::Color4b(
              (unsigned char)(wc.at(indcl * 4).toFloat()), (unsigned char)(wc.at(indcl * 4 + 1).toFloat()),
              (unsigned char)(wc.at(indcl * 4 + 2).toFloat()), (unsigned char)(wc.at(indcl * 4 + 3).toFloat()));
        }
        return indcl;
    }
    ...
    static DAEError LoadPolygonalListMesh(QDomNodeList& polylist, ColladaMesh& m, const size_t offset, InfoDAE& info,
                                          QMap<QString, QString>& materialBinding)
    {
        ...
        for (int tript = 0; tript < polylist.size(); ++tript)
        {
            ...
            QDEBUG("******    material id '%s' -> '%s'", qPrintable(materialId),
                   qPrintable(materialBinding[materialId]));
            ...
            if (faceIndexList.size() != 0 && faceSizeList.size() != 0)
            {
                ...
                QDEBUG("*******                 Start Reading faces. Attributes Offsets: offtx %i - offnm %i - offcl "
                       "%i",
                       wa.offtx, wa.offnm, wa.offcl);
                ...
                for (int ff = 0; ff < (int)faceSizeList.size(); ++ff)  // for each polygon
                {
                    ...
                    for (int tt = 0; tt < curFaceVertNum; ++tt)  // for each vertex of the polygon
                    {
                        ...
                        if (faceSizeList.size() < 100)
                            QDEBUG("*******                 Reading face[%3i].V(%i) = %4i  (%i-th of the index list) "
                                   "(face has %i vertices)",
                                   ff, tt, indvt, faceIndexCnt, curFaceVertNum);
                        assert(indvt + offset < m.vert.size());
                        polyTemp._pv[tt] = &(m.vert[indvt + offset]);
                        faceIndexCnt += faceAttributeNum;

                        WedgeTextureAttribute(polyTemp._txc[tt], faceIndexList, ind_txt, wa.wt, wa.wtsrc, jj + wa.offtx,
                                              wa.stride);
                        ...
                        {
                                                        WedgeTextureAttribute(m,face,ind_txt,wa.wt,wa.wtsrc,ff,jj +
                        wa.offtx,tt,wa.stride);
                        }
                                ****************/
        ...
        QDEBUG("****** LoadPolygonalListMesh (final  mesh size vn %i vertsize %i - fn %i facesize %i)", m.vn,
               m.vert.size(), m.fn, m.face.size());
        return E_NOERROR;
    }
     ...
     */
    static DAEError LoadTriangularMesh(QDomNodeList& triNodeList, ColladaMesh& m, const size_t offset, InfoDAE& info,
                                       QMap<QString, QString>& materialBinding)
    {
        if (triNodeList.isEmpty())
            return E_NOERROR;
        ...
        for (int tript = 0; tript < triNodeList.size(); ++tript)
        {
            ...
            if (face.size() != 0)
            {
                ...
                for (int ff = offsetface; ff < (int)m.face.size(); ++ff)
                {
                    ...
                    if (!((m.face[ff].V(0) != m.face[ff].V(1)) && (m.face[ff].V(0) != m.face[ff].V(2)) &&
                          (m.face[ff].V(1) != m.face[ff].V(2))))
                        QDEBUG("********* WARNING face %i, (%i %i %i) is a DEGENERATE FACE!", ff,
                               m.face[ff].V(0) - &m.vert.front(), m.face[ff].V(1) - &m.vert.front(),
                               m.face[ff].V(2) - &m.vert.front());
                }
            }
        }
    ...
    static int LoadControllerMesh(ColladaMesh& m, InfoDAE& info, const QDomElement& geo,
                                  QMap<QString, QString> materialBindingMap, CallBackPos* cb = 0)
    {
        (void)cb;
    ...
    /* before instancing a geometry you can make a binding that allow you to substitute next material names with other
    names.
    this is very useful for instancing the same geometry with different materials. therefore when you encounter a
    material name in a mesh, this name can be a 'symbol' that you have to bind.
    */
    ...
    /*
         ...
         this map is necessary because when using a geometry when it is instanced its material can be bind with
       different names.
         if the map fails you should directly search in the material library.

         */

    static int LoadGeometry(ColladaMesh& m, InfoDAE& info, const QDomElement& geo,
                            QMap<QString, QString>& materialBinding, CallBackPos* cb = 0)
    {
        assert(geo.tagName() == "geometry");
        if (!isThereTag(geo, "mesh"))
            return E_NOMESH;

        if ((cb != NULL) && (((info.numvert + info.numface) % 100) == 0) &&
            !(*cb)((100 * (info.numvert + info.numface)) / (info.numvert + info.numface), "Vertex Loading"))
            return E_CANTOPEN;
        ...
        if (geosrcposarr_size != 0)
        {
            ...
            {
                Point3f positionCoord(geosrcposarr[ii * 3].toFloat(), geosrcposarr[ii * 3 + 1].toFloat(),
                                      geosrcposarr[ii * 3 + 2].toFloat());
                m.vert[vv].P() = positionCoord;

                if (!srcnodenorm.isNull())
                {
                    Point3f normalCoord(geosrcvertnorm[ii * 3].toFloat(), geosrcvertnorm[ii * 3 + 1].toFloat(),
                ...
                /*if (!srcnodecolor.isNull())
                {
                assert((ii * 4 < geosrcvertcol.size()) && (ii * 4 + 1 < geosrcvertcol.size()) && (ii * 4 + 2 <
                geosrcvertcol.size()) && (ii * 4 + 1 < geosrcvertcol.size()));
                m.vert[vv].C() = vcg::Color4b(geosrcvertcol[ii * 4].toFloat(),geosrcvertcol[ii * 4 +
                1].toFloat(),geosrcvertcol[ii * 4 + 2].toFloat(),geosrcvertcol[ii * 4 + 3].toFloat());
                }*/
        ...
        QDEBUG("**** Loading a Geometry Mesh **** (final   mesh size %i %i - %i %i)", m.vn, m.vert.size(), m.fn,
               m.face.size());
        return E_NOERROR;
    }
    ...
    static void AddNodeToMesh(QDomElement node, ColladaMesh& m, Matrix44f curTr, InfoDAE& info)
    {
        ...
        for (int ch = 0; ch < geomNodeList.size(); ++ch)
        {
            ...
            {
                QDEBUG("** instance_geometry with url %s (intial mesh size %i %i T = %i)",
                       qPrintable(instGeomNode.attribute("url")), m.vn, m.fn, m.textures.size());
                ...
                QDEBUG("** instance_geometry with url %s (final mesh size %i %i - %i %i)",
                       qPrintable(instGeomNode.attribute("url")), m.vn, m.vert.size(), m.fn, m.face.size());
            }
        }
        ...
        for (int ch = 0; ch < controllerNodeList.size(); ++ch)
        {
            ...
            if (instContrNode.parentNode() == node)  // process only direct child
            {
                ...
                QDomNode refNode =
                  findNodeBySpecificAttributeValue(*(info.doc), "controller", "id", controllerNode_url);
    ...
    static Matrix44f getTransfMatrixFromNode(const QDomElement parentNode)
    {
        ...
        if (!translationNode.isNull())
            ParseTranslation(transM, translationNode);
        if (!rotationList.empty())
            ParseRotationMatrix(rotM, rotationList);
  ...
  public:
    ...
    static int Open(OpenMeshType& m, const char* filename, InfoDAE& info, CallBackPos* cb = 0)
    {
        ...
        <visual_scene> can be directly written there (check!) or instanced from their definition in the
      <library_visual_scene>
      ...
      each <node> contains
        ...
        An instance of geometry node should contain the <mesh> node and as a son of the <instance geometry> the material
      node (again referenced from a library)
    ...
    static bool LoadMask(const char* filename, InfoDAE& info)
    {
        ...
        for (int scn = 0; scn < scn_size; ++scn)
        {
            ...
            for (int instscn = 0; instscn < instscn_size; ++instscn)
            {
                ...
                for (int chdind = 0; chdind < visscn_child.size(); ++chdind)
                {
                    // QDomNodeList& geoinst =
                    // visscn_child.at(chdind).toElement().elementsByTagName("instance_geometry");
                    ...
                    if (geoinst_size != 0)
                    {
                        ...
                        for (int geoinst_ind = 0; geoinst_ind < geoinst_size; ++geoinst_ind)
                        {
                            ...
                            for (int vert = 0; vert < vertlist.size(); ++vert)
                            {
                                QDomNode no;
                                no =
                                  findNodeBySpecificAttributeValue(vertlist.at(vert), "input", "semantic", "POSITION");
                                QString srcurl;
                                ...
                                no =
                                  findNodeBySpecificAttributeValue(vertlist.at(vert), "input", "semantic", "TEXCOORD");
                                if (!no.isNull())
                            ...
                            for (unsigned int tt = 0; tt < 3; ++tt)
                            {
                                ...
                                for (int face = 0; face < facelist.size(); ++face)
                                {
                                    ...
                                    QDomNode no;
                                    no = findNodeBySpecificAttributeValue(facelist.at(face), "input", "semantic", "NORM"
                                                                                                                  "AL");
                                    if (!no.isNull())
                                        bHasPerWedgeNormal = true;
                                    no = findNodeBySpecificAttributeValue(facelist.at(face), "input", "semantic",
                                                                          "TEXCOORD");
                                    if (!no.isNull())
