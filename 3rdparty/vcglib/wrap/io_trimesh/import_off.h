    ...
    */
    static const char *ErrorMsg(int message_code)
    {
        static const char *error_msg[] = {
            ...
            "File with high dimensional vertexes are not supported",
    ...
    static int Open(MESH_TYPE &mesh, const char *filename, int &loadmask, CallBackPos *cb = 0)
    {
        ...
        /*
         ...
         x[0]  y[0]  z[0]	# Vertices, possibly with normals, colors, and/or texture coordinates, in that order,  if
         the prefixes N, C, ST are present.
            ...
            if (isColorDefined)
            {
                ...
                if (tri::HasPerVertexColor(mesh))
                {
                    ...
                    else if (nb_color_components == 3)
                    {
                        ...
                        {
                            // integers
                            unsigned char r = static_cast<unsigned char>(atoi(tokens[k].c_str()));
                            unsigned char g = static_cast<unsigned char>(atoi(tokens[k + 1].c_str()));
                            unsigned char b = static_cast<unsigned char>(atoi(tokens[k + 2].c_str()));
                    ...
                    else if (nb_color_components == 4)
                    {
                        ...
                        if (tokens[k].find(".") == size_t(-1))
                        {
                            ...
                            unsigned char g = static_cast<unsigned char>(atoi(tokens[k + 1].c_str()));
                            unsigned char b = static_cast<unsigned char>(atoi(tokens[k + 2].c_str()));
                            unsigned char a = static_cast<unsigned char>(atoi(tokens[k + 3].c_str()));
            ...
            else
            {
                ...
                for (int j = 0; j <= vert_per_face - 3; j++)
                {
                    ...
                    if (tri::HasPerFaceFlags(mesh))
                    {
                        ...
                        if (j > 0)
                            mesh.face[f + j].SetF(0);
                        if (j < vert_per_face - 3)
                            mesh.face[f + j].SetF(2);
