...
The stl format is quite simple and rather un-flexible. It just stores, in ascii or binary the, unindexed, geometry of
the faces.
...
class ImporterSTL
{
  public:
    ...
    static int Open(OpenMeshType &m, const char *filename, CallBackPos *cb = 0)
    {
        ...
        fclose(fp);
        if (binary)
            return OpenBinary(m, filename, cb);
        else
    ...
    static int OpenAscii(OpenMeshType &m, const char *filename, CallBackPos *cb = 0)
    {
        ...
        {
            if (cb && (++cnt) % 1000)
                cb(int(double(ftell(fp)) * 100.0 / fileLen), "STL Mesh Loading");
            ret = fscanf(fp, "%*s %*s %f %f %f\n", &f.n.X(), &f.n.Y(), &f.n.Z());  // --> "facet normal 0 0 0"
            if (ret != 3)
            {
                // we could be in the case of a multiple solid object, where after a endfaced instead of another facet
                // we have to skip two lines:
                //     endloop
                //	 endfacet
