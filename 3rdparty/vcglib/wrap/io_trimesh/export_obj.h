  ...
  public:
    ...
    */
    static int Save(SaveMeshType &m, const char *filename, int mask, CallBackPos *cb = 0)
    {
        ...
        if (mask & vcg::tri::io::Mask::IOM_WEDGTEXCOORD && mask & vcg::tri::io::Mask::IOM_VERTTEXCOORD)
        {
            mask &= ~vcg::tri::io::Mask::IOM_VERTTEXCOORD;
        }
        if (mask & vcg::tri::io::Mask::IOM_WEDGCOLOR && mask & vcg::tri::io::Mask::IOM_VERTCOLOR)
        {
        ...
        fprintf(fp, "# Object %s\n#\n# Vertices: %d\n# Faces: %d\n#\n####\n", fn.substr(LastSlash + 1).c_str(), m.vn,
                m.fn);
        ...
        for (vi = m.vert.begin(); vi != m.vert.end(); ++vi)
            if (!(*vi).IsD())
            {
                ...
                if (mask & Mask::IOM_VERTCOLOR)
                    fprintf(fp, " %f %f %f", double((*vi).C()[0]) / 255., double((*vi).C()[1]) / 255.,
                            double((*vi).C()[2]) / 255.);
                fprintf(fp, "\n");
        ...
        for (fi = m.face.begin(); fi != m.face.end(); ++fi)
            if (!(*fi).IsD())
            {
                ...
                for (int k = 0; k < (*fi).VN(); k++)
                {
                    ...
                    if (mask & Mask::IOM_WEDGNORMAL)
                        vn =
                          GetIndexVertexNormal(m, NormalVertex, (*fi).V(k)->cN());  // index of vertex normal per face.
                    if (mask & Mask::IOM_VERTNORMAL)
                        vn = vInd;
    ...
    */
    inline static int GetIndexVertexTexture(std::map<TexCoord2<float>, int> &mapTexToInt,
    ...
    */
    inline static int GetIndexVertexNormal(SaveMeshType & /*m*/, std::map<Point3f, int> &mapNormToInt,
                                           const Point3f &norm)
    {
    ...
    */
    inline static bool AddNewTextureCoord(std::map<vcg::TexCoord2<float>, int> &m, const vcg::TexCoord2<float> &wt,
                                          int value)
    {
        int index = m[wt];
        if (index == 0)
