  ...
  public:
    ...
    class Info
    {
      public:
        ...
        // it returns the current position, and formats a string with a description of what th efunction is doing
        // (loading vertexes, faces...)
        CallBackPos *cb;
    ...
    static const char *ErrorMsg(int error)
    {
        static const char *obj_error_msg[] = {
            ...
            "Some materials definitions were not found, a default white material is used where no material was "
            "available",                                        // 2
            ...
            if (numTokens > 0)
            {
                ...
                if (header.compare("v") == 0)  // vertex
                {
                    ...
                    if (((oi.mask & vcg::tri::io::Mask::IOM_VERTCOLOR) != 0) && (m.HasPerVertexColor()))
                    {
                        if (numTokens >= 7)
                        {
                            ScalarType rf(atof(tokens[4].c_str())), gf(atof(tokens[5].c_str())),
                              bf(atof(tokens[6].c_str()));
                            ...
                            unsigned char alpha =
                              (unsigned char)((numTokens >= 8 ? (ScalarType)atof(tokens[7].c_str()) : 1) * scaling);
                    ...
                    if ((cb != NULL) && (((numTriangles + numVertices) % 100) == 0) &&
                        !(*cb)((100 * (numTriangles + numVertices)) / numVerticesPlusFaces, "Vertex Loading"))
                        return E_ABORTED;
                }
                ...
                else if ((header.compare("f") == 0) || (header.compare("q") == 0))  // face
                {
                    ...
                    if (numTokens < 4)
                        return E_LESS_THAN_3VERTINFACE;
...
#ifdef __gl_h_
                    {
                        ...
                        {
                            qDebug("Warning there is a degenerate poligon of %i verteces that was triangulated into %i "
                                   "triangles",
                                   vertexesPerFace, int(indexTriangulatedVect.size() / 3));
                            for (size_t qq = 0; qq < polygonVect[0].size(); ++qq)
                                qDebug("      (%f %f %f)", polygonVect[0][qq][0], polygonVect[0][qq][1],
                                       polygonVect[0][qq][2]);
                            for (size_t qq = 0; qq < tokens.size(); ++qq)
                                qDebug("<%s>", tokens[qq].c_str());
                        }
#endif
                        // qDebug("Triangulated a face of %i vertexes into %i
                        // triangles",polygonVect[0].size(),indexTriangulatedVect.size());
                        ...
                        for (size_t pi = 0; pi < indexTriangulatedVect.size(); pi += 3)
                        {
                            ...
                            if ((i0 + 1) % vertexesPerFace == i1)
                                ff.edge[0] = false;
                            else
                                ff.edge[0] = true;
                            if ((i1 + 1) % vertexesPerFace == i2)
                                ff.edge[1] = false;
                            else
                                ff.edge[1] = true;
                            if ((i2 + 1) % vertexesPerFace == i0)
                                ff.edge[2] = false;
                            else
                                ff.edge[2] = true;
...
#else
                    {
                        ...
                        while (iVertex < vertexesPerFace)  // add other triangles
                        {
                            ...
                            if ((ff.v[0] == v4_index) || (ff.v[2] == v4_index))
                                result = E_VERTICES_WITH_SAME_IDX_IN_FACE;
                            ffNew.v[1] = ff.v[2];
                            ffNew.v[2] = v4_index;
    ...
    */
    ...
    inline static const void SplitToken(std::string token, int &vId, int &nId, int &tId, int mask)
    {
        ...
        if ((mask & Mask::IOM_WEDGTEXCOORD) && (mask & Mask::IOM_WEDGNORMAL))
            SplitVVTVNToken(token, vertex, texcoord, normal);
        if (!(mask & Mask::IOM_WEDGTEXCOORD) && (mask & Mask::IOM_WEDGNORMAL))
            SplitVVNToken(token, vertex, normal);
        if ((mask & Mask::IOM_WEDGTEXCOORD) && !(mask & Mask::IOM_WEDGNORMAL))
            SplitVVTToken(token, vertex, texcoord);
        if (!(mask & Mask::IOM_WEDGTEXCOORD) && !(mask & Mask::IOM_WEDGNORMAL))
            SplitVToken(token, vertex);

        vId = atoi(vertex.c_str()) - 1;
        if (mask & Mask::IOM_WEDGTEXCOORD)
            tId = atoi(texcoord.c_str()) - 1;
        if (mask & Mask::IOM_WEDGNORMAL)
            nId = atoi(normal.c_str()) - 1;
    }
    ...
    inline static const void SplitVVTVNToken(std::string token, std::string &vertex, std::string &texcoord,
                                             std::string &normal)
    {
        vertex.clear();
        texcoord.clear();
    ...
    static bool LoadMask(const char *filename, Info &oi)
    {
        ...
        while (!stream.eof())
        {
            ...
            if (line.size() > 2)
            {
                ...
                {
                    if ((line[0] == 'f') || (line[0] == 'q'))
                        oi.numFaces++;
                    else if (line[0] == 'u' && line[1] == 's')
                        bHasPerFaceColor = true;  // there is a usematerial so add per face color
                }
            }
        }
        oi.mask = 0;
