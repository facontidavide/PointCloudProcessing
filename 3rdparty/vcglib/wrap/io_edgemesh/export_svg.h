  ...
  public:
    ...
    static void WriteXmlBody(FILE *fpo, EdgeMeshType &mp, SVGProperties &pro, int meshIndex)
    {
        ...
        fprintf(fpo, "  <rect width= \" %fcm \" height= \" %fcm \" x=\"%fcm \" y=\"%fcm \" "
                ...
                pro.sizeCm[0], pro.sizeCm[1], pro.marginCm[0] + colInd * (pro.sizeCm[0] + pro.marginCm[0]),
                pro.marginCm[1] + rowInd * (pro.sizeCm[1] + pro.marginCm[1]));
        fprintf(fpo, "<g stroke=\"%s\" stroke-linecap=\"%s\" stroke-width = \"%fpt\" > \n", pro.strokeColor.c_str(),
                pro.strokeLineCap.c_str(), pro.lineWidthPt / 100.0f);
        fprintf(fpo, "  <svg id = \"SliceNum%d\" viewBox=\"-1000 -1000 2000 2000\" width=\"%fcm\" height=\"%fcm\" "
                     "x=\"%fcm\" y=\"%fcm\" >\n",
                meshIndex, pro.sizeCm[0], pro.sizeCm[1], pro.marginCm[0] + colInd * (pro.sizeCm[0] + pro.marginCm[0]),
                pro.marginCm[1] + rowInd * (pro.sizeCm[1] + pro.marginCm[1]));
