...
#define MAX_USER_DATA 256
    ...
    static const PropDescriptor &VertDesc(int i)
    {
        const static PropDescriptor pv[9] = {
            { "vertex", "x", ply::T_FLOAT, PlyType<ScalarType>(), offsetof(LoadPly_VertAux<ScalarType>, p[0]), 0, 0, 0,
              0, 0 },
            { "vertex", "y", ply::T_FLOAT, PlyType<ScalarType>(), offsetof(LoadPly_VertAux<ScalarType>, p[1]), 0, 0, 0,
              0, 0 },
            { "vertex", "z", ply::T_FLOAT, PlyType<ScalarType>(), offsetof(LoadPly_VertAux<ScalarType>, p[2]), 0, 0, 0,
              0, 0 },
            { "vertex", "flags", ply::T_INT, ply::T_INT, offsetof(LoadPly_VertAux<ScalarType>, flags), 0, 0, 0, 0, 0 },
            { "vertex", "quality", ply::T_FLOAT, ply::T_FLOAT, offsetof(LoadPly_VertAux<ScalarType>, q), 0, 0, 0, 0,
              0 },
            ...
            { "vertex", "confidence", ply::T_FLOAT, ply::T_FLOAT, offsetof(LoadPly_VertAux<ScalarType>, q), 0, 0, 0, 0,
              0 },
        };
        return pv[i];
    }
    ...
    static const PropDescriptor &TetraDesc(int i)
    {
        const static PropDescriptor qf[10] = {
            { "tetra", "vertex_indices", ply::T_INT, ply::T_INT, offsetof(LoadPly_TetraAux, v), 1, 0, ply::T_UCHAR,
              ply::T_UCHAR, offsetof(LoadPly_TetraAux, size) },
            ...
            { "tetra", "texcoord", ply::T_FLOAT, ply::T_FLOAT, offsetof(LoadPly_TetraAux, texcoord), 1, 0, ply::T_UCHAR,
              ply::T_UCHAR, offsetof(LoadPly_TetraAux, ntexcoord) },
            { "tetra", "color", ply::T_FLOAT, ply::T_FLOAT, offsetof(LoadPly_TetraAux, colors), 1, 0, ply::T_UCHAR,
              ply::T_UCHAR, offsetof(LoadPly_TetraAux, ncolors) },
            ...
            { "tetra", "vertex_index", ply::T_INT, ply::T_INT, offsetof(LoadPly_TetraAux, v), 1, 0, ply::T_UCHAR,
              ply::T_CHAR, offsetof(LoadPly_TetraAux, size) },
        };
        return qf[i];
    }
    ...
    static int Open(OpenMeshType &m, const char *filename, PlyInfo &pi)
    {
        ...
        if (VertexType::HasQuality())
        {
            if (pf.AddToRead(VertDesc(4)) != -1 || pf.AddToRead(VertDesc(8)) != -1)
        ...
        for (int i = 0; i < int(pf.elements.size()); i++)
        {
            ...
            else if (!strcmp(pf.ElemName(i), "tetra")) /************************************************************/
            {
                ...
                for (k = 0; k < pi.fdn; k++)
                    memcpy((char *)(&(*fi)) + pi.TetraData[k].offset1, (char *)(&fa) + FPV[k].offset1,
                           FPV[k].memtypesize());
                ++fi;
            }
            //		}
    ...
    bool LoadMask(const char *filename, int &mask)
    {
        ...
        if (pf.AddToRead(VertDesc(0)) != -1 && pf.AddToRead(VertDesc(1)) != -1 && pf.AddToRead(VertDesc(2)) != -1)
            mask |= ply::PLYMask::PM_VERTCOORD;

        if (pf.AddToRead(VertDesc(3)) != -1)
            mask |= ply::PLYMask::PM_VERTFLAGS;
        if (pf.AddToRead(VertDesc(4)) != -1)
            mask |= ply::PLYMask::PM_VERTQUALITY;
        ...
        if (pf.AddToRead(TetraDesc(0)) != -1)
            mask |= ply::PLYMask::PM_TETRAINDEX;
        if (pf.AddToRead(TetraDesc(1)) != -1)
            mask |= ply::PLYMask::PM_TETRAFLAGS;

        if (pf.AddToRead(TetraDesc(2)) != -1)
            mask |= ply::PLYMask::PM_TETRAQUALITY;
        ...
        if (pf.AddToRead(TetraDesc(4)) != -1)
            mask |= ply::PLYMask::PM_WEDGCOLOR;
        if ((pf.AddToRead(TetraDesc(6)) != -1) && (pf.AddToRead(TetraDesc(7)) != -1) &&
            (pf.AddToRead(TetraDesc(8)) != -1))
            mask |= ply::PLYMask::PM_TETRACOLOR;
