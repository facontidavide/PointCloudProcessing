...
class Add_Ons
{
  ...
  public:
    ...
    static void glArrow(Point3f tail, Point3f head, float body_width, float head_lenght, float head_width,
                        int body_slice = 10, int head_slice = 10, bool useDisplList = true)
    {
        if (tail != head)
        {
            // assert(!glGetError());
