...
template <class MESH_TYPE, bool partial = false,
          class FACE_POINTER_CONTAINER = std::vector<typename MESH_TYPE::FacePointer> >
  ...
  public:
    ...
    void DrawFill()
    {
        ...
        else
        {
            ...
            if (tm == TMPerVert &&
                !TMId.empty())  // in the case of per vertex tex coord we assume that we have a SINGLE texture.
            {
                curtexname = 0;
                glEnable(GL_TEXTURE_2D);
                glBindTexture(GL_TEXTURE_2D, TMId[curtexname]);
    ...
    /// Utility function that computes in eyespace the current distance between the camera and the center of the bbox of
    /// the mesh
    ...
    void DrawPoints()
    {
        ...
        if (GetHintParami(HNPPointSmooth) > 0)
        ...
        else
        {
            ...
            if (cm == CMPerVert)
            {
                ...
                if (m->vert.size() != 0)
                    glColorPointer(4, GL_UNSIGNED_BYTE, sizeof(typename MESH_TYPE::VertexType),
                                   &(m->vert.begin()->C()[0]));
            }
...
void Crease(MESH_TYPE &m, typename MESH_TYPE::scalar_type angleRad)
{
    ...
    enum
    {
        VISITED_0 = MESH_TYPE::FaceType::USER0,
    ...
    for (fi = m.face.begin(); fi != m.face.end(); ++fi)
        if (!(*fi).IsD())
            (*fi).Supervisor_Flags() &= (~(VISITED_0 | VISITED_1 | VISITED_2));
            ...
            for (int j = 0; j < 3; ++j)
                if (!((*fi).Supervisor_Flags() & (vis[j])))
                {
                    ...
                    do
                    {
                        ...
                        if (ps < cosangle)
                        {
                            //									VCTRACE("splitting faces %i-%i edge %i vert
