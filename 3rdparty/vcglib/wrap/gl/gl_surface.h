  ...
  public:
    ...
    bool set(int width, int height, const std::vector<GLenum>& color_formats, GLenum depth_format)
    {
        ...
        for (size_t i = 0; i < this->color_texs.size(); ++i)
        {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, this->color_formats[i], this->width, this->height, 0, GL_LUMINANCE,
                         GL_UNSIGNED_BYTE, 0);
        }
        ...
        if (this->depth_format != GL_NONE)
        {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, this->depth_format, this->width, this->height, 0, GL_DEPTH_COMPONENT,
                         GL_UNSIGNED_INT, 0);
        }
    ...
    GLenum get_color_attachment_format(int attachment) const
    {
        if (!this->is_valid())
            return GL_NONE;
        if ((attachment < 0) || (attachment >= this->color_attachments_count()))
            return GL_NONE;
    ...
    {
        if (!this->begin_read_color(attachment))
            return false;

        glTexImage2D(GL_TEXTURE_2D, 0, this->color_formats[attachment], this->width, this->height, 0, format, type,
                     pixels);
