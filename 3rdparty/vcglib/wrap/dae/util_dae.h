...
class UtilDAE
{
  public:
    ...
    class MyPolygon
    {
      public:
        ...
        MyPolygon(int n) : _nvert(n), _pv(_nvert), _txc(_nvert)
        {
        }
    };
    ...
    class PolygonalMesh
    {
      public:
        ...
        void usePerWedgeAttributes(PERWEDGEATTRIBUTETYPE att, const unsigned int multitexture = 1,
                                   const unsigned int multicolor = 1)
        {
            if (att != NONE)
            {
                ...
                {
                    if (att & MULTICOLOR)
                        itp->usePerWedgeColor(multicolor);
                    if (att & MULTITEXTURECOORD)
                        itp->usePerWedgeMultiTexture(multitexture);
                    if (att & NORMAL)
                        itp->usePerWedgeNormal();
                }
            }
        }
  ...
  protected:
    ...
    inline static QDomNode findNodeBySpecificAttributeValue(const QDomNodeList& ndl, const QString& attrname,
    ...
    inline static QDomNode findNodeBySpecificAttributeValue(const QDomNode n, const QString& tag,
    ...
    inline static QDomNode findNodeBySpecificAttributeValue(const QDomDocument n, const QString& tag,
                                                            const QString& attrname, const QString& attrvalue)
    {
    ...
    inline static QDomNode attributeSourcePerSimplex(const QDomNode n, const QDomDocument startpoint,
                                                     const QString& sem)
    {
    ...
    inline static void valueStringList(QStringList& res, const QDomNode srcnode, const QString& tag)
    {
        ...
        if (res.empty())
        {
            qDebug("Warning valueStringList returned and emtpy list. nothing inside element with tag '%s'",
                   qPrintable(tag));
            return;
        }
        if (res.last() == "")
        ...
        //			if(emptyCount>0) qDebug("- - - - - - - - valueStringList: Removed %i null strings when parsing tag
        //%s",emptyCount,qPrintable(tag));
    ...
    inline static bool removeChildNodeList(QDomNodeList& nodelst, const QString& tag = "",
                                           const QString& attribname = "", const QString& attribvalue = "")
    {
    ...
    inline static bool removeChildNode(QDomNode node, const QString& tag = "", const QString& attribname = "",
    ...
    static void ParseRotationMatrix(vcg::Matrix44f& m, const std::vector<QDomNode>& t)
    {
        ...
        for (unsigned int ii = 0; ii < t.size(); ++ii)
        {
            ...
            if (rtl.last() == "")
                rtl.removeLast();
            assert(rtl.size() == 4);
            tmp.SetRotateDeg(rtl.at(3).toFloat(),
                             vcg::Point3f(rtl.at(0).toFloat(), rtl.at(1).toFloat(), rtl.at(2).toFloat()));
            rotTmp = rotTmp * tmp;
        }
        m = m * rotTmp;
    }
    ...
    inline static int findStringListAttribute(QStringList& list, const QDomNode node, const QDomNode poly,
                                              const QDomDocument startpoint, const char* token)
    {
        int offset = 0;
        if (!node.isNull())
        {
    ...
    */
    ...
    inline static QDomNode textureFinder(QString& boundMaterialName, QString& textureFileName, const QDomDocument doc)
    {
        ...
        if (lib_mat.size() != 1)
            return QDomNode();
        QDomNode material =
          findNodeBySpecificAttributeValue(lib_mat.at(0), QString("material"), QString("id"), boundMaterialName);
        if (material.isNull())
            return QDomNode();
        ...
        if (libraryImageNodeList.size() != 1)
            return QDomNode();
        QDomNode imageNode =
          findNodeBySpecificAttributeValue(libraryImageNodeList.at(0), QString("image"), QString("id"), img_id);
        ...
        qDebug("====== the image '%s' has a %i init_from nodes text '%s'", qPrintable(img_id), initfromNode.size(),
