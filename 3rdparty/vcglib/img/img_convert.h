...
inline void convert_Y_to_RGB(const Image<1, SrcScalarType, SrcSafe> &source,
                             Image<3, DestScalarType, DestSafe> &destination)
{
    ...
    {
        if (!source.isValid())
            throw ImageException("Invalid source image");
    }
...
// Y to RGBA
template <typename SrcScalarType, bool SrcSafe, typename DestScalarType, bool DestSafe>
inline void convert_Y_to_RGBA(const Image<1, SrcScalarType, SrcSafe> &source,
                              Image<4, DestScalarType, DestSafe> &destination, DestScalarType alpha_value = 255.0f)
{
    assert(source.isValid());
    if (SrcSafe || DestSafe)
    {
...
// RGB to Y
template <typename SrcScalarType, bool SrcSafe, typename DestScalarType, bool DestSafe>
inline void convert_RGB_to_Y(const Image<3, SrcScalarType, SrcSafe> &source,
...
// RGB to RGBA
template <typename SrcScalarType, bool SrcSafe, typename DestScalarType, bool DestSafe>
inline void convert_RGB_to_RGBA(const Image<3, SrcScalarType, SrcSafe> &source,
                                Image<4, DestScalarType, DestSafe> &destination, DestScalarType alpha_value = 255.0f)
{
    assert(source.isValid());
    if (SrcSafe || DestSafe)
    ...
    for (int y_coord = 0; y_coord < source.height(); ++y_coord)
        for (int x_coord = 0; x_coord < source.width(); ++x_coord)
        {
            destination.setValue(x_coord, y_coord, 0,
                                 static_cast<DestScalarType>(source.getValue(x_coord, y_coord, 0)));
            destination.setValue(x_coord, y_coord, 1,
                                 static_cast<DestScalarType>(source.getValue(x_coord, y_coord, 1)));
            destination.setValue(x_coord, y_coord, 2,
                                 static_cast<DestScalarType>(source.getValue(x_coord, y_coord, 2)));
            destination.setValue(x_coord, y_coord, 3, alpha_value);
        }
    destination.attributes.setRange(source.attributes);
    destination.attributes.setGamma(source.attributes);
...
// RGBA to Y
template <typename SrcScalarType, bool SrcSafe, typename DestScalarType, bool DestSafe>
inline void convert_RGBA_to_Y(const Image<4, SrcScalarType, SrcSafe> &source,
                              Image<1, DestScalarType, DestSafe> &destination)
{
    assert(source.isValid());
    if (SrcSafe || DestSafe)
...
// RGBA to RGB
template <typename SrcScalarType, bool SrcSafe, typename DestScalarType, bool DestSafe>
inline void convert_RGBA_to_RGB(const Image<4, SrcScalarType, SrcSafe> &source,
                                Image<3, DestScalarType, DestSafe> &destination)
{
    assert(source.isValid());
    if (SrcSafe || DestSafe)
...
inline void convert_range_0_255_to_0_1(const Image<Channels, SrcScalarType, SrcSafe> &source,
                                       Image<Channels, DestScalarType, DestSafe> &destination)
{
    ...
    if (SrcSafe || DestSafe)
    {
        if (!source.isValid())
            throw ImageException("Invalid source image");
        if (!source.attributes.hasRange(0, 255))
            throw ImageException("Invalid range attribute");
    }
    destination.setZero(source.width(), source.height());
    for (int offset = 0; offset < source.dataValuesSize(); offset++)
        destination.dataValues()[offset] =
          static_cast<DestScalarType>(source.dataValues()[offset]) / DestScalarType(255.0);
    destination.attributes = source.attributes;
    destination.attributes.setRange(0, 1);
}

template <int Channels, typename SrcScalarType, bool SrcSafe, typename DestScalarType, bool DestSafe>
inline void convert_range_0_1_to_0_255(const Image<Channels, SrcScalarType, SrcSafe> &source,
                                       Image<Channels, DestScalarType, DestSafe> &destination)
{
    assert(source.isValid());
    ...
    if (SrcSafe || DestSafe)
    {
        if (!source.isValid())
            throw ImageException("Invalid source image");
        if (!source.attributes.hasRange(0, 1))
            throw ImageException("Invalid range attribute");
    }
    destination.setZero(source.width(), source.height());
    for (int offset = 0; offset < source.dataValuesSize(); offset++)
