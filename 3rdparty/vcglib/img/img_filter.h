...
inline void normalize(Image<Channels, ScalarType, Safe> &image)
{
    ...
    if (Safe)
    {
        ...
        if (!image.attributes.hasRange(0, 255))
...
inline void convolution(const Image<Channels, SrcScalarType, SrcSafe> &source,
                        Image<Channels, DestScalarType, DestSafe> &destination, const DestScalarType *matrix,
                        int matrix_width, int matrix_height)
{
    ...
    if (SrcSafe || DestSafe)
    {
        if (!source.isValid())
            throw ImageException("Invalid image");
        if (matrix == NULL)
            throw ImageException("NULL convolution matrix");
        if (!((matrix_width > 0) && ((matrix_width) % 2 == 1)))
            throw ImageException("Matrix width must be a positive odd number");
        if (!((matrix_height > 0) && ((matrix_height) % 2 == 1)))
            throw ImageException("Matrix height must be a positive odd number");
    }
            ...
            {  // colonne immagine
                ...
                for (int my = y - y_radius; my <= y + y_radius; my++)      // righe matrice
                    for (int mx = x - x_radius; mx <= x + x_radius; mx++)  // colonne matrice
                        sum +=
                          matrix[offset++] * static_cast<DestScalarType>(source.getValueAsClamped(mx, my, channel));
...
template <int Channels, typename SrcScalarType, bool SrcSafe, typename DestScalarType, bool DestSafe>
inline void boxFilter(const Image<Channels, SrcScalarType, SrcSafe> &source,
                      Image<Channels, DestScalarType, DestSafe> &destination, int radius)
{
    assert(radius > 0);
    if (SrcSafe || DestSafe)
    {
        if (radius <= 0)
            throw ImageException("Nonpositive radius");
    }
...
template <int Channels, typename SrcScalarType, bool SrcSafe, typename DestScalarType, bool DestSafe>
inline void GaussianSmooth(const Image<Channels, SrcScalarType, SrcSafe> &source,
                           Image<Channels, DestScalarType, DestSafe> &destination, const int radius)
{
    assert(radius > 0.0f);
    if (SrcSafe || DestSafe)
    {
...
template <int Channels, typename SrcScalarType, bool SrcSafe, typename DestScalarType, bool DestSafe>
inline void LaplacianFilter(const Image<Channels, SrcScalarType, SrcSafe> &source,
...
template <int Channels, typename SrcScalarType, bool SrcSafe, typename DestScalarType, bool DestSafe>
inline void LoGFilter(const Image<Channels, SrcScalarType, SrcSafe> &source,
                      Image<Channels, DestScalarType, DestSafe> &destination, int radius)
{
    assert(radius > 0.0f);
    if (SrcSafe || DestSafe)
...
template <int Channels, typename SrcScalarType, bool SrcSafe, typename DestScalarType, bool DestSafe>
inline void DoGFilter(const Image<Channels, SrcScalarType, SrcSafe> &source,
                      Image<Channels, DestScalarType, DestSafe> &destination, int radius1, int radius2)
{
    assert(radius1 > 0.0f);
    assert(radius2 > 0.0f);
    ...
    {
        if (radius1 <= 0.0f)
            throw ImageException("Nonpositive radius1");
        if (radius2 <= 0.0f)
            throw ImageException("Nonpositive radius2");
        if (radius2 <= radius1)
            throw ImageException("radius2 is less than radius1");
    }
...
template <int Channels, typename SrcScalarType, bool SrcSafe, typename DestScalarType, bool DestSafe>
inline void UnsharpMask(const Image<Channels, SrcScalarType, SrcSafe> &source,
                        Image<Channels, DestScalarType, DestSafe> &destination, int radius, float factor)
{
    assert(radius > 0);
    assert(factor > 0.0f);
    if (SrcSafe || DestSafe)
    {
        if (radius <= 0.0f)
            throw ImageException("Nonpositive radius");
        if (factor <= 0.0f)
            throw ImageException("Nonpositive factor");
    }
...
template <int Channels, typename SrcScalarType, bool SrcSafe, typename DestScalarType, bool DestSafe>
inline void medianFilter(const Image<Channels, SrcScalarType, SrcSafe> &source,
                         Image<Channels, DestScalarType, DestSafe> &destination, int radius)
{
    assert(source.isValid());
    assert(radius > 0);
    ...
    {  // canali immagine
        ...
        {  // righe immagine
            ...
            {  // colonne immagine
                ...
                if ((s % 2) == 0)
                {  // even s: mean of the 2 middle elements
...
inline void channels_mean(const img::Image<Channels, ScalarType1, Safe1> &channels_image,
                          img::Image<1, ScalarType2, Safe2> &mean_image)
{
    assert(channels_image.isValid());
    if (Safe1 || Safe2)
    {
        if (!channels_image.isValid())
            throw img::ImageException("channels_image rgb image");
    }
