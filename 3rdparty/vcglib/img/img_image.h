...
/*! \brief the generic image class
  ...
  The image class is templated over three parameters:
   - The number of the image channels: an image can have from one to an arbitrary large number of channels, i.e. values
  per pixel. The default  is to have 3 color channels.
   - The type of the values: the image stores the pixels' values in floating-point variables. The default is to have
  double floating point precision, the other option is to have single floating point precision. Static assertions are
  used to ensure that the value type of an image is a floating  point number.
   - The safeness: the image throws runtime exceptions when its member functions are called with "wrong" parameters.
  This behavior, active by default, can be disabled in order to speed up the computation. The parameter correctness is
  also independently checked with the dynamic assertions mechanism that can be disabled by compiling the code in
  "release" mode.

  The image data is packed by the pixel, to optimize multichannel processing efficiency. The image class interface is
  restricted to pixel and metadata access, processing and I/O are entirely delegated to external functions. The data is
  accessible in multiple ways: value and pixel wise, using float coordinates with nearest and bilinear interpolation,
  viewing the image with clamping, direct access to the data and so on.

  An auxiliary structured attribute contains all the metadata information, currently this data consists in the
  specification of the numeric range of the values, the image color space, the white point and the gamma compression of
  the image. The various functions checks these data before processing, failing if the image is not compatible with the
  operation in course.
*/
  ...
  public:
    ...
    */
    ...
    /*! \brief (deep) copy constructor when all template parameters matches
      ...
      An explicit copy constructor is needed because when all template parameters matches the templated copy constructor
      is not consideredand and a wrong copy constructor is synthetized by compiler.
    ...
    */
    Image(const Image<Channels, ScalarType, Safe> &image)  // copy constructor (deep copy)
    {
        ...
        {
            if (image._width <= 0)
                throw ImageException("Image(Image): Nonpositive width");
            if (image._height <= 0)
                throw ImageException("Image(Image): Nonpositive height");
            if (image._data == NULL)
                throw ImageException("Image(Image): NULL data");
        }
        _width = image._width;
    ...
    */
    Image(int arg_width, int arg_height) : _width(arg_width), _height(arg_height), _data(NULL), attributes()
    {
        STATIC_ASSERT(Channels > 0);
        STATIC_FLOAT_OR_DOUBLE_TYPECHECK(ScalarType);
        ...
        {
            if (arg_width <= 0)
                throw ImageException("Image(int,int): Nonpositive width");
            if (arg_height <= 0)
                throw ImageException("Image(int,int): Nonpositive height");
        }
        _data = new ScalarType[Channels * _width * _height];
        for (int offset = 0; offset < Channels * _width * _height; ++offset)
            _data[offset] = 0.0f;
  ...
  public:
    ...
    */
    ...
    inline Image<Channels, ScalarType, Safe> &operator=(const Image<Channels, OtherScalarType, OtherSafe> &image)
    {
        ...
        {
            if (image._width <= 0)
                throw ImageException("operator =: Nonpositive width");
            if (image._height <= 0)
                throw ImageException("operator =: Nonpositive height");
            if (image._data == NULL)
                throw ImageException("operator =: NULL data");
        }
        _width = image._width;
        _height = image._height;
        _data = new ScalarType[Channels * _width * _height];
    ...
    /*! \brief blanks and change the image dimensions
      ...
      Delete the current image data and create an arg_width x arg_height pixel image with 0-valued data and default
      attributes
      \param arg_width the width of the blank image
      \param arg_height the height of the blank image
    */
    inline void setZero(int arg_width, int arg_height)
    {
        ...
        {
            if (arg_width <= 0)
                throw ImageException("setZero: Nonpositive width");
            if (arg_height <= 0)
                throw ImageException("setZero: Nonpositive height");
        }
        if (_data != NULL)
        {
            delete[] _data;
    ...
    */
    inline void getPixel(int x, int y, ScalarType (&ret_pixel)[Channels]) const
    {
        ...
        {
            if (_data == NULL)
                throw ImageException("getPixel: NULL data");
            if (!(x >= 0 && x < _width))
                throw ImageException("getPixel: x out of bounds");
            if (!(y >= 0 && y < _height))
                throw ImageException("getPixel: y out of bounds");
        }
    ...
    */
    inline void setPixel(int x, int y, const ScalarType (&pixel)[Channels])
    {
        ...
        {
            if (_data == NULL)
                throw ImageException("setPixel: NULL data");
            if (!(x >= 0 && x < _width))
                throw ImageException("setPixel: x out of bounds");
            if (!(y >= 0 && y < _height))
                throw ImageException("setPixel: y out of bounds");
        }
        for (int channel = 0; channel < Channels; ++channel)
    ...
    */
    inline ScalarType getValue(int x, int y, int channel) const
    {
        ...
        {
            if (_data == NULL)
                throw ImageException("getFloat: NULL data");
            if (!(x >= 0 && x < _width))
                throw ImageException("getFloat: x out of bounds");
            if (!(y >= 0 && y < _height))
                throw ImageException("getFloat: y out of bounds");
            if (!(channel >= 0 && channel < Channels))
                throw ImageException("channel out of bounds");
        }
    ...
    */
    inline void setValue(int x, int y, int channel, ScalarType value)
    {
        ...
        {
            if (_data == NULL)
                throw ImageException("setFloat: NULL data");
            if (!(x >= 0 && x < _width))
                throw ImageException("setFloat: x out of bounds");
            if (!(y >= 0 && y < _height))
                throw ImageException("setFloat: y out of bounds");
            if (!(channel >= 0 && channel < Channels))
                throw ImageException("channel out of bounds");
        }
    ...
    */
    ...
    {
        return getValue(x < 0 ? 0 : (x < _width ? x : _width - 1), y < 0 ? 0 : (y < _height ? y : _height - 1),
                        channel);
    }
  ...
  public:
    ...
    */
    inline void setWidth(int width)
    {
        ...
        {
            if (width <= 0)
                throw ImageException("setWidth: Nonpositive width");
        }
        _width = width;
    }
    ...
    */
    inline void setHeight(int height)
    {
        ...
        {
            if (height <= 0)
                throw ImageException("setHeight: Nonpositive height");
        }
        _height = height;
    }
    ...
    */
    inline void setValues(ScalarType *data)
    {
        ...
        {
            if (data == NULL)
                throw ImageException("setValues: NULL data");
        }
        _data = data;
    }
