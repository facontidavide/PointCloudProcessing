...
class CFace;
class CEdge;
class CVertex : public VertexSimp2<CVertex, CEdge, CFace, vert::VFAdj, vert::Coord3f, vert::BitFlags, vert::Normal3f>
{
};
class CFace : public FaceSimp2<CVertex, CEdge, CFace, face::FFAdj, face::VFAdj, face::VertexRef, face::Normal3f,
                               face::BitFlags, face::Mark>
{
};
...
void SaveMeshInfoHtmlTable(fstream &fout, MeshInfo &mi)
{
    ...
    if (mi.SelfIntersect)
        fout << "          <td>" << static_cast<unsigned int>(mi.intersections.size()) << "</td>" << std::endl;
    else
...
int main(int argc, char **argv)
{
    ...
    if ((load_mask & vcg::tri::io::Mask::IOM_VERTTEXCOORD) || (load_mask & vcg::tri::io::Mask::IOM_WEDGTEXCOORD))
        mi.hasTexture = true;
    else
        mi.hasTexture = false;
    ...
    // GENUS
    if (mi.VManifold && mi.FManifold)
        mi.Genus = tri::Clean<CMesh>::MeshGenus(m, mi.numholes, mi.numcomponents, mi.count_e);
