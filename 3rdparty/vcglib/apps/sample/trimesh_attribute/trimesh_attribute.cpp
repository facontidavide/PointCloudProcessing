...
int main()
{
    ...
    MyMesh::PerVertexAttributeHandle<float> ih =
      vcg::tri::Allocator<MyMesh>::AddPerVertexAttribute<float>(m, std::string("Irradiance"));
    ...
    MyMesh::PerVertexAttributeHandle<float> rh =
    ...
    res = vcg::tri::Allocator<MyMesh>::IsValidHandle(m, ih);
    printf("%d\n", res);
    res = vcg::tri::Allocator<MyMesh>::IsValidHandle(m, blocked_hf);
    printf("%d\n", res);
