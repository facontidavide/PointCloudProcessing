...
void sample011_general_convolutions(QString input_file, QString output_dir, QString output_suffix)
{
    ...
    foreach (m, mm)
    {
        ...
        QString output_file(output_dir + "/011_general_convolution_" + matrix_name + "_" +
                            (normalize ? " normalized_" : "") + output_suffix);
...
int main(int argc, char** argv)
{
    ...
    QStringList readable_image_extensions = QStringList() << "*.bmp"
                                                          << "*.gif"
                                                          << "*.jpg"
                                                          << "*.jpeg"
                                                          << "*.png"
    ...
    QStringList image_list =
      QDir(input_dir).entryList(readable_image_extensions, QDir::Files | QDir::Readable, QDir::Name);
...
{  // utility, unrelated with the sample
    if (!dir.exists())
    {
