...
class VisShader
{
  ...
  public:
    ...
    virtual bool ReadVisibility(const char * /*filename*/)
    {
        assert(0);
        return false;
    }
    virtual bool WriteVisibility(const char * /*filename*/)
    {
        assert(0);
        return false;
    }
    ...
    /*
     ...
     questa funzione calcola un valore float per ogni entita' che indica quanto  e' visibile lungo una data direzione
     camera
    ...
    *******************************/
    ...
    void GenMatrix(Matrix44d &a, Point3d Axis, double angle)
    {
        ...
        if (math::Abs(RotAx.Norm()) < eps)
        {  // in questo caso Axis e' collineare con l'asse z
...
class VertexVisShader : public VisShader<MESH_TYPE>
{
  public:
    // Function Members
    ...
    int GLAccumPixel(std::vector<int> &PixSeen)
    {
        ...
        {
            gluProject(m.vert[i].P()[0], m.vert[i].P()[1], m.vert[i].P()[2], MM, MP, VP, &tx, &ty, &tz);
            int col = 1;
