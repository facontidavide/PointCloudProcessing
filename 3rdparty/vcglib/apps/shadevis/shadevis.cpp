...
void ViewKey(unsigned char key, int, int)
{
    ...
    switch (key)
    {
        ...
        break;
        case 'r':
            BaseColor[0] = min(255, BaseColor[0] + 2);
            break;
        case 'R':
            BaseColor[0] = max(0, BaseColor[0] - 2);
            break;
        case 'g':
            BaseColor[1] = min(255, BaseColor[1] + 2);
            break;
        case 'G':
            BaseColor[1] = max(0, BaseColor[1] - 2);
            break;
        case 'b':
            BaseColor[2] = min(255, BaseColor[2] + 2);
            break;
        case 'B':
            BaseColor[2] = max(0, BaseColor[2] - 2);
            break;
...
void ViewMouse(int button, int state, int x, int y)
{
    ...
    if (state == GLUT_DOWN)
    {
        ...
        if (GLUT_ACTIVE_SHIFT & KeyMod)
            m_mask |= Trackball::KEY_SHIFT;
        if (GLUT_ACTIVE_ALT & KeyMod)
            m_mask |= Trackball::KEY_ALT;
        if (GLUT_ACTIVE_CTRL & KeyMod)
            m_mask |= Trackball::KEY_CTRL;
...
int main(int argc, char **argv)
{
    ...
    while (i < argc && (argv[i][0] == '-'))
    {
        switch (argv[i][1])
        {
            case 'd':
                if (argv[i][2] == 'a')
                {
                    ConeAngleRad = math::ToRad(atof(argv[i + 1]));
                    ++i;
                    break;
                }
                if (argv[i][2] == 'v')
            ...
            case 'c':
                ...
                break;
            case 'w':
                WindowRes = atoi(argv[i] + 2);
                printf("Set WindowRes to %i\n", WindowRes);
                break;
            case 's':
                Vis.SplitNum = atoi(argv[i] + 2);
                printf("Set SplitNum to %i\n", Vis.SplitNum);
                break;
            case 'z':
                Vis.ZTWIST = atof(argv[i] + 2);
    ...
    printf("Mesh bbox (%f %f %f)-(%f %f %f)\n\n", m.bbox.min[0], m.bbox.min[1], m.bbox.min[2], m.bbox.max[0],
           m.bbox.max[1], m.bbox.max[2]);
