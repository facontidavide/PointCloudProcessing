...
bool Cross(const Point2<ScalarType>& p00, const Point2<ScalarType>& p01, const Point2<ScalarType>& p10,
           const Point2<ScalarType>& p11)
{
    Point2<ScalarType> vec0 = p01 - p00;
    Point2<ScalarType> vec1 = p11 - p10;
    if ((vec0 ^ (p11 - p00)) * (vec0 ^ (p10 - p00)) >= 0)
        return false;
    if ((vec1 ^ (p01 - p10)) * (vec1 ^ (p00 - p10)) >= 0)
        return false;
    return true;
}
...
void TessellatePlanarPolygon2(POINT_CONTAINER& points2, std::vector<int>& output)
{
    ...
    for (int i = 0; i < points2.size(); ++i)
        next.push_back((i + 1) % points2.size());
    for (int i = 0; i < points2.size(); ++i)
        prev.push_back((i + points2.size() - 1) % points2.size());
    int v1, v2;
    // check orientation
    S orient = 0.0;
    for (int i = 0; i < points2.size(); ++i)
...
void TessellatePlanarPolygon3(POINT_CONTAINER& points, std::vector<int>& output)
{
    ...
    for (int i = 0; i < points.size(); ++i)
    {
        for (int j = 0; j < 2; ++j)
        {
            i12[j] = i;
            while (i12[j] == i)
                i12[j] = rg.generate(points.size() - 1);
        }
