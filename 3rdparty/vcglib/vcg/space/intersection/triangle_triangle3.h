...
/*@{*/
/**
        Triangle/triangle intersection ,based on the algorithm presented in "A Fast Triangle-Triangle Intersection
   Test",
...
*/
bool coplanar_tri_tri(const Point3<T> N, const Point3<T> V0, const Point3<T> V1, const Point3<T> V2, const Point3<T> U0,
                      const Point3<T> U1, const Point3<T> U2)
{
    T A[3];
    short i0, i1;
...
*/
bool NoDivTriTriIsect(const Point3<T> V0, const Point3<T> V1, const Point3<T> V2, const Point3<T> U0,
                      const Point3<T> U1, const Point3<T> U2)
{
    Point3<T> E1, E2;
    Point3<T> N1, N2;
...
/* coplanarity robustness check */
    ...
    if (FABS(du0) < TRI_TRI_INT_EPSILON)
        du0 = 0.0;
    if (FABS(du1) < TRI_TRI_INT_EPSILON)
        du1 = 0.0;
    if (FABS(du2) < TRI_TRI_INT_EPSILON)
        du2 = 0.0;
#endif
    du0du1 = du0 * du1;
    du0du2 = du0 * du2;
...
#ifdef USE_TRI_TRI_INT_EPSILON_TEST
    if (FABS(dv0) < TRI_TRI_INT_EPSILON)
        dv0 = 0.0;
    if (FABS(dv1) < TRI_TRI_INT_EPSILON)
        dv1 = 0.0;
    if (FABS(dv2) < TRI_TRI_INT_EPSILON)
        dv2 = 0.0;
#endif
...
template <class T>
inline void isect2(Point3<T> VTX0, Point3<T> VTX1, Point3<T> VTX2, float VV0, float VV1, float VV2, float D0, float D1,
                   float D2, float *isect0, float *isect1, Point3<T> &isectpoint0, Point3<T> &isectpoint1)
{
    float tmp = D0 / (D0 - D1);
    float diff[3];
...
template <class T>
inline int compute_intervals_isectline(Point3<T> VERT0, Point3<T> VERT1, Point3<T> VERT2, float VV0, float VV1,
                                       float VV2, float D0, float D1, float D2, float D0D1, float D0D2, float *isect0,
                                       float *isect1, Point3<T> &isectpoint0, Point3<T> &isectpoint1)
{
    if (D0D1 > 0.0f)
    {
        /* here we know that D0D2<=0.0 */
...
template <class T>
bool tri_tri_intersect_with_isectline(Point3<T> V0, Point3<T> V1, Point3<T> V2, Point3<T> U0, Point3<T> U1,
                                      Point3<T> U2, bool &coplanar, Point3<T> &isectpt1, Point3<T> &isectpt2)
{
    Point3<T> E1, E2;
    Point3<T> N1, N2;
    T d1, d2;
