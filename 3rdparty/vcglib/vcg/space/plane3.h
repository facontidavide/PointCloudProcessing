   ...
   **/
    ...
    /** @name Members to access the distance or direction
      Direction() cannot be assigned directly.
        Use SetDirection() or Set() instead. This is mandatory to make possible the automatic autonormalization template
   mechanism.
...
template <class T>
T Distance(const Plane3<T> &plane, const Point3<T> &point)
{
    return plane.Direction().dot(point) - plane.Offset();
}

template <class T>
T SquaredDistance(const Plane3<T> &plane, const Point3<T> &point)
{
...
template <class T>
T Distance(const Point3<T> &point, const Plane3<T> &plane)
{
    return Distance(plane, point);
}

template <class T>
T SquaredDistance(const Point3<T> &point, const Plane3<T> &plane)
{
