...
class PerfectSpatialHashing : public vcg::SpatialIndex<OBJECT_TYPE, SCALAR_TYPE>
{
    ...
    {
        typedef typename ReferenceType<TYPE>::Type Type;
    };
    ...
    */
    struct NeighboringEntryIterator
    {
        ...
        */
        void operator++(int)
        {
            ...
            {
                case 1:
                    m_CurrentNeighbor.X() = (m_Center.X() + 1) % m_TableSize;
                    break;
                case 2:
                    m_CurrentNeighbor.X() = m_Center.X();
                    m_CurrentNeighbor.Y() = (m_Center.Y() + m_TableSize - 1) % m_TableSize;
                    break;
                case 3:
                    m_CurrentNeighbor.Y() = (m_Center.Y() + 1) % m_TableSize;
                    break;
                case 4:
                    m_CurrentNeighbor.Y() = m_Center.Y();
                    m_CurrentNeighbor.Z() = (m_Center.Z() + m_TableSize - 1) % m_TableSize;
                    break;
                case 5:
                    m_CurrentNeighbor.Z() = (m_Center.Z() + 1) % m_TableSize;
                    break;
                default:
                    m_CurrentNeighbor = vcg::Point3i(-1, -1, -1);
                    break;
            }
        }
        ...
        */
        ...
        {
            return m_CurrentIteration < value;
        }
    ...
    class UniformGrid
    {
      public:
        ...
        EntryIterator Begin()
        {
            return EntryIterator(this, CellCoordinate(0, 0, 0));
        }
        EntryIterator End()
        {
            return EntryIterator(this, CellCoordinate(-1, -1, -1));
        }
        ...
        {
            return NeighboringEntryIterator(at, m_CellPerSide);
        }
         ...
         * @param[in] end		The iterator addressing the position one past the final element in the range to be
         * added.
         */
        ...
        void InsertElements(const OBJECT_ITERATOR &begin, const OBJECT_ITERATOR &end)
        {
            ...
            for (ObjectIterator iObject = begin; iObject != end; iObject++)
            {
                ...
                for (std::vector<CellCoordinate>::iterator iCell = cells_occupied.begin(), eCell = cells_occupied.end();
                     iCell != eCell; iCell++)
         ...
         * \return				A pointer to a vector of pointers to the objects falling in the cell having index
         * <CODE>at</CODE>.
         */
      ...
      protected:
        std::vector<std::vector<std::vector<std::vector<ObjectPointer> > > > m_Grid; /*!< The uniform grid
                                                                                        */
    ...
    class HashTable
    {
      public:
        ...
        {
            return NeighboringEntryIterator(at, m_EntryPerSide);
        }
        ...
        {
            while (entry.X() < 0)
                entry.X() += m_EntryPerSide;
            while (entry.Y() < 0)
                entry.Y() += m_EntryPerSide;
            while (entry.Z() < 0)
                entry.Z() += m_EntryPerSide;
        }
         ...
         */
        inline Data *GetData(const int i, const int j, const int k) const
        {
            return m_Table[i][j][k];
        }
        inline Data *GetData(const EntryCoordinate &at) const
        {
            return m_Table[at.X()][at.Y()][at.Z()];
        }
        inline Data *operator[](const EntryCoordinate &at) const
        {
            return m_Table[at.X()][at.Y()][at.Z()];
        }
    ...
    class OffsetTable
    {
      public:
        ...
        struct PreImage
        {
            /*!
             * Default constructor.
             * @param[in] at				The entry in the offset table where the cells in the preimage are mapped
             * into.
             ...
             * @param second
             * \return <code>true</code> if and only if the cardinality of this preimage slot is greater than that of
             * <code>second</code>.
             */
            inline bool operator<(const PreImage &second) const
            {
                return (cardinality > second.cardinality);
            }

            std::vector<typename UniformGrid::CellCoordinate> *pre_image; /*!< The set of entries in the uniform grid
                                                                             whose image through \f$h_1\f$ is this
                                                                             entry.*/
         ...
         */
        void BuildH1PreImage(const typename UniformGrid::EntryIterator &begin,
        ...
        /*!
         * Assures that the given entry can be used to access the offset table without throwing an out-of-bound
         * exception.
        ...
        */
         ...
         */
        inline int GetNumberOfOccupiedCells() const
        {
            return m_NumberOfOccupiedEntries;
        }

        /*!
         * Return the pointer to the offset stored at the given entry. NULL if that entry doesn't contain a offset
         */
        inline OffsetPointer &GetOffset(const int i, const int j, const int k)
        {
            return m_Table[i][j][k];
        }
        inline OffsetPointer GetOffset(const int i, const int j, const int k) const
        {
            return m_Table[i][j][k];
        }

        inline OffsetPointer &GetOffset(const EntryCoordinate &at)
        {
            return m_Table[at.X()][at.Y()][at.Z()];
        }
        inline OffsetPointer GetOffset(const EntryCoordinate &at) const
        {
            return m_Table[at.X()][at.Y()][at.Z()];
        }

        inline OffsetPointer &operator[](const EntryCoordinate &at)
        {
            return m_Table[at.X()][at.Y()][at.Z()];
        }
        inline OffsetPointer operator[](const EntryCoordinate &at) const
        {
            return m_Table[at.X()][at.Y()][at.Z()];
        }
      ...
      protected:
        ...
        std::vector<std::vector<std::vector<std::vector<typename UniformGrid::CellCoordinate> > > >
          m_H1PreImage; /*!< The \f$f1\f$ pre-image.		*/
    };                  // end of class OffsetTable

    /*******************************************************************************************************************************/
    /*! \class BinaryImage
    * This class is used to encode the sparsity of the dataset. Since the hash table stores data associated with a
    * sparse
    ...
    class BinaryImage
    {
      public:
         ...
         * Checks if a portion of the dataset fall inside the UniformGrid cell having coordinate <CODE>at</CODE>.
           ...
           * \return				<code>true</code> if and only if a portion of the dataset in included in this
         * UniformGrid cell.
           */
        inline bool ContainsData(const typename UniformGrid::CellCoordinate &at) const
        {
            return GetFlag(at) == true;
        }
         ...
         * Return the value stored in the d-dimensional bitmap at the given position.
           ...
           *	\return
           */
        inline bool operator()(const int i, const int j, const int k)
        {
            return m_Mask[i][j][k];
        }
         ...
         * Return the value stored at the given position in the d-dimensional bitmap.
           ...
           * \return
           */
        inline bool operator[](const typename UniformGrid::CellCoordinate &at)
        {
            return m_Mask[at.X()][at.Y()][at.Z()];
        }
        inline bool &GetFlag(const int i, const int j, const int k) const
        {
            return m_Mask[i][j][k];
        }
        inline void SetFlat(const int i, const int j, const int k)
        {
            m_Mask[i][j][k] = true;
        }

        inline bool GetFlag(const typename UniformGrid::CellCoordinate &at) const
        {
            return m_Mask[at.X()][at.Y()][at.Z()];
        }
        inline void SetFlag(const typename UniformGrid::CellCoordinate &at)
        {
            m_Mask[at.X()][at.Y()][at.Z()] = true;
        }

      protected:
        std::vector<std::vector<std::vector<bool> > > m_Mask; /*!< The bitmap image.							*/
  ...
  public:
     ...
     * the second try to construct the offset table up to m_MAX_TRIALS_IN_COMPACT_CONSTRUCTION times, and then chooses
     * the minimum size for which the construction succeeded.
     */
    enum ConstructionApproach
    {
        FastConstructionApproach = 0,
        CompactConstructionApproach = 1
    };
     ...
     */
    ~PerfectSpatialHashing()
    { /* ... I don't remember if there is something to delete! :D */
    }
     ...
     * @param[in] eObj			The iterator addressing the position after the last element to be included in the
     * hashing.
             ...
             vcg::CallBackPos *callback)
    {
        ...
        float longest_side =
          vcg::math::Max(resulting_bb.DimX(), vcg::math::Max(resulting_bb.DimY(), resulting_bb.DimZ())) / 2.0f;
...
// Try to find a reasonable space partition
        ...
        switch (approach)
        {
            case FastConstructionApproach:
                PerformFastConstruction(number_of_cells_occupied, callback);
                break;
            case CompactConstructionApproach:
                PerformCompactConstruction(number_of_cells_occupied, callback);
                break;
            default:
                assert(false);
        }
     ...
     * \return
     */
    template <class OBJECT_POINT_DISTANCE_FUNCTOR, class OBJECT_MARKER, class OBJECT_POINTER_CONTAINER,
              class DISTANCE_CONTAINER, class POINT_CONTAINER>
    unsigned int GetInSphere(OBJECT_POINT_DISTANCE_FUNCTOR &distance_functor, OBJECT_MARKER &marker,
                             const CoordType &sphere_center, const ScalarType &sphere_radius,
                             OBJECT_POINTER_CONTAINER &objects, DISTANCE_CONTAINER &distances, POINT_CONTAINER &points,
                             bool sort_per_distance = true, bool allow_zero_distance = true)
    {
        ...
        for (std::vector<std::vector<ObjectPointer> *>::iterator iVec = contained_objects.begin(),
                                                                 eVec = contained_objects.end();
             iVec != eVec; iVec++)
            for (std::vector<ObjectPointer>::iterator iObj = (*iVec)->begin(), eObj = (*iVec)->end(); iObj != eObj;
                 iObj++)
            {
                ...
                if (!distance_functor(*results[r].object, sphere_center, results[r].distance,
                                      results[r].nearest_point) ||
                    (results[r].distance == ScalarType(0.0) && !allow_zero_distance))
                    results.pop_back();
            }
  ...
  protected:
    ...
    */
    ...
    {
#ifdef _DEBUG
        for (UniformGrid::EntryIterator iUGEntry = m_UniformGrid.Begin(), eUGEntry = m_UniformGrid.End();
             iUGEntry != eUGEntry; iUGEntry++)
     ...
     * \return
     */
    bool IsAValidOffset(const std::vector<typename UniformGrid::CellCoordinate> *pre_image,
                        const typename OffsetTable::Offset &offset)
    {
        ...
        // In order to guarantee that the PerfectHashFunction is injective, the image of each domain-entry must be
    ...
    /*!
     * Given the size of the hash table and an initial seed for the size of the offset table, returns an appropriate
     * size
    ...
    int GetUnefectiveOffsetTableSize(const int hash_table_size, const int offset_table_size)
    {
        ...
        while (GreatestCommonDivisor(hash_table_size, result) != 1 || hash_table_size % result == 0)
            result += (hash_table_size % 2 == 0) ?
                        (result % 2) + 1 :
                        1;  // Change the offset table size, otherwise the hash construction should be ineffective
        return result;
    }
    ...
    * @param[in] number_of_filled_cells The number of entries in the uniform grid containing some elements of the
    * dataset.
    */
    ...
    */
    void PerformCompactConstruction(const int number_of_filled_cells, vcg::CallBackPos *callback)
    {
        ...
        // According to the original article, a maximum number of trials are to be made in order to select the optimal
        // (i.e. minimal) offset table size
        for (int t = 0; t < m_MAX_TRIALS_IN_COMPACT_CONSTRUCTION; t++)
        {
            int lower_bound = GetUnefectiveOffsetTableSize(
              hash_table_size, int(double(rand()) / double(RAND_MAX) * half_hash_table_size + 1));
            int upper_bound = GetUnefectiveOffsetTableSize(
              hash_table_size, int(((double)rand() / (double)RAND_MAX) * hash_table_size + half_hash_table_size));
            ...
            {
                candidate_offset_table_size =
                  GetUnefectiveOffsetTableSize(hash_table_size, int(floorf((lower_bound + upper_bound) / 2.0f)));
                ...
                if (OffsetTableConstructionSucceded((last_tried_size = candidate_offset_table_size), callback))
                {
                    ...
                    min_successfully_dimension =
                      vcg::math::Min(candidate_offset_table_size, min_successfully_dimension);
                }
                else
    ...
    */
    bool OffsetTableConstructionSucceded(const int offset_table_size, vcg::CallBackPos *callback)
    {
        ...
        int perc = 0;
        int iter = 0;
        for (std::list<OffsetTable::PreImage>::iterator iPreImage = preimage_slots.begin(),
                                                        ePreImage = preimage_slots.end();
             iPreImage != ePreImage; iPreImage++, iter++)
        {
            if (callback != NULL && iter % step == 0 && (perc = iter * 100 / number_of_slots) < 100)
                (*callback)(perc, msg);
            ...
            for (std::vector<typename OffsetTable::Offset>::iterator iOffset = consistent_offsets.begin(),
                                                                     eOffset = consistent_offsets.end();
                 iOffset != eOffset && !found_valid_offset; iOffset++)
            ...
            if (!found_valid_offset)
            {
                ...
                for (std::vector<typename UniformGrid::CellCoordinate>::iterator iPreImage = pre_image->begin(),
                                                                                 ePreImage = pre_image->end();
                     iPreImage != ePreImage && !found_valid_offset; iPreImage++)
                    for (NeighboringEntryIterator iUGNeighbourhood =
                           m_UniformGrid.GetNeighboringEntryIterator(*iPreImage);
                         iUGNeighbourhood < 6 && !found_valid_offset; iUGNeighbourhood++)
                        if (!m_OffsetTable.IsFree(m_OffsetTable.DomainToOffsetTable(*iUGNeighbourhood)))
                        {
                            ...
                            for (NeighboringEntryIterator iHTNeighbourhood =
                                   m_HashTable.GetNeighborintEntryIterator(ht_entry);
                                 iHTNeighbourhood < 6 && !found_valid_offset; iHTNeighbourhood++)
                                if (m_HashTable.IsFree(*iHTNeighbourhood))
                                {
                                    candidate_offset.Import(*iHTNeighbourhood -
                                                            m_HashTable.DomainToHashTable(*iPreImage));
                                    // m_OffsetTable.ValidateEntry( candidate_offset ); Is'n necessary, becouse the
                                    // offset type is unsigned char.
            ...
            if (!found_valid_offset)
            {
                ...
                // The chance to find a valid offset table via random searches decreases toward the end of the offset
                // table construction:
                // So a exhaustive search over all the free hash table entries is performed.
                for (std::list<HashTable::EntryCoordinate>::const_iterator
                       iFreeCell = m_HashTable.GetFreeEntryList()->begin(),
                       eFreeCell = m_HashTable.GetFreeEntryList()->end();
                     iFreeCell != eFreeCell && !found_valid_offset; iFreeCell++)
                {
            ...
            if (found_valid_offset)
            {
                ...
                for (int c = 0, pre_image_cardinality = iPreImage->cardinality; c < pre_image_cardinality; c++)
                {
                    ...
                    m_HashTable.SetEntry(ht_entry, domain_data /*, (*iPreImage->pre_image).at(c)*/);  // might be useful
                                                                                                      // for encoding
                                                                                                      // sparsity
...
template <class OBJECT_TYPE, class SCALAR_TYPE>
const SCALAR_TYPE
  vcg::PerfectSpatialHashing<OBJECT_TYPE, SCALAR_TYPE>::m_BOUNDING_BOX_EXPANSION_FACTOR = SCALAR_TYPE(0.035);

template <class OBJECT_TYPE, class SCALAR_TYPE>
const SCALAR_TYPE
  vcg::PerfectSpatialHashing<OBJECT_TYPE, SCALAR_TYPE>::m_SIGMA = SCALAR_TYPE(1.0f / (2.0f * SCALAR_TYPE(m_DIMENSION)));
