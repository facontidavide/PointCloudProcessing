...
class OctreeTemplate
{
  ...
  public:
    ...
    inline BoundingBoxType BoundingBox()
    {
        return boundingBox;
    }

    // Return the Voxel of the n-th node
    inline VoxelPointer Voxel(const NodePointer n)
    {
        return &(n->voxel);
    }
    ...
    inline const NodePointer Root() const
    {
        return nodes[0];
    }

    // Return the level of the n-th node
    inline char Level(const NodePointer n) const
    {
        return n->level;
    }

    // Return the referente to the i-th son of the n-th node
    inline NodePointer &Son(NodePointer n, int i) const
    {
        return n->Son(i);
    }

    // Return the parent index of the n-th node
    inline NodePointer Parent(const NodePointer n) const
    {
        return n->parent;
    }
    ...
    inline CenterType CenterInOctreeCoordinates(const NodePointer n) const
    {
        return n->center;
    }
    ...
    */
    ...
    Note how each cell is meant to contains a right open interval (e.g. the first cell contains [0,12.5) and the second
    [12.5,25) and so on)
    ...
    */
    ...
    BoundingBoxType SubBox(BoundingBoxType &lbb, int i)
    {
        BoundingBoxType bs;
        if (i & 1)
            bs.min.X() = (lbb.min.X() + (bs.max.X() = lbb.max.X())) / 2.0f;
        else
            bs.max.X() = ((bs.min.X() = lbb.min.X()) + lbb.max.X()) / 2.0f;
        if (i & 2)
            bs.min.Y() = (lbb.min.Y() + (bs.max.Y() = lbb.max.Y())) / 2.0f;
        else
            bs.max.Y() = ((bs.min.Y() = lbb.min.Y()) + lbb.max.Y()) / 2.0f;
        if (i & 4)
            bs.min.Z() = (lbb.min.Z() + (bs.max.Z() = lbb.max.Z())) / 2.0f;
        else
            bs.max.Z() = ((bs.min.Z() = lbb.min.Z()) + lbb.max.Z()) / 2.0f;
    ...
    */
    ...
    NodePointer AddNode(CenterType path)
    {
        ...
        {
            int nextSon = 0;
            if ((path[0] >> shiftLevel) % 2)
                nextSon += 1;
            if ((path[1] >> shiftLevel) % 2)
                nextSon += 2;
            if ((path[2] >> shiftLevel) % 2)
                nextSon += 4;
    ...
    */
    ...
    {
        return ZOrder(GetPath(n), Level(n));
    }
    ZOrderType ComputeZOrder(const CoordinateType &query) const
    {
        return ZOrder(CenterType::Construct(Interize(query)), maximumDepth);
    };
    ...
    void ContainedNodes(BoundingBoxType &query, std::vector<NodePointer> &nodes, int depth, NodePointer n,
                        BoundingBoxType &nodeBB)
    {
    ...
    void ContainedLeaves(BoundingBoxType &query, std::vector<NodePointer> &leaves, NodePointer node,
                         BoundingBoxType &nodeBB)
    {
        NodePointer son;
        BoundingBoxType sonBB;
