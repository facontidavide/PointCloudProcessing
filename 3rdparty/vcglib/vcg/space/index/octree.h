...
class Octree : public vcg::OctreeTemplate<Voxel, SCALAR_TYPE>, public vcg::SpatialIndex<OBJECT_TYPE, SCALAR_TYPE>
{
  ...
  protected:
    ...
    */
    ...
    struct ObjectPlaceholder
    {
        ...
        ObjectPlaceholder()
        {
            z_order = object_index = -1, leaf_pointer = NULL;
        }
  ...
  public:
    ...
    */
    ...
    ObjectPointer GetClosest(OBJECT_POINT_DISTANCE_FUNCTOR &distance_functor, OBJECT_MARKER & /*marker*/,
                             const CoordType &query_point, const ScalarType &max_distance, ScalarType &distance,
                             CoordType &point, bool allow_zero_distance = true)
    {
    ...
    */
    ...
    unsigned int GetKClosest(OBJECT_POINT_DISTANCE_FUNCTOR &distance_functor, OBJECT_MARKER & /*marker*/,
                             unsigned int k, const CoordType &query_point, const ScalarType &max_distance,
    ...
    OBJECT_RETRIEVER:
        ...
        if (sort_per_distance)
            std::partial_sort<NeighbourIterator>(first, first + object_count, last);
        else
            std::nth_element<NeighbourIterator>(first, first + object_count, last);

        k_distance = neighbors[object_count - 1].distance;
        if (k_distance > sphere_radius && sphere_radius < max_distance)
            goto OBJECT_RETRIEVER;

        return CopyQueryResults<OBJECT_POINTER_CONTAINER, DISTANCE_CONTAINER, POINT_CONTAINER>(
          neighbors, object_count, objects, distances, points);
    };  // end of GetKClosest
    ...
    */
    ...
    unsigned int GetInSphere(OBJECT_POINT_DISTANCE_FUNCTOR &distance_functor, OBJECT_MARKER & /*marker*/,
                             ...
                             OBJECT_POINTER_CONTAINER &objects, DISTANCE_CONTAINER &distances, POINT_CONTAINER &points,
                             bool sort_per_distance = false, bool allow_zero_distance = false)
    {
        ...
        int object_count = RetrieveContainedObjects(sphere_center, distance_functor, sphere_radius, allow_zero_distance,
                                                    leaves, neighbors);
    ...
    */
    ...
    unsigned int GetInBox(OBJECT_MARKER & /*marker*/, const BoundingBoxType &query_bounding_box,
                          OBJECT_POINTER_CONTAINER &objects)
    {
        ...
        TemplatedOctree::ContainedLeaves(query_bounding_box, leaves, TemplatedOctree::Root(),
                                         TemplatedOctree::boundingBox);
  ...
  protected:
    ...
    * \return true iff it's possibile to find a sphere, centered in query_point and having radius max_distance at most,
    * which collide the octree bounding-box.
    */
    inline bool GuessInitialBoundingBox(const CoordType &query_point, const ScalarType max_distance,
                                        ScalarType &sphere_radius, BoundingBoxType &query_bb)
    {
    ...
    */
    inline int AdjustBoundingBox(BoundingBoxType &query_bb, ScalarType &sphere_radius,
                                 const ScalarType max_allowed_distance, std::vector<NodePointer> &leaves,
    ...
    */
    ...
    inline int RetrieveContainedObjects(const CoordType query_point, OBJECT_POINT_DISTANCE_FUNCTOR &distance_functor,
                                        const ScalarType max_allowed_distance, bool allow_zero_distance,
                                        std::vector<NodePointer> &leaves, std::vector<Neighbour> &neighbors)
    {
    ...
    */
    ...
    inline int CopyQueryResults(std::vector<Neighbour> &neighbors, const unsigned int object_count,
                                OBJECT_POINTER_CONTAINER &objects, DISTANCE_CONTAINER &distances,
    ...
    /*!
    * When all the leaves are indexed, this procedure is called in order to propagate the indexing information to the
    * inner nodes
    */
