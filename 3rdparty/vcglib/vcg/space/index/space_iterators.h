...
corrected bug in closest iterator.. if doesn't find any alement at first cells examinated continue until find some
element
...
class RayIterator
{
  ...
  protected:
    ...
    void _ControlLimits()
    {
        for (int i = 0; i < 3; i++)
        {
            ...
            if (CurrentCell.V(i) < 0)
                CurrentCell.V(i) = 0;
            else if (CurrentCell.V(i) >= dim.V(i))
    ...
    {
        return (
          ((CurrentCell.X() < 0) || (CurrentCell.Y() < 0) || (CurrentCell.Z() < 0)) ||
  ...
  protected:
    ...
    struct Entry_Type
    {
      public:
        ...
        inline bool operator<(const Entry_Type &l) const
        {
            return (dist > l.dist);
        }
        ObjType *elem;
...
class ClosestIterator
{
    ...
    bool _NextShell()
    {
        ...
        {
            assert(!(to_explore.min.X() < 0 || to_explore.max.X() >= Si.siz[0] || to_explore.min.Y() < 0 ||
                     to_explore.max.Y() >= Si.siz[1] || to_explore.min.Z() < 0 || to_explore.max.Z() >= Si.siz[2]));
            return true;
        }
        return false;
    }
        ...
        for (iz = to_explore.min.Z(); iz <= to_explore.max.Z(); ++iz)
            for (iy = to_explore.min.Y(); iy <= to_explore.max.Y(); ++iy)
                for (ix = to_explore.min.X(); ix <= to_explore.max.X(); ++ix)
                {
                    ...
                    if ((explored.IsNull()) || (ix < explored.min[0] || ix > explored.max[0] || iy < explored.min[1] ||
                                                iy > explored.max[1] || iz < explored.min[2] || iz > explored.max[2]))
                    {
                        typename Spatial_Idexing::CellIterator first, last, l;
