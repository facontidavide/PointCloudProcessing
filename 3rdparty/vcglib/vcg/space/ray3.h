...
class Ray3
{
   ...
   **/
    ...
    {
        if (NORM)
            return ScalarType((p - _ori).dot(_dir));
        else
            return ScalarType((p - _ori).dot(_dir) / _dir.SquaredNorm());
    }
   ...
   **/
    ...
    inline Ray3<ScalarType, false> operator/(const ScalarType s) const
    {
        ...
        return RayType(_ori * s0, _dir * s0);
    }
    //@}
