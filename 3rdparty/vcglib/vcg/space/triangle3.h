...
/**
    ...
    Note the relation with the Face class of TriMesh complex, both classes provide the P(i) access functions to their
   points and therefore they share the algorithms on it (e.g. area, normal etc...)
 */
...
bool InterpolationParameters(const TriangleType t, const int Axis, const Point3<ScalarType> &P, Point3<ScalarType> &L)
{
    ...
    if (Axis == 0)
        return InterpolationParameters2(P2(t.P(0)[1], t.P(0)[2]), P2(t.P(1)[1], t.P(1)[2]), P2(t.P(2)[1], t.P(2)[2]),
                                        P2(P[1], P[2]), L);
    if (Axis == 1)
        return InterpolationParameters2(P2(t.P(0)[0], t.P(0)[2]), P2(t.P(1)[0], t.P(1)[2]), P2(t.P(2)[0], t.P(2)[2]),
                                        P2(P[0], P[2]), L);
    if (Axis == 2)
        return InterpolationParameters2(P2(t.P(0)[0], t.P(0)[1]), P2(t.P(1)[0], t.P(1)[1]), P2(t.P(2)[0], t.P(2)[1]),
                                        P2(P[0], P[1]), L);
    return false;
}
/// Handy Wrapper of the above one that uses the passed normal N to choose the right orientation
template <class TriangleType, class ScalarType>
bool InterpolationParameters(const TriangleType t, const Point3<ScalarType> &N, const Point3<ScalarType> &P,
                             Point3<ScalarType> &L)
{
    if (N[0] > N[1])
    {
...
// Algorithm: simply find a base for the frame of the triangle, assuming v3 as origin (matrix T) invert it and apply to
// P-v3.

template <class ScalarType>
bool InterpolationParameters2(const Point2<ScalarType> &V1, const Point2<ScalarType> &V2, const Point2<ScalarType> &V3,
...
*/
template <class TriangleType, class ScalarType>
bool InterpolationParameters(const TriangleType t, const Point3<ScalarType> &N, const Point3<ScalarType> &bq,
...
template <class P3ScalarType>
P3ScalarType QualityRadii(Point3<P3ScalarType> const &p0, Point3<P3ScalarType> const &p1,
...
/// the range is range [0, 1]
template <class P3ScalarType>
P3ScalarType QualityMeanRatio(Point3<P3ScalarType> const &p0, Point3<P3ScalarType> const &p1,
                              Point3<P3ScalarType> const &p2)
{
    P3ScalarType a = (p1 - p0).Norm();
    P3ScalarType b = (p2 - p0).Norm();
...
{
    typename TriangleType::CoordType e0 = t.P((k + 1) % 3) - t.P(k), e1 = t.P((k + 2) % 3) - t.P(k);
...
Point3<typename TriangleType::ScalarType> Circumcenter(const TriangleType &t)
{
    ...
    Point3<typename TriangleType::ScalarType> c =
      t.P(0) * a2 * (-a2 + b2 + c2) + t.P(1) * b2 * (a2 - b2 + c2) + t.P(2) * c2 * (a2 + b2 - c2);
