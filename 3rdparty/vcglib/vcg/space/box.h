...
class Box : public Space<N, S>, Linear<Box>
{
  ...
  public:
    inline const PointType &Max() const
    {
        return _max;
    }
    inline PointType &Max()
    {
        return _max;
    }
    inline const PointType &Min() const
    {
        return _min;
    }
    inline PointType &Min()
    {
        return _min;
    }
    ...
    /// Min Max constructor
    inline Box(const PointType &mi, const PointType &ma)
    {
        _min = mi;
        _max = ma;
    }
    ...
    */
    void InflateFix(const S s)
    {
        S k = Diag() * s;
        if (N == 2)
            Inflate(PointType(k, k));
        if (N == 3)
            Inflate(PointType(k, k, k));
    }
    ...
    */
    ...
    {
        return b._min.X() < _max.X() && b._max.X() > _min.X() && b._min.Y() < _max.Y() && b._max.Y() > _min.Y() &&
    ...
    */
    ...
    {
        return PointType(_min[0] + p[0] * (_max[0] - _min[0]), _min[1] + p[1] * (_max[1] - _min[1]),
    ...
    {
        return PointType((p[0] - _min[0]) / (_max[0] - _min[0]), (p[1] - _min[1]) / (_max[1] - _min[1]),
    ...
    /// Compute box size X.
    inline S SizeX() const
    {
        return _max.X() - _min.X();
    }
    /// Compute box size Y.
    inline S SizeY() const
    {
        return _max.Y() - _min.Y();
    }
    /// Compute box size Z.
    inline S SizeZ() const
    {
        static_assert(N > 2);
