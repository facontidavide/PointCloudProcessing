...
/**
  ...
  the type of the tex coord and the number of texcoord to be stored. This class is intended to be used when many
  textures
  id are shared over the same surface, so for each coord the id of the texture is stored. If no id is needed see the
  vcg::TexCoord2Simple class.
*/
...
class TexCoord2
{
  ...
  public:
    ...
    inline const T &U(const int i) const
    {
        assert(i > 0 && i < NMAX);
        return _t[i][0];
    }
    ...
    inline short &N(const int i)
    {
        assert(i > 0 && i < NMAX);
        return _n[i];
    }
    inline short N(const int i) const
    {
        assert(i > 0 && i < NMAX);
        return _n[i];
    }
    ...
    /**/
    /**/ inline Point2<T> &t(const int i)
    {
        return _t[i];
    }
    /**/ inline Point2<T> t(const int i) const
    {
        return _t[i];
    }
    /**/
...
class TexCoord2Simple
{
  ...
  public:
    ...
    inline Point2<T> &P(const int i)
    {
        assert(i == 0);
        return _t;
    }
    inline Point2<T> P(const int i) const
    {
        assert(i == 0);
        return _t;
    }
