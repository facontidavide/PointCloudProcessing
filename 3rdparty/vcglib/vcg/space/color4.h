...
/**
    ...
    The class is templated over the ScalarType.  class that is used to represent color with float or with unsigned
   chars. All the usual
...
class Color4 : public Point4<T>
{
  ...
  public:
    ...
    /// given a float and a range set the corresponding color in the well known red->green->blue color ramp. To reverse
    /// the direction of the ramp just swap minf and maxf.
...
inline void Color4<T>::ColorRamp(const float &minf, const float &maxf, float v)
{
    ...
    if (v < step)
    {
        lerp(Color4<T>(Color4<T>::Red), Color4<T>(Color4<T>::Yellow), v / step);
        return;
    }
    v -= step;
    if (v < step)
    {
        lerp(Color4<T>(Color4<T>::Yellow), Color4<T>(Color4<T>::Green), v / step);
        return;
    }
    v -= step;
    if (v < step)
    {
        lerp(Color4<T>(Color4<T>::Green), Color4<T>(Color4<T>::Cyan), v / step);
        return;
    }
    v -= step;
    if (v < step)
    {
        lerp(Color4<T>(Color4<T>::Cyan), Color4<T>(Color4<T>::Blue), v / step);
        return;
    }
...
{
    return Color4<unsigned char>((unsigned char)(b[0] * 255.0f), (unsigned char)(b[1] * 255.0f),
                                 (unsigned char)(b[2] * 255.0f), (unsigned char)(b[3] * 255.0f));
}
...
{
    return Color4<float>((float)(b[0]) / 255.0f, (float)(b[1]) / 255.0f, (float)(b[2]) / 255.0f,
                         (float)(b[3]) / 255.0f);
}
...
{
    return Color4<unsigned char>((unsigned char)(math::Clamp(int((*this)[0]) + int(p[0]), 0, 255)),
