...
class PointMatching
{
  public:
    ...
    */
                                             ...
                                             std::vector<Point3x> &Pmov)  // normali scelti su mov (verdi)
    {
        ...
        if (!ret)
            return false;
        Matrix44x scaleM;
        scaleM.SetDiagonal(1.0 / scalingFactor);
    ...
    */
    static bool ComputeWeightedRigidMatchMatrix(Matrix44x &res, std::vector<Point3x> &Pfix, std::vector<Point3x> &Pmov,
                                                std::vector<ScalarType> weights, Quaternionx &q, Point3x &tr)
    {
        Matrix33x ccm;
        ...
        Matrix44x Rot;
        q.ToMatrix(Rot);
        // the translation (last row) is simply the difference between the transformed src barycenter and the trg
        // baricenter
        tr = (bfix - Rot * bmov);
