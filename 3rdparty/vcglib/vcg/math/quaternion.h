...
template <class S>
Quaternion<S>::Quaternion(const S phi, const Point3<S> &a)
{
    FromAxis(phi, a);
}

template <class S>
Quaternion<S> Quaternion<S>::operator*(const S &s) const
{
    return (Quaternion(V(0) * s, V(1) * s, V(2) * s, V(3) * s));
}

template <class S>
Quaternion<S> Quaternion<S>::operator*(const Quaternion &q) const
{
...
template <class S>
Quaternion<S> &Quaternion<S>::operator*=(const Quaternion &q)
{
...
template <class S>
Quaternion<S> Quaternion<S>::Inverse() const
{
    return Quaternion<S>(V(0), -V(1), -V(2), -V(3));
}

template <class S>
void Quaternion<S>::FromAxis(const S phi, const Point3<S> &a)
{
    Point3<S> b = a;
    b.Normalize();
...
template <class S>
void Quaternion<S>::ToAxis(S &phi, Point3<S> &a) const
{
...
template <class S>
Point3<S> Quaternion<S>::Rotate(const Point3<S> p) const
{
...
template <class S>
void Quaternion<S>::ToMatrix(Matrix44<S> &m) const
{
...
template <class S>
void Quaternion<S>::ToMatrix(Matrix33<S> &m) const
{
...
template <class S>
void Quaternion<S>::FromMatrix(const Matrix44<S> &m)
{
    MatrixToQuaternion<S, Matrix44<S> >(m, *this);
}
template <class S>
void Quaternion<S>::FromMatrix(const Matrix33<S> &m)
{
...
template <class S>
Quaternion<S> &Invert(Quaternion<S> &m)
{
    m.Invert();
    return m;
}
...
template <class S>
Quaternion<S> Inverse(const Quaternion<S> &m)
{
    Quaternion<S> a = m;
    a.Invert();
    return a;
