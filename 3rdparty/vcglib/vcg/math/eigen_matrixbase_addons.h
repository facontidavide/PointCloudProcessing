...
EIGEN_DEPRECATED inline Scalar V(int i) const
{
    return (*this)[i];
};
EIGEN_DEPRECATED inline Scalar& V(int i)
{
    return (*this)[i];
};
...
/** \deprecated use norm() */
EIGEN_DEPRECATED inline Scalar Norm() const
{
    return norm();
};
/** \deprecated use squaredNorm() */
EIGEN_DEPRECATED inline Scalar SquaredNorm() const
{
    return squaredNorm();
};
/** \deprecated use normalize() or normalized() */
EIGEN_DEPRECATED inline Derived& Normalize()
{
    normalize();
    return derived();
};
/** \deprecated use normalized() */
EIGEN_DEPRECATED inline const PlainMatrixType Normalize() const
{
    return normalized();
};

/** \deprecated use transposeInPlace() or transpose() */
EIGEN_DEPRECATED inline Derived& Transpose()
{
    transposeInPlace();
    return derived();
};
/** \deprecated use transpose() */
EIGEN_DEPRECATED inline const Eigen::Transpose<Derived> Transpose() const
{
    return transpose();
};

/** \deprecated use .cross(p) */
EIGEN_DEPRECATED inline PlainMatrixType operator^(const Derived& p) const
{
    return this->cross(p);
}
...
inline Derived& HomoNormalize()
{
    ...
    if (last != Scalar(0))
    {
        Block<Derived, SubRows, SubCols>(derived(), 0, 0, (int(Flags) & RowMajorBit) ? size() - 1 : 1,
                                         (int(Flags) & RowMajorBit) ? 1 : (size() - 1)) /
          last;
        last = Scalar(1.0);
    }
    return *this;
...
EIGEN_DEPRECATED inline Scalar NormInfinity() const
{
    return derived().cwise().abs().maxCoeff();
}
/// norm 1: sum of absolute values of components
EIGEN_DEPRECATED inline Scalar NormOne() const
{
    return derived().cwise().abs().sum();
}

/// the sum of the components
EIGEN_DEPRECATED inline Scalar Sum() const
{
    return sum();
}
/// returns the biggest component
EIGEN_DEPRECATED inline Scalar Max() const
{
    return maxCoeff();
}
/// returns the smallest component
EIGEN_DEPRECATED inline Scalar Min() const
{
    return minCoeff();
}
/// returns the index of the biggest component
EIGEN_DEPRECATED inline int MaxI() const
{
    EIGEN_STATIC_ASSERT_VECTOR_ONLY(Derived);
    int i;
    maxCoeff(&i, 0);
...
EIGEN_DEPRECATED inline int MinI() const
{
    EIGEN_STATIC_ASSERT_VECTOR_ONLY(Derived);
    int i;
    minCoeff(&i, 0);
...
template <typename OtherDerived>
EIGEN_DEPRECATED inline CwiseBinaryOp<ei_scalar_product_op<Scalar>, Derived, OtherDerived>
