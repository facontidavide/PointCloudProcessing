...
class GenNormal
{
  public:
    ...
    static void UniformCone(int vn, std::vector<Point3<ScalarType> > &NN, ScalarType AngleRad,
      ...
      public:
        ...
        void Init(int lev)
        {
            ...
            if (lev == 0)
            {
                ...
                Val(0, 2) = Point3x(0, 0, -1);

                Val(1, 0) = Point3x(-1, 0, 0);
                Val(1, 1) = Point3x(0, 0, 1);
                Val(1, 2) = Point3x(1, 0, 0);

                Val(2, 0) = Point3x(0, 0, -1);
                Val(2, 1) = Point3x(0, -1, 0);
                Val(2, 2) = Point3x(0, 0, -1);
            }
            else
            {
                OctaLevel tmp;
                ...
                for (i = 0; i < sz; ++i)
                    for (j = 0; j < sz; ++j)
                    {
                        ...
                        if ((i % 2) != 0 && (j % 2) != 0)
                            Val(i, j) = (tmp.Val(i / 2 + 0, j / 2 + 0) + tmp.Val(i / 2 + 0, j / 2 + 1) +
                                         tmp.Val(i / 2 + 1, j / 2 + 0) + tmp.Val(i / 2 + 1, j / 2 + 1)) /
                                        4.0;
                    }
                typename std::vector<Point3<ScalarType> >::iterator vi;
