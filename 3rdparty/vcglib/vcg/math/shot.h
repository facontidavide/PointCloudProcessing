...
added commodity function ProjectWorldtoViewport() to obtain directly pixel indices without calling two separate function
...
class Shot
{
  public:
    ...
    /// inverse projection from 2d camera viewport (pixel) + Zdepth to 3d world coordinates, uses inverse instead of
    /// trranspose
    /// for non-exactly-rigid rotation matrices (such as calculated by tsai and garcia)
...
/// look at (opengl-like)
template <class S, class RotationType>
void Shot<S, RotationType>::LookAt(const S &eye_x, const S &eye_y, const S &eye_z, const S &at_x, const S &at_y,
                                   const S &at_z, const S &up_x, const S &up_y, const S &up_z)
{
