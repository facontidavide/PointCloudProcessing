...
class Histogram
{
  ...
  public:
     ...
     * for a more robust working, the Histogram class stores also the two out of range intervals (-inf, minv] and [maxv,
     * +inf)
     * Each bin is left closed (eg it contains the value
     * The \a gamma parameter is applied to modify the distribution of the ranges of the bins. Default uniform
     * distibution.
     *
     */
    ...
    {
        return minElem;
    };  //! Minimum element added to the histogram. It could be < or > than MinV;.
    ScalarType MaxElem()
    {
        return maxElem;
    };  //! Maximum element added to the histogram. It could be < or > than MinV;..
    ...
    ScalarType RMS()
    {
        return sqrt(rms / double(cnt));
    }

    //! Returns the variance of the data.
    ScalarType Variance()
    {
        return fabs(rms / cnt - Avg() * Avg());
    }
...
{
    // lower_bound returns the furthermost iterator i in [first, last) such that, for every iterator j in [first, i), *j
    // < value.
