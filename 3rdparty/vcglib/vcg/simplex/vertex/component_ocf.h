...
class vector_ocf : public std::vector<VALUE_TYPE>
{
  ...
  public:
    ...
    void push_back(const VALUE_TYPE &v)
    {
        ...
        if (CurvatureEnabled)
            CuV.push_back(typename VALUE_TYPE::CurvatureType());
        if (CurvatureDirEnabled)
            CuDV.push_back(typename VALUE_TYPE::CurvatureDirType());
        if (RadiusEnabled)
            RadiusV.push_back(typename VALUE_TYPE::RadiusType());
    }
    void pop_back();
...
class NormalOcf : public T
{
  public:
    ...
    void ImportData(const LeftV &leftV)
    {
        if ((*this).Base().NormalEnabled &&
            leftV.Base().NormalEnabled)  // copy the data only if they are enabled in both vertices
            N().Import(leftV.cN());
        T::ImportData(leftV);
    }
};
  ...
  public:
    ...
    void ImportData(const LeftV &leftV)
    {
        if ((*this).Base().ColorEnabled &&
            leftV.Base().ColorEnabled)  // copy the data only if they are enabled in both vertices
            C() = leftV.cC();
        T::ImportData(leftV);
    }
...
class TexCoordOcf : public TT
{
  public:
    ...
    void ImportData(const LeftV &leftV)
    {
        // if((*this).Base().TexCoordEnabled && leftV.Base().TexCoordEnabled ) // WRONG I do not know anything about
        // leftV!
...
template <class T>
class TexCoordfOcf : public TexCoordOcf<TexCoord2<float, 1>, T>
{
};
...
class CurvatureOcf : public TT
{
  public:
    ...
    {
        return v->Base().CurvatureEnabled;
    }
    ...
    {
        name.push_back(std::string("CurvatureOcf"));
        TT::Name(name);
    }
...
template <class T>
class CurvaturefOcf : public CurvatureOcf<float, T>
{
};
template <class T>
class CurvaturedOcf : public CurvatureOcf<double, T>
{
};
...
template <class A, class TT>
class CurvatureDirOcf : public TT
{
  public:
    ...
    VecType &PD1()
    {
        ...
        return (*this).Base().CuDV[(*this).Index()].max_dir;
    }
    VecType &PD2()
    {
        assert((*this).Base().CurvatureDirEnabled);
        return (*this).Base().CuDV[(*this).Index()].min_dir;
    }
    ...
    {
        assert((*this).Base().CurvatureDirEnabled);
        return (*this).Base().CuDV[(*this).Index()].k1;
    }
    ScalarType &K2()
    {
        assert((*this).Base().CurvatureDirEnabled);
        return (*this).Base().CuDV[(*this).Index()].k2;
    }
    ...
    static void Name(std::vector<std::string> &name)
    {
        name.push_back(std::string("CurvatureDirOcf"));
        TT::Name(name);
    }
...
template <class T>
class CurvatureDirfOcf : public CurvatureDirOcf<CurvatureDirTypeOcf<float>, T>
{
  public:
    static void Name(std::vector<std::string> &name)
    {
        name.push_back(std::string("CurvatureDirfOcf"));
        T::Name(name);
    }
};
template <class T>
class CurvatureDirdOcf : public CurvatureDirOcf<CurvatureDirTypeOcf<double>, T>
{
  public:
    static void Name(std::vector<std::string> &name)
    {
...
class RadiusOcf : public TT
{
  public:
    ...
    RadiusType &R()
    {
        assert((*this).Base().RadiusEnabled);
        return (*this).Base().RadiusV[(*this).Index()];
    }
    ...
    static void Name(std::vector<std::string> &name)
    {
        name.push_back(std::string("RadiusOcf"));
        TT::Name(name);
    }
...
template <class T>
class RadiusfOcf : public RadiusOcf<float, T>
{
};
template <class T>
class RadiusdOcf : public RadiusOcf<double, T>
{
};
...
{
    if (VertexType::HasVFAdjacencyOcf())
        return m.vert.IsVFAdjacencyEnabled();
    else
...
{
    if (VertexType::HasRadiusOcf())
        return m.vert.IsRadiusEnabled();
    else
        return VertexType::HasRadius();
}

template <class VertexType, class ContainerType0, class ContainerType1, class ContainerType2>
bool HasPerVertexQuality(
  const TriMesh<vertex::vector_ocf<VertexType>, ContainerType0, ContainerType1, ContainerType2> &m)
{
    if (VertexType::HasQualityOcf())
        return m.vert.IsQualityEnabled();
    else
        return VertexType::HasQuality();
}

template <class VertexType, class ContainerType0, class ContainerType1, class ContainerType2>
bool HasPerVertexTexCoord(
  const TriMesh<vertex::vector_ocf<VertexType>, ContainerType0, ContainerType1, ContainerType2> &m)
{
    if (VertexType::HasTexCoordOcf())
        return m.vert.IsTexCoordEnabled();
    else
...
{
    if (VertexType::HasColorOcf())
        return m.vert.IsColorEnabled();
    else
        return VertexType::HasColor();
}

template <class VertexType, class ContainerType0, class ContainerType1, class ContainerType2>
bool HasPerVertexCurvature(
  const TriMesh<vertex::vector_ocf<VertexType>, ContainerType0, ContainerType1, ContainerType2> &m)
{
    if (VertexType::HasCurvatureOcf())
        return m.vert.IsCurvatureEnabled();
    else
        return VertexType::HasCurvature();
}

template <class VertexType, class ContainerType0, class ContainerType1, class ContainerType2>
bool HasPerVertexCurvatureDir(
  const TriMesh<vertex::vector_ocf<VertexType>, ContainerType0, ContainerType1, ContainerType2> &m)
{
    if (VertexType::HasCurvatureDirOcf())
        return m.vert.IsCurvatureDirEnabled();
    else
        return VertexType::HasCurvatureDir();
}
}
}  // end namespace vcg
#endif
