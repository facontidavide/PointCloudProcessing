 ...
 * including 'vcg/simplex/vertex/with/VCVN.h' allow you to use the class VertVCVN that has per-vertex color and normal
 * stored inside.
 */
template <class FLTYPE, class VETYPE = DUMMYEDGETYPE, class VFTYPE = DUMMYFACETYPE, class VTTYPE = DUMMYTETRATYPE,
          class TCTYPE = TexCoord2<float, 1>, class CoordTYPE = Point3<FLTYPE> >
class VERTEX_TYPE
{
  public:
  ...
  public:
    ...
    Users can ask and dispose for a bit for their own purposes with the  vcg::VertexFull::NewUserBit() and
    vcg::VertexFull::DeleteUserBit() functions.
    The value returned by these functions has to be passed to the
    vcg::VertexFull::SetUserBit() vcg::VertexFull::ClearUserBit() and vcg::VertexFull::IsUserBit() functions to check
    and modify the obtained bit flag.
...
//@{
  ...
  public:
    ...
    const CoordType GenericNormal()
    {
        ...
        {
            vcg::face::VFIterator<typename VERTEX_TYPE::FaceType> VFi =
              vcg::face::VFIterator<typename VERTEX_TYPE::FaceType>();
            VFi.f = VFp();
            VFi.z = VFi();
    ...
    const CoordType Normal()
    {
        return GenericNormal<false>();
    }

    /// Return the normalized value of the vertex normal as it correspond to the current geometry.
    /// It is always computed and never use any stored value.
    /// REQUIRES vertex-face topology
    const CoordType NormalizedNormal()
    {
        return GenericNormal<true>();
    }
    ...
    **/
    ...
    {  // Note the plural because ONE vertex has many Flags (but just one color, normal, mark, quality ecc.)
        return true;
    }
