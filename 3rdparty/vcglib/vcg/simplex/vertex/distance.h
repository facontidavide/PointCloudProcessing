...
class PointDistanceFunctor
{
  public:
     ...
     *
     */
    inline bool operator()(const VERTEXTYPE& v, const Point3<SCALARTYPE>& p, SCALARTYPE& minDist,
                           Point3<SCALARTYPE>& q) const
    {
...
class PointNormalDistanceFunctor
{
  public:
    ...
    static inline const Point3<typename VERTYPE::ScalarType>& Pos(const QueryType& qt)
    {
        return qt.P();
    }
    ...
    inline bool operator()(const VERTEXTYPE& v, const VERTEXTYPE& vp, SCALARTYPE& minDist, Point3<SCALARTYPE>& q)
    {
        ...
        float dev =
          InterPoint() * (pow((ScalarType)(1 - v.cN().dot(vp.cN())), (ScalarType)Beta()) / (Gamma() * h + 0.1));
        if (h + dev < minDist)
        {
            minDist = h + dev;
            q = v.P();
