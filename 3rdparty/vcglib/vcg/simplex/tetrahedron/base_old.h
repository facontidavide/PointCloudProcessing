...
class TETRA_TYPE
{
  public:
    ...
    Users can ask and dispose for a bit for their own purposes with the  vcg::TetrahedronFull::NewUserBit() and
    vcg::TetrahedronFull::DeleteUserBit() functions.
    The value returned by these functions has to be passed to the
    vcg::TetrahedronFull::SetUserBit() vcg::TetrahedronFull::ClearUserBit() and vcg::TetrahedronFull::IsUserBit()
    functions to check and modify the obtained bit flag.
...
//@{
  ...
  public:
    ...
    /// Calculate the normal to all the faces of a tetrahedron, the value is store in a position of vecton _n for each
    /// face
    void ComputeNormal()
    {
#ifdef __VCGLIB_TETRA_TN
...
//@{
   ...
   are resolved at compile time and they give information about the data supported by the current tetra type.
    **/
    //@{
