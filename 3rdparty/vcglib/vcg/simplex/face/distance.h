...
changed Dist Function to PointDistance... the function is on vcg::face::PointDistance this file will contain all
distance functions between a face and othe entities
...
bool PointDistance(const FaceType &f, const vcg::Point3<typename FaceType::ScalarType> &q,
                   typename FaceType::ScalarType &dist, vcg::Point3<typename FaceType::ScalarType> &p)
{
    ...
    switch (f.Flags() & (FaceType::NORMX | FaceType::NORMY | FaceType::NORMZ))
    {
        case FaceType::NORMX:
            ...
            if ((b = std::min(b0, std::min(b1, b2))) < EPS * DoubleArea(f))
            {
                ScalarType bt;
                if (b == b0)
                    bt = PSDist(q, f.V(1)->cP(), f.V(2)->cP(), p);
                else if (b == b1)
                    bt = PSDist(q, f.V(2)->cP(), f.V(0)->cP(), p);
                else
                {
                    assert(b == b2);
...
class PointNormalDistanceFunctor
{
  public:
    ...
    {
        static ScalarType alpha = 1.0;
        return alpha;
    }
    static ScalarType &Beta()
    {
        static ScalarType beta = 1.0;
        return beta;
    }
    static ScalarType &Gamma()
    {
        static ScalarType gamma = 1.0;
        return gamma;
    }
    static ScalarType &InterPoint()
    {
        static ScalarType interpoint = 1.0;
        return interpoint;
    }

    template <class FACETYPE, class SCALARTYPE>
    inline bool operator()(const FACETYPE &f, const typename FACETYPE::VertexType &p, SCALARTYPE &minDist,
                           Point3<SCALARTYPE> &q)
    {
        ...
        SCALARTYPE dev =
          InterPoint() * (pow((ScalarType)(1 - f.cN().dot(fn)), (ScalarType)Beta()) / (Gamma() * md + 0.1));
...
bool PointDistanceBase(const FaceType &f,                                    /// the face to be tested
                       ...
                       typename FaceType::ScalarType &dist,  /// bailout distance. It must be initialized with the max
                                                             /// admittable value.
    ...
    */
    ...
    switch (f.Flags() & (FaceType::NORMX | FaceType::NORMY | FaceType::NORMZ))
    {
        ...
        case FaceType::NORMZ:
            ...
            break;
        default:
            assert(0);  // if you get this assert it means that you forgot to set the required
                        // UpdateFlags<MeshType>::FaceProjection(m);
    }
