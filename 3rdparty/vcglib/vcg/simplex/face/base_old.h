 ...
 * including 'vcg/simplex/face/with/FCFN.h' allow you to use the class FaceFCFN that has per-face color and normal
 * stored inside.
 */
template <class FVTYPE, class FETYPE, class FFTYPE, class TCTYPE = TexCoord2<float, 1> >
class FACE_TYPE
{
  public:
...
#error Error: You cannot specify vertex-face and shared topology together
  ...
  public:
    ...
    inline const FFTYPE *cFFp(const int j) const
    {
        return FFp(j);
    }

    inline FFTYPE *&FFp1(const int j)
    {
        return FFp((j + 1) % 3);
    }
    inline FFTYPE *&FFp2(const int j)
    {
        return FFp((j + 2) % 3);
    }
    inline const FFTYPE *const &FFp1(const int j) const
    {
        return FFp((j + 1) % 3);
    }
    inline const FFTYPE *const &FFp2(const int j) const
    {
        return FFp((j + 2) % 3);
    }
    ...
    inline const FFTYPE *cVFp(const int j) const
    {
        return VFp(j);
    }
    ...
    inline const char &cFFi(const int j) const
    {
        return FFi(j);
    }
...
//@{
  ...
  public:
    ...
    bool IsRW() const
    {
        return (_flags & (NOTREAD | NOTWRITE)) == 0;
    }
   ...
   Reflection is a mechanism making it possible to investigate yourself. Reflection is used to investigate format of
   objects at runtime, invoke methods and access fields of these objects. Here we provide static const functions that
   are resolved at compile time and they give information about the data (normal, color etc.) supported by the current
   vertex type.
    **/
    //@{
...
#undef pz
    ...
    {
        return Distance(V(0)->P(), V(1)->P()) + Distance(V(1)->P(), V(2)->P()) + Distance(V(2)->P(), V(0)->P());
    }
...
// Sezione dist e ray
    ...
    CoordType &Edge(const int &i)
    {
        return edges[i];
    };
    CoordType cEdge(const int &i) const
    {
        return edges[i];
    };
    ...
    Plane3<ScalarType> cPlane() const
    {
        return plane;
    };
...
void ComputeNormal(FaceType &f)
{
    f.N() = vcg::Normal<FaceType>(f);
}

template <class FaceType>
void ComputeNormalizedNormal(FaceType &f)
{
    f.N() = vcg::NormalizedNormal<FaceType>(f);
}
