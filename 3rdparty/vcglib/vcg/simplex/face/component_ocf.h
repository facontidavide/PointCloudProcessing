  ...
  public:
    ...
    void push_back(const VALUE_TYPE &v)
    {
        ...
        if (QualityEnabled)
            QV.push_back(0);
        if (ColorEnabled)
            CV.push_back(vcg::Color4b(vcg::Color4b::White));
        if (MarkEnabled)
            MV.push_back(0);
        if (NormalEnabled)
        ...
        if (WedgeTexEnabled)
            WTV.push_back(WedgeTexTypePack());
        if (WedgeColorEnabled)
            WCV.push_back(WedgeColorTypePack());
        if (WedgeNormalEnabled)
            WNV.push_back(WedgeNormalTypePack());
    }
    void pop_back();
    void resize(const unsigned int &_size)
    {
        ...
        if (FFAdjacencyEnabled)
            AF.resize(_size);
        if (WedgeTexEnabled)
            WTV.resize(_size, WedgeTexTypePack());
        if (WedgeColorEnabled)
            WCV.resize(_size);
        if (WedgeNormalEnabled)
            WNV.resize(_size);
    ...
    // this function is called by the specialized Reorder function, that is called whenever someone call the
    // allocator::CompactVertVector
    void ReorderFace(std::vector<size_t> &newFaceIndex)
    {
        size_t i = 0;
        if (QualityEnabled)
            assert(QV.size() == newFaceIndex.size());
        if (ColorEnabled)
            assert(CV.size() == newFaceIndex.size());
        if (MarkEnabled)
            assert(MV.size() == newFaceIndex.size());
        if (NormalEnabled)
            assert(NV.size() == newFaceIndex.size());
        if (VFAdjacencyEnabled)
            assert(AV.size() == newFaceIndex.size());
        if (FFAdjacencyEnabled)
            assert(AF.size() == newFaceIndex.size());
        if (WedgeTexEnabled)
            assert(WTV.size() == newFaceIndex.size());
        if (WedgeColorEnabled)
            assert(WCV.size() == newFaceIndex.size());
        if (WedgeNormalEnabled)
            assert(WNV.size() == newFaceIndex.size());
        ...
        for (i = 0; i < newFaceIndex.size(); ++i)
        {
            if (newFaceIndex[i] != std::numeric_limits<size_t>::max())
            {
                ...
                if (QualityEnabled)
                    QV[newFaceIndex[i]] = QV[i];
                if (ColorEnabled)
                    CV[newFaceIndex[i]] = CV[i];
                if (MarkEnabled)
                    MV[newFaceIndex[i]] = MV[i];
                if (NormalEnabled)
                    NV[newFaceIndex[i]] = NV[i];
                if (VFAdjacencyEnabled)
                    AV[newFaceIndex[i]] = AV[i];
                if (FFAdjacencyEnabled)
                    AF[newFaceIndex[i]] = AF[i];
                if (WedgeTexEnabled)
                    WTV[newFaceIndex[i]] = WTV[i];
                if (WedgeColorEnabled)
                    WCV[newFaceIndex[i]] = WCV[i];
                if (WedgeNormalEnabled)
                    WNV[newFaceIndex[i]] = WNV[i];
            }
        }

        if (QualityEnabled)
            QV.resize(BaseType::size());
        if (ColorEnabled)
            CV.resize(BaseType::size());
        if (MarkEnabled)
            MV.resize(BaseType::size());
        if (NormalEnabled)
            NV.resize(BaseType::size());
        if (VFAdjacencyEnabled)
            AV.resize(BaseType::size());
        if (FFAdjacencyEnabled)
            AF.resize(BaseType::size());
        if (WedgeTexEnabled)
            WTV.resize(BaseType::size());
        if (WedgeColorEnabled)
            WCV.resize(BaseType::size());
        if (WedgeNormalEnabled)
            WNV.resize(BaseType::size());
    }
...
template <class T>
class Normal3sOcf : public NormalOcf<vcg::Point3s, T>
{
};
template <class T>
class Normal3fOcf : public NormalOcf<vcg::Point3f, T>
{
};
template <class T>
class Normal3dOcf : public NormalOcf<vcg::Point3d, T>
{
};
...
template <class T>
class QualityfOcf : public QualityOcf<float, T>
{
};
...
template <class T>
class Color4bOcf : public ColorOcf<vcg::Color4b, T>
{
};
...
class WedgeTexCoordOcf : public TT
{
  public:
    WedgeTexCoordOcf()
    {
    ...
    {
        assert((*this).Base().WedgeTexEnabled);
        return (*this).Base().WTV[(*this).Index()].wt[i];
    }
    ...
    void ImportData(const LeftF &leftF)
    {
        // if(this->Base().WedgeTexEnabled && leftF.Base().WedgeTexEnabled)  // WRONG I do not know anything about
        // leftV!
...
template <class T>
class WedgeTexCoordfOcf : public WedgeTexCoordOcf<TexCoord2<float, 1>, T>
{
};
...
class WedgeColorOcf : public TT
{
  public:
    ...
    {
        assert((*this).Base().WedgeColorEnabled);
        return (*this).Base().WCV[(*this).Index()].wc[i];
    }
    ...
    void ImportData(const LeftF &leftF)
    {
        // if(this->Base().WedgeColorEnabled && leftF.Base().WedgeColorEnabled)  // WRONG I do not know anything about
        // leftV!
...
template <class T>
class WedgeColor4bOcf : public WedgeColorOcf<vcg::Color4b, T>
{
};
...
template <class A, class TT>
class WedgeNormalOcf : public TT
{
  public:
    WedgeNormalOcf()
    {
    ...
    {
        assert((*this).Base().WedgeNormalEnabled);
        return (*this).Base().WNV[(*this).Index()].wn[i];
    }
    ...
    void ImportData(const LeftF &leftF)
    {
        // if(this->Base().WedgeNormalEnabled && leftF.Base().WedgeNormalEnabled)  // WRONG I do not know anything about
        // leftV!
...
template <class T>
class WedgeNormal3sOcf : public WedgeNormalOcf<vcg::Point3s, T>
{
};
template <class T>
class WedgeNormal3fOcf : public WedgeNormalOcf<vcg::Point3f, T>
{
};
template <class T>
class WedgeNormal3dOcf : public WedgeNormalOcf<vcg::Point3d, T>
{
};
...
bool HasPerFaceVFAdjacency(const TriMesh<VertContainerType, face::vector_ocf<FaceType>, Container1, Container2> &m)
{
    if (FaceType::HasVFAdjacencyOcf())
        return m.face.IsVFAdjacencyEnabled();
    else
        return FaceType::FaceType::HasVFAdjacency();
}
...
bool HasFFAdjacency(const TriMesh<VertContainerType, face::vector_ocf<FaceType>, Container1, Container2> &m)
{
    if (FaceType::HasFFAdjacencyOcf())
        return m.face.IsFFAdjacencyEnabled();
    else
        return FaceType::FaceType::HasFFAdjacency();
}

template <class VertContainerType, class FaceType, class Container1, class Container2>
bool HasPerWedgeTexCoord(const TriMesh<VertContainerType, face::vector_ocf<FaceType>, Container1, Container2> &m)
{
    if (FaceType::HasWedgeTexCoordOcf())
        return m.face.IsWedgeTexEnabled();
    else
...
{
    if (FaceType::HasFaceColorOcf())
        return m.face.IsColorEnabled();
    else
...
{
    if (FaceType::HasFaceQualityOcf())
        return m.face.IsQualityEnabled();
    else
...
{
    if (FaceType::HasFaceMarkOcf())
        return m.face.IsMarkEnabled();
    else
