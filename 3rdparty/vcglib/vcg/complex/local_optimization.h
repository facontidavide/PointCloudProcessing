...
/****************************************************************************
  ...
  Changed the sense of the < operator for heap: it is reversed according to the stl where highest score elements must
float in the heap
  ...
  *** empty log message ***
  ...
  Added a member with a cached copy of the floating Priority() value inside the HeapElem to optimize operator< in heap
updating operator
...
class LocalModification
{
  public:
    ...
    /// Perform the operation and return the variation in the number of simplicies (>0 is refinement, <0 is
    /// simplification)
...
/// Implementation note: in order to keep the local modification itself indepented by its use in this class, they are
/// not
/// really derived by LocalModification. Instead, a wrapper is done to this purpose (see
...
{
  public:
    LocalOptimization(MeshType &mm) : m(mm)
    {
        ClearTermination();
        e = 0.0;
        HeapSimplexRatio = 5;
    ...
    int nPerfmormedOps, nTargetOps, nTargetSimplices, nTargetVertices;
    ...
    {
        nTargetSimplices = ts;
        SetTerminationFlag(LOnSimplices);
    }
    void SetTargetVertices(int tv)
    {
        nTargetVertices = tv;
        SetTerminationFlag(LOnVertices);
    }
    void SetTargetOperations(int to)
    {
        nTargetOps = to;
        SetTerminationFlag(LOnOps);
    }

    void SetTargetMetric(ScalarType tm)
    {
        targetMetric = tm;
        SetTerminationFlag(LOMetric);
    }
    void SetTimeBudget(float tb)
    {
        timeBudget = tb;
        SetTerminationFlag(LOTime);
    }
    ...
    bool GoalReached()
    {
        ...
        if (IsTerminationFlag(LOnSimplices) && (m.SimplexNumber() <= nTargetSimplices))
            return true;
        if (IsTerminationFlag(LOnVertices) && (m.VertexNumber() <= nTargetVertices))
            return true;
        if (IsTerminationFlag(LOnOps) && (nPerfmormedOps == nTargetOps))
            return true;
        if (IsTerminationFlag(LOMetric) && (currMetric > targetMetric))
            return true;
        if (IsTerminationFlag(LOTime) && ((clock() - start) / (float)CLOCKS_PER_SEC > timeBudget))
            return true;
        return false;
    }
