...
Added Rad to the NormalThr Option and removed a bug in multiple exectuion of non optimal simplification (missing an isD
check)
...
Heavyly restructured for meshlab inclusion. Now the access to the quadric elements are mediated by a static helper
class.
...
Moved param classes inside; added support for optimal placement and symmetric; added update heap also here (not only in
the base class)
...
class QInfoStandard
{
  public:
    ...
    static typename VERTEX_TYPE::ScalarType W(VERTEX_TYPE *v)
    {
        return 1.0;
    };
    static typename VERTEX_TYPE::ScalarType W(VERTEX_TYPE &v)
...
class TriEdgeCollapseQuadricParameter
{
  public:
    ...
    //***********************
    bool QualityQuadric;  // During the initialization manage all the edges as border edges adding a set of additional
                          // quadrics that are useful mostly for keeping face aspect ratio good.
...
class TriEdgeCollapseQuadric : public TriEdgeCollapse<TriMeshType, MYTYPE>
{
  public:
    ...
    {
        static std::vector<typename TriMeshType::VertexPointer> _WV;
        return _WV;
    };
    ...
    inline bool IsFeasible()
    {
        ...
        if (!res)
            ++(TriEdgeCollapse<TriMeshType, MYTYPE>::FailStat::LinkConditionEdge());
        return res;
    }
    ...
    void Execute(TriMeshType &m)
    {
        ...
        if (Params().OptimalPlacement)
            newPos = static_cast<MYTYPE *>(this)->ComputeMinimal();
        else
    ...
    static void Init(TriMeshType &m, HeapType &h_ret)
    {
        ...
        else if (!IsSetHint(HNHasBorderFlag))
            vcg::tri::UpdateFlags<TriMeshType>::FaceBorderFromVF(m);
            ...
            for (vi = m.vert.begin(); vi != m.vert.end(); ++vi)
                if (!(*vi).IsD() && (*vi).IsRW())
                {
                    ...
                    for (x.F() = (*vi).VFp(), x.I() = (*vi).VFi(); x.F() != 0; ++x)
                    {
                        ...
                        {
                            x.V1()->SetV();
                            h_ret.push_back(HeapElem(new MYTYPE(EdgeType(x.V0(), x.V1()),
                                                                TriEdgeCollapse<TriMeshType, MYTYPE>::GlobalMark())));
                        }
                        if ((x.V0() < x.V2()) && x.V2()->IsRW() && !x.V2()->IsV())
                        {
                            x.V2()->SetV();
                            h_ret.push_back(HeapElem(new MYTYPE(EdgeType(x.V0(), x.V2()),
                                                                TriEdgeCollapse<TriMeshType, MYTYPE>::GlobalMark())));
                        }
                    }
                }
            ...
            for (vi = m.vert.begin(); vi != m.vert.end(); ++vi)
                if (!(*vi).IsD() && (*vi).IsRW())
                {
                    ...
                    for (x.F() = (*vi).VFp(), x.I() = (*vi).VFi(); x.F() != 0; ++x)
                    {
                        ...
                        {
                            h_ret.push_back(HeapElem(
                              new MYTYPE(EdgeType(x.V(), x.V1()), TriEdgeCollapse<TriMeshType, MYTYPE>::GlobalMark())));
                        }
                        if (x.V()->IsRW() && x.V2()->IsRW() && !IsMarked(m, x.F()->V2(x.I())))
                        {
                            h_ret.push_back(HeapElem(
                              new MYTYPE(EdgeType(x.V(), x.V2()), TriEdgeCollapse<TriMeshType, MYTYPE>::GlobalMark())));
                        }
                    }
                }
        }
    ...
    {
        return IsSymmetric() ? 5.0f : 9.0f;
    }
    static bool IsSymmetric()
    {
        return Params().OptimalPlacement;
    }
    ...
    ScalarType ComputePriority()
    {
        ...
        if (MinQual > Params().QualityThr)
            MinQual = Params().QualityThr;

        if (Params().NormalCheck)
        {
            // All collapses where the normal vary less  than <NormalThr> (e.g. more than CosineThr)
            // have no penalty
            if (MinCos > Params().CosineThr)
                MinCos = Params().CosineThr;
            MinCos = (MinCos + 1) / 2.0;  // Now it is in the range 0..1 with 0 very dangerous!
        }

        if (QuadErr < Params().QuadricEpsilon)
            QuadErr = Params().QuadricEpsilon;

        if (Params().UseVertexWeight)
            QuadErr *= (QH::W(v[1]) + QH::W(v[0])) / 2;

        if (!Params().QualityCheck && !Params().NormalCheck)
            error = (ScalarType)(QuadErr);
        if (Params().QualityCheck && !Params().NormalCheck)
            error = (ScalarType)(QuadErr / MinQual);
        if (!Params().QualityCheck && Params().NormalCheck)
            error = (ScalarType)(QuadErr / MinCos);
        if (Params().QualityCheck && Params().NormalCheck)
    ...
    static void InitQuadric(TriMeshType &m)
    {
                ...
                if ((*pf).V(0)->IsR() && (*pf).V(1)->IsR() && (*pf).V(2)->IsR())
                {
                    ...
                    for (j = 0; j < 3; ++j)
                        if ((*pf).IsB(j) || Params().QualityQuadric)  // Bordo!
                        {
                            ...
                            // Senza la normalize il bordo e' pesato in funzione della grandezza della mesh (mesh grandi
                            // non decimano sul bordo)
                            pb.SetDirection(p.Direction() ^ ((*pf).V1(j)->cP() - (*pf).V(j)->cP()).normalized());
                            if ((*pf).IsB(j))
                                pb.SetDirection(pb.Direction() *
                                                (ScalarType)Params().BoundaryWeight);  // amplify border planes
                            else
                                pb.SetDirection(pb.Direction() *
                                                (ScalarType)(Params().BoundaryWeight /
                                                             100.0));  // and consider much less quadric for quality
                            pb.SetOffset(pb.Direction().dot((*pf).V(j)->cP()));
                            q.ByPlane(pb);

                            if ((*pf).V(j)->IsW())
                                QH::Qd((*pf).V(j)) += q;  // Sommo le quadriche
                            if ((*pf).V1(j)->IsW())
                                QH::Qd((*pf).V1(j)) += q;
                        }
                }
        ...
        if (Params().ScaleIndependent)
        {
            ...
            // printf("bb (%5.2f %5.2f %5.2f)-(%5.2f %5.2f %5.2f) Diag
