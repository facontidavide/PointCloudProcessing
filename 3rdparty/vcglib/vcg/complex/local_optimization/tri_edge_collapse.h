...
class TriEdgeCollapse : public LocalOptimization<TriMeshType>::LocModType, public EdgeCollapse<TriMeshType>
{
  public:
    ...
    class FailStat
    {
      public:
        ...
        static int &LinkConditionFace()
        {
            static int lkf = 0;
            return lkf;
        }
        static int &LinkConditionEdge()
        {
            static int lke = 0;
            return lke;
        }
        static int &LinkConditionVert()
        {
            static int lkv = 0;
            return lkv;
        }
  ...
  public:
    ...
    // in the plain case we just put again in the heap all the edges around the vertex resulting from the previous
    // collapse: v[1].
    // if the collapse is not symmetric you should add also backward edges (because v0->v1 collapse could be different
    // from v1->v0)
        ...
        while (!vfi.End())
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                ...
                if (!(vfi.V2()->IsV()) && (vfi.V2()->IsRW()))
                {
                    vfi.V2()->SetV();
                    h_ret.push_back(
                      HeapElem(new MYTYPE(EdgeType(vfi.F()->V(vfi.I()), vfi.F()->V2(vfi.I())), GlobalMark())));
                    std::push_heap(h_ret.begin(), h_ret.end());
                    if (!this->IsSymmetric())
                    {
                        h_ret.push_back(
                          HeapElem(new MYTYPE(EdgeType(vfi.F()->V1(vfi.I()), vfi.F()->V(vfi.I())), GlobalMark())));
                        std::push_heap(h_ret.begin(), h_ret.end());
                    }
                }
    ...
    inline bool IsUpToDate()
    {
        ...
        //				 localMark>=v0->IMark() &&
        //				 localMark>=v1->IMark()))
        if (v0->IsD() || v1->IsD() || localMark < v0->IMark() || localMark < v1->IMark())
        {
            ++FailStat::OutOfDate();
            return false;
        }
        ...
        for (fi = m.face.begin(); fi != m.face.end(); ++fi)
            if (!(*fi).IsD())
            {
                for (int j = 0; j < 3; j++)
                {
                    ...
                    // printf("Inserting in heap coll %3i ->%3i
