...
template <class TRIMESH_TYPE, class MYTYPE,
          typename TRIMESH_TYPE::ScalarType (*QualityFunc)(
            Point3<typename TRIMESH_TYPE::ScalarType> const &p0, Point3<typename TRIMESH_TYPE::ScalarType> const &p1,
...
class TopoEdgeFlip : public PlanarEdgeFlip<TRIMESH_TYPE, MYTYPE>
{
  ...
  public:
    ...
    ScalarType ComputePriority()
    {
        ...
        ScalarType varbefore =
          (powf(v0->Q() - avg, 2.0) + powf(v1->Q() - avg, 2.0) + powf(v2->Q() - avg, 2.0) + powf(v3->Q() - avg, 2.0)) /
          4.0;

        ScalarType varafter = (powf(v0->Q() - 1 - avg, 2.0) + powf(v1->Q() - 1 - avg, 2.0) +
                               powf(v2->Q() + 1 - avg, 2.0) + powf(v3->Q() + 1 - avg, 2.0)) /
                              4.0;
    ...
    void UpdateHeap(HeapType &heap)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            do
            {  // go to the first border (if there is one)
                pos.NextE();
