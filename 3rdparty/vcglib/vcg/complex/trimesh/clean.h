...
class Clean
{
  public:
    ...
    */
    static int RemoveDuplicateFace(MeshType &m)  // V1.0
    {
        ...
        for (fi = m.face.begin(); fi != m.face.end(); ++fi)
            ...
            {
                fvec.push_back(
                  SortedTriple(tri::Index(m, (*fi).V(0)), tri::Index(m, (*fi).V(1)), tri::Index(m, (*fi).V(2)), &*fi));
            }
        assert(size_t(m.fn) == fvec.size());
        // for(int i=0;i<fvec.size();++i) qDebug("fvec[%i] = (%i %i
        // %i)(%i)",i,fvec[i].v[0],fvec[i].v[1],fvec[i].v[2],tri::Index(m,fvec[i].fp));
    ...
    */
    ...
    static int RemoveDegenerateVertex(MeshType &m)
    {
        ...
        for (vi = m.vert.begin(); vi != m.vert.end(); ++vi)
            if (math::IsNAN((*vi).P()[0]) || math::IsNAN((*vi).P()[1]) || math::IsNAN((*vi).P()[2]))
            {
                count_vd++;
                Allocator<MeshType>::DeleteVertex(m, *vi);
            }
    ...
    */
    ...
    static int RemoveNonManifoldFace(MeshType &m)
    {
        ...
        for (fi = m.face.begin(); fi != m.face.end(); ++fi)
            if (!fi->IsD())
            {
                if ((!IsManifold(*fi, 0)) || (!IsManifold(*fi, 1)) || (!IsManifold(*fi, 2)))
        ...
        for (size_t i = 0; i < ToDelVec.size(); ++i)
        {
            if (!ToDelVec[i]->IsD())
            {
                ...
                if ((!IsManifold(ff, 0)) || (!IsManifold(ff, 1)) || (!IsManifold(ff, 2)))
                {
                    for (int j = 0; j < 3; ++j)
    ...
    They remove the faces that are out of a given range of area or edges (e.g. faces too large or too small, or with
    edges too short or too long)
    ...
    */
    ...
    static int RemoveFaceOutOfRangeAreaSel(MeshType &m, ScalarType MinAreaThr = 0,
    ...
    {
        return RemoveFaceOutOfRangeArea(m);
    }
    ...
    static bool IsBitQuadOnly(const MeshType &m)
    {
        ...
        for (ConstFaceIterator fi = m.face.begin(); fi != m.face.end(); ++fi)
            if (!fi->IsD())
            {
                ...
                if (tmp != F::FAUX0 && tmp != F::FAUX1 && tmp != F::FAUX2)
                    return false;
            }
        return true;
    }
    ...
    static bool IsBitTriQuadOnly(const MeshType &m)
    {
        ...
        for (ConstFaceIterator fi = m.face.begin(); fi != m.face.end(); ++fi)
            ...
            {
                unsigned int tmp = fi->Flags() & (F::FAUX0 | F::FAUX1 | F::FAUX2);
                if (tmp != F::FAUX0 && tmp != F::FAUX1 && tmp != F::FAUX2 && tmp != 0)
                    return false;
            }
        return true;
    }
    ...
    /**
        ...
        *  FN - EN_f (each faux edge hides exactly one triangular face or in other words a polygon of n edges has n-3
  *faux edges.)
  ...
  * as a intuitive proof think to a internal vertex that is collapsed onto a border of a polygon:
    ...
    static int CountBitLargePolygons(MeshType &m)
    {
        ...
        // Second Loop, count (twice) faux edges and mark all vertices touched by non faux edges (e.g vertexes on the
            ...
            if (!((*fi).IsD()))
            {
                ...
                for (int j = 0; j < 3; j++)
                {
                    ...
                    else  // We have a non-manifold edge
                    {
                        ...
                        {
                            if (he.f->IsV())  // if one of the other faces was already visited than this edge was
                                              // counted already.
                            {
                                counted = true;
                                break;
            ...
            for (int j = 0; j < 3; j++)  // for all edges
            {
                ...
                if (face::IsBorder(*fi, j))  // found an unvisited border edge
                {
                    ...
                    while (fi->V(j) != he.v)  // will we do not encounter the first boundary edge.
                    {
                        ...
                        if (he.v->IsS())  // check if this vertex was selected already, because then we have an
                                          // additional hole.
                        {
                            // cut and paste the additional hole.
                            std::vector<Point3x> hole2;
                            int index = static_cast<int>(find(hole.begin(), hole.end(), newpoint) - hole.begin());
    ...
    */
    ...
    static int RemoveFaceFoldByFlip(MeshType &m, float normalThresholdDeg = 175, bool repeat = true)
    {
        ...
        do
        {
            ...
            ScalarType eps =
              0.0001;  // this epsilon value is in absolute value. It is a distance from edge in baricentric coords.
            // detection stage
            for (FaceIterator fi = m.face.begin(); fi != m.face.end(); ++fi)
                if (!(*fi).IsV())
                {
    ...
    /**
    This function simply test that the vn and fn counters be consistent with the size of the containers and the number
    of deleted simplexes.
    */
    static bool IsSizeConsistent(MeshType &m)
    {
        ...
        if (size_t(m.vn + DeletedVertexNum) != m.vert.size())
            return false;
        if (size_t(m.fn + DeletedFaceNum) != m.face.size())
            return false;
    ...
    */
    static bool HasZeroTexCoordFace(MeshType &m)
    {
        ...
        for (FaceIterator fi = m.face.begin(); fi != m.face.end(); ++fi)
            if (!(*fi).IsD())
            {
                if ((*fi).WT(0).P() == (*fi).WT(1).P() && (*fi).WT(0).P() == (*fi).WT(2).P())
                    return false;
            }
        return true;
    }
    ...
    */
    static bool TestIntersection(FaceType *f0, FaceType *f1)
    {
        ...
        if (sv == 1)
        {
            ...
            if (vcg::IntersectionSegmentTriangle(Segment3<ScalarType>((*f0).V1(i0)->P(), (*f0).V2(i0)->P()), *f1, a, b))
                return true;
