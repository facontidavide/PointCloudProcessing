...
class TrivialEar
{
  public:
    ...
    virtual inline bool operator<(const TrivialEar &c) const
    {
    ...
    {
        quality = QualityFace(*this);
    };
    bool IsUpToDate()
    {
        return (e0.IsBorder() && e1.IsBorder());
    };
...
class MinimumWeightEar : public TrivialEar<MESH>
{
  public:
    ...
    // the pow function is here just to give a way to play with different weighting schemas, balancing in a different
    // way
    ...
    virtual inline bool operator<(const MinimumWeightEar &c) const
    {
        ...
        {
            return (pow((float)dihedralRad, (float)DiedralWeight()) / aspectRatio) >
                   (pow((float)c.dihedralRad, (float)DiedralWeight()) / c.aspectRatio);
        }
        if (TE::IsConcave())
            return true;
        // assert(c.IsConcave());
  ...
  public:
    class Info
    {
      public:
        ...
        bool operator<(const Info &hh) const
        {
            return size < hh.size;
        }
    ...
    class Weight
    {
      public:
        ...
        {
            return Weight(std::max(angle(), other.angle()), area() + other.area());
        }
...
*/
    ...
    static void triangulate(MESH &m, FaceIterator &f, int i, int j, std::vector<std::vector<int> > vi,
