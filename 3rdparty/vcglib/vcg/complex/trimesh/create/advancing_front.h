...
class FrontEdge
{
  public:
    ...
    FrontEdge(int _v0, int _v1, int _v2, int _face) : v0(_v0), v1(_v1), v2(_v2), face(_face), active(true)
    {
...
class AdvancingFront
{
  ...
  public:
    ...
    void BuildMesh(CallBackPos call = NULL, int interval = 512)
    {
        ...
        if (call)
            call(0, "Advancing front");
        while (1)
        {
            for (int i = 0; i < interval; i++)
            {
                if (!front.size() && !SeedFace())
                    return;
                AddFace();
                if (call)
                {
...
template <class MESH>
class AdvancingTest : public AdvancingFront<MESH>
{
  public:
    typedef typename MESH::VertexType VertexType;
