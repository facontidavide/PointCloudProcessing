...
{
template <class MESH>
class BallPivoting : public AdvancingFront<MESH>
{
  public:
  ...
  public:
    ...
    bool Seed(int &v0, int &v1, int &v2)
    {
        ...
        while (++last_seed < (int)(this->mesh.vert.size()))
        {
            ...
            if (seed.IsD() || seed.IsUserBit(usedBit))
                continue;
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                for (int k = i + 1; k < n; k++)
                {
                    ...
                    for (int j = k + 1; j < n; j++)
                    {
                        ...
                        Point3x &p2 = vv2->P();
                        float d1 = (p2 - p0).Norm();
                        if (d1 < min_edge || d1 > max_edge)
                            continue;
                        float d0 = (p2 - p1).Norm();
                        if (d0 < min_edge || d0 > max_edge)
                            continue;

                        Point3x normal = (p1 - p0) ^ (p2 - p0);
                        if (normal.dot(p0 - baricenter) < 0)
                            continue;
                        /*            if(use_normals) {
        ...
        */
        ...
        for (int i = 0; i < static_cast<int>(targets.size()); i++)
        {
            ...
            if (v->IsB())
                assert(v->IsV());
            if (v->IsV())
                assert(v->IsUserBit(usedBit));

            if (v->IsUserBit(usedBit) && !(v->IsB()))
                continue;
            if (id == edge.v0 || id == edge.v1 || id == edge.v2)
                continue;
