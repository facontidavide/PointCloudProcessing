...
template <class OLD_MESH_TYPE, class NEW_MESH_TYPE, class FLT,
          class DISTFUNCTOR = vcg::face::PointDistanceBaseFunctor<typename OLD_MESH_TYPE::ScalarType> >
class Resampler : public BasicGrid<FLT>
{
    ...
    class Walker : BasicGrid<float>
    {
      ...
      public:
        ...
        float offset;   // an offset value that is always added to the returned value. Useful for extrarting isosurface
        ...
        {
            if (DiscretizeFlag)
                return VV(x, y, z).second + offset < 0 ? -1 : 1;
        ...
        field_value DistanceFromMesh(Point3f &pp, Old_Mesh * /*mesh*/)
        {
            ...
            if (f == NULL)
                return field_value(false, 0);
            if (AbsDistFlag)
                return field_value(true, dist);
            assert(!f->IsD());
            bool retIP;

            // To compute the interpolated normal we use the more robust function that require to know what is the most
            // orhogonal direction of the face.
            if ((*f).Flags() & Old_Mesh::FaceType::NORMX)
                retIP = InterpolationParameters(*f, 0, closestPt, pip);
            else if ((*f).Flags() & Old_Mesh::FaceType::NORMY)
                retIP = InterpolationParameters(*f, 1, closestPt, pip);
            else if ((*f).Flags() & Old_Mesh::FaceType::NORMZ)
                retIP = InterpolationParameters(*f, 2, closestPt, pip);
            else
                assert(0);
            ...
            int zeroCnt = 0;
            if (pip[0] < InterpolationEpsilon)
                ++zeroCnt;
            if (pip[1] < InterpolationEpsilon)
                ++zeroCnt;
            if (pip[2] < InterpolationEpsilon)
                ++zeroCnt;
            assert(zeroCnt < 3);
        ...
        field_value MultiDistanceFromMesh(Point3f &pp, Old_Mesh * /*mesh*/)
        {
            ...
            const Point3f delta[7] = { Point3f(0, 0, 0),           Point3f(0.2, -0.01, -0.02),
                                       Point3f(-0.2, 0.01, 0.02),  Point3f(0.01, 0.2, 0.01),
                                       Point3f(0.03, -0.2, -0.03), Point3f(-0.02, -0.03, 0.2),
        ...
        void ComputeSliceValues(int slice, field_value *slice_values)
        {
            for (int i = 0; i <= this->siz.X(); i++)
            {
                for (int k = 0; k <= this->siz.Z(); k++)
                {
                    ...
                    if (this->MultiSampleFlag)
                        slice_values[index] = MultiDistanceFromMesh(pp, _oldM);
                    else
                        slice_values[index] = DistanceFromMesh(pp, _oldM);
                }
            }
        ...
        */
                ...
                for (int i = 0; i <= this->siz.X(); i++)
                {
                    for (int k = 0; k <= this->siz.Z(); k++)
                    {
                        ...
                        if (slice_values[index].first)
                        {
                            ...
                            if (i > 0)
                                index_l = GetSliceIndex(i - 1, k);
                            else
                                index_l = index;
                            if (i < this->siz.X())
                                index_r = GetSliceIndex(i + 1, k);
                            else
                                index_r = index;
                            if (k > 0)
                                index_d = GetSliceIndex(i, k - 1);
                            else
                                index_d = index;
                            if (k < this->siz.Z())
                                index_u = GetSliceIndex(i, k + 1);
                            else
                                index_u = index;

                            if (slice_values[index_l].first)
                            {
                                goodCnt++;
                                if (fabs(slice_values[index_l].second - curVal) > max_dist)
                                    badCnt++;
                            }
                            if (slice_values[index_r].first)
                            {
                                goodCnt++;
                                if (fabs(slice_values[index_r].second - curVal) > max_dist)
                                    badCnt++;
                            }
                            if (slice_values[index_u].first)
                            {
                                goodCnt++;
                                if (fabs(slice_values[index_u].second - curVal) > max_dist)
                                    badCnt++;
                            }
                            if (slice_values[index_d].first)
                            {
                                goodCnt++;
                                if (fabs(slice_values[index_d].second - curVal) > max_dist)
                                    badCnt++;
                            }
        ...
        void BuildMesh(Old_Mesh &old_mesh, New_Mesh &new_mesh, EXTRACTOR_TYPE &extractor, vcg::CallBackPos *cb)
        {
            ...
            // the following two steps are required to be sure that the point-face distance without precomputed data
            // works well.
  ...
  public:
    ...
    static void Resample(Old_Mesh &old_mesh, New_Mesh &new_mesh, Box3f volumeBox, vcg::Point3<int> accuracy,
                         float max_dist, float thr = 0, bool DiscretizeFlag = false, bool MultiSampleFlag = false,
                         bool AbsDistFlag = false, vcg::CallBackPos *cb = 0)
    {
