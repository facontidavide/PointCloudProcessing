...
class UpdateCurvature
{
  ...
  public:
    ...
    */
                                       ...
                                       vcg::CallBackPos* cb = NULL)
    {
        ...
        if (pointVSfaceInt)
        {
            ...
            vcg::tri::SurfaceSampling<MeshType, vcg::tri::TrivialSampler<MeshType> >::Montecarlo(
              m, vs, 1000 * area / (2 * M_PI * r * r));
            vi = vcg::tri::Allocator<MeshType>::AddVertices(tmpM, m.vert.size());
            for (size_t y = 0; y < m.vert.size(); ++y, ++vi)
                (*vi).P() = m.vert[y].P();
        ...
        for (vi = m.vert.begin(); vi != m.vert.end(); ++vi)
        {
            ...
            if (pointVSfaceInt)
            {
                vcg::tri::GetInSphereVertex<MeshType, PointsGridType, std::vector<VertexType*>, std::vector<ScalarType>,
                                            std::vector<CoordType> >(tmpM, pGrid, (*vi).cP(), r, closests, distances,
                                                                     points);
            ...
            int best = 0;
            ScalarType bestv = fabs((*vi).cN().dot(eigenvectors.GetColumn(0).normalized()));
            for (int i = 1; i < 3; ++i)
            {
            ...
            (*vi).K1() = (2.0 / 5.0) *
                         (4.0 * M_PI * r5 + 15 * eigenvalues[(best + 2) % 3] - 45.0 * eigenvalues[(best + 1) % 3]) /
                         (M_PI * r6);
            (*vi).K2() = (2.0 / 5.0) *
                         (4.0 * M_PI * r5 + 15 * eigenvalues[(best + 1) % 3] - 45.0 * eigenvalues[(best + 2) % 3]) /
                         (M_PI * r6);
            if ((*vi).K1() < (*vi).K2())
            {
                std::swap((*vi).K1(), (*vi).K2());
    ...
    */
    static void MeanAndGaussian(MeshType& m)
    {
        ...
        for (vi = m.vert.begin(); vi != m.vert.end(); ++vi)
            if (!(*vi).IsD() /*&& !(*vi).IsB()*/)
            {
                ...
                {
                    (*vi).Kh() = (((TDContr)[*vi].dot((*vi).cN()) > 0) ? 1.0 : -1.0) *
                                 ((TDContr)[*vi] / (TDAreaPtr)[*vi].A).Norm();
    ...
    \return It will return the voronoi area around the vertex.  If (norm == true) the mean and the gaussian curvature
    are normalized.
     Based on the paper  <a href="http://www2.in.tu-clausthal.de/~hormann/papers/Dyn.2001.OTU.pdf">  <em> "Optimizing 3d
    triangulations using discrete curvature analysis" </em> </a>
      */
    ...
    static float VertexCurvature(VertexPointer v, bool norm = true)
    {
        ...
        while (!vfi.End())
        {
            if (!vfi.F()->IsD())
            {
                ...
                v->Kh() += ((math::Sqrt(s01) / 2.0) * ang1 + (math::Sqrt(s02) / 2.0) * ang2);
            }
            ...
            if (!((*vi).IsD()))
            {
                ...
                do
                {
                    if (p.F() != p.FFlip())
                    {
                        ...
                        Point3<ScalarType> n1 = p.F()->cN();
                        n1.Normalize();
                        Point3<ScalarType> n2 = p.FFlip()->cN();
                        n2.Normalize();
                ...
                int maxI = (normI + 2) % 3;
                int minI = (normI + 1) % 3;
                if (fabs(lambda[maxI]) < fabs(lambda[minI]))
                    std::swap(maxI, minI);
