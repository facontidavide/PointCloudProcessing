...
Updating of edge values id divided into 2 functions ( the first one update only a face...) added also resetting of edges
flags.. (see first line of Set function)
...
/// \brief This class is used to compute or update the precomputed data used to efficiently compute point-face
/// distances.
...
class UpdateEdges
{
  public:
    ...
    static void Set(FaceType &f)
    {
        ...
        if (nx > ny && nx > nz)
        {
            f.Flags() |= FaceType::NORMX;
            d = 1 / f.Plane().Direction()[0];
        }
        else if (ny > nz)
        {
            f.Flags() |= FaceType::NORMY;
            d = 1 / f.Plane().Direction()[1];
        }
        else
        {
            f.Flags() |= FaceType::NORMZ;
            d = 1 / f.Plane().Direction()[2];
        }
