...
/**
This class is used to compute per face or per vertex color with respect to for example Border
(UpdateColor::VertexBorderFlag), Selection (UpdateColor::FaceSelected), Quality .
*/
  ...
  public:
    ...
    static void FaceFromVertex(UpdateMeshType &m)
    {
        ...
        for (fi = m.face.begin(); fi != m.face.end(); ++fi)
            if (!(*fi).IsD())
            {
                Color4f avg = (Color4f::Construct((*fi).V(0)->C()) + Color4f::Construct((*fi).V(1)->C()) +
                               Color4f::Construct((*fi).V(2)->C())) /
                              3.0;
                (*fi).C().Import(avg);
            }
    }
    ...
    */
    static void VertexBorderFlag(UpdateMeshType &m, Color4b BorderColor = Color4b::Blue,
                                 Color4b InternalColor = Color4b::White, Color4b MixColor = Color4b::Cyan)
    {
        ...
        for (fi = m.face.begin(); fi != m.face.end(); ++fi)
            if (!(*fi).IsD())
                for (int j = 0; j < 3; ++j)
                    if ((*fi).IsB(j))
                    {
                        if ((*fi).V(j)->C() == BaseColor)
                            (*fi).V(j)->C() = BorderColor;
                        if ((*fi).V(j)->C() == InternalColor)
                            (*fi).V(j)->C() = MixColor;
                        if ((*fi).V1(j)->C() == BaseColor)
                            (*fi).V1(j)->C() = BorderColor;
                        if ((*fi).V1(j)->C() == InternalColor)
                            (*fi).V1(j)->C() = MixColor;
                    }
                    else
                    {
                        if ((*fi).V(j)->C() == BaseColor)
                            (*fi).V(j)->C() = InternalColor;
                        if ((*fi).V(j)->C() == BorderColor)
                            (*fi).V(j)->C() = MixColor;
                        if ((*fi).V1(j)->C() == BaseColor)
                            (*fi).V1(j)->C() = InternalColor;
                        if ((*fi).V1(j)->C() == BorderColor)
                            (*fi).V1(j)->C() = MixColor;
                    }
    }
    ...
    static void MultiFaceRandom(UpdateMeshType &m)
    {
        ...
        for (fi = m.face.begin(); fi != m.face.end(); ++fi)
            if (!(*fi).IsD())
            {
                id_num++;
                if ((*fi).C() == BaseColor)
                    (*fi).C() = Color4b::Scatter(50, id_num % 50, .4f, .7f);
                for (int j = 0; j < 3; ++j)
                    if ((*fi).IsF(j))
                    {
    ...
    {
        vcg::Color4b cc[7] = { vcg::Color4b::White, vcg::Color4b::Red,    vcg::Color4b::Green,  vcg::Color4b::Blue,
                               vcg::Color4b::Cyan,  vcg::Color4b::Yellow, vcg::Color4b::Magenta };
        int cnt = 0;

        typename std::vector<FacePointer>::iterator fi;
        for (fi = TStripF.begin(); fi != TStripF.end(); ++fi)
            if (*fi)
                (**fi).C().ColorRamp(0, 16, cnt);
            else
                cnt = (cnt + 1) % 16;
        //	if(*fi) (**fi).C()=cc[cnt];
        //			else cnt=(cnt+1)%7;
    ...
    static void VertexBorderManifoldFlag(UpdateMeshType &m, Color4b vn = Color4b::White, Color4b vb = Color4b::Blue,
    ...
    static int Thresholding(UpdateMeshType &m, float threshold, Color4b c1 = Color4<unsigned char>::Black,
                            Color4b c2 = Color4<unsigned char>::White, const bool ProcessSelected = false)
    {
        int counter = 0;
        VertexIterator vi;
        for (vi = m.vert.begin(); vi != m.vert.end(); ++vi)  // scan all the vertex...
    ...
    static int Brighting(UpdateMeshType &m, float amount, const bool ProcessSelected = false)
    {
        ...
        for (vi = m.vert.begin(); vi != m.vert.end(); ++vi)  // scan all the vertex...
        {
            if (!(*vi).IsD())  // if it has not been deleted...
            {
                if (!ProcessSelected || (*vi).IsS())  // if this vertex has been selected, do transormation
                {
                    (*vi).C() = Color4b(math::Clamp(int((*vi).C()[0] + amount), 0, 255),
                                        math::Clamp(int((*vi).C()[1] + amount), 0, 255),
                                        math::Clamp(int((*vi).C()[2] + amount), 0, 255), 255);
                    ++counter;
                }
            }
        }
    ...
    static int BrightnessContrast(UpdateMeshType &m, float brightness, float contrast,
    ...
    {
        return vcg::Color4b(math::Clamp((int)(ValuePow(float(c[0]) / 255, exponent) * 255), 0, 255),
                            math::Clamp((int)(ValuePow(float(c[1]) / 255, exponent) * 255), 0, 255),
                            math::Clamp((int)(ValuePow(float(c[2]) / 255, exponent) * 255), 0, 255), 255);
    }
    ...
    static int Levels(UpdateMeshType &m, float gamma, float in_min, float in_max, float out_min, float out_max,
                      unsigned char rgbMask, const bool ProcessSelected = false)
    {
        int counter = 0;
        VertexIterator vi;
        for (vi = m.vert.begin(); vi != m.vert.end(); ++vi)  // scan all the vertex...
    ...
    static Color4b ColorLevels(Color4b c, float gamma, float in_min, float in_max, float out_min, float out_max,
                               unsigned char rgbMask)
    {
        unsigned char r = c[0], g = c[1], b = c[2];
        if (rgbMask & RED_CHANNEL)
            r = ValueLevels(c[0], gamma, in_min, in_max, out_min, out_max);
        if (rgbMask & GREEN_CHANNEL)
            g = ValueLevels(c[1], gamma, in_min, in_max, out_min, out_max);
        if (rgbMask & BLUE_CHANNEL)
            b = ValueLevels(c[2], gamma, in_min, in_max, out_min, out_max);
    ...
    static int ValueLevels(int value, float gamma, float in_min, float in_max, float out_min, float out_max)
    {
        ...
        // normalize
        fvalue =
          math::Clamp<float>(fvalue - in_min, 0.0f, 1.0f) / math::Clamp<float>(in_max - in_min, 1.0f / 255.0f, 1.0f);
        // transform gamma
    ...
    static int Equalize(UpdateMeshType &m, unsigned int rgbMask, const bool ProcessSelected = false)
    {
        ...
        Hg.Clear();
        Hb.Clear();
        Hl.SetRange(0, 255, 255);
        Hr.SetRange(0, 255, 255);
        Hg.SetRange(0, 255, 255);
        Hb.SetRange(0, 255, 255);
        ...
        cdf_l[0] = Hl.BinCount(0);
        cdf_r[0] = Hr.BinCount(0);
        cdf_g[0] = Hg.BinCount(0);
        cdf_b[0] = Hb.BinCount(0);
        for (int i = 1; i < 256; i++)
        {
    ...
    static Color4b ColorEqualize(Color4b c, int cdf_l[256], int cdf_r[256], int cdf_g[256], int cdf_b[256],
                                 unsigned int rgbMask)
    {
        ...
        if (rgbMask & RED_CHANNEL)
            r = ValueEqualize(cdf_r[c[0]], cdf_r[0], cdf_r[255]);  // Equalizes red
        if (rgbMask & GREEN_CHANNEL)
            g = ValueEqualize(cdf_g[c[1]], cdf_g[0], cdf_g[255]);  // Equalizes green
        if (rgbMask & BLUE_CHANNEL)
            b = ValueEqualize(cdf_b[c[2]], cdf_b[0], cdf_b[255]);  // Equalizes blue
    ...
    static int WhiteBalance(UpdateMeshType &m, bool automatic, Color4b userColor, const bool ProcessSelected = false)
    {
        ...
        if (!automatic)
            unbalancedWhite = userColor;  // no auto regolation required, user has provided a color.
    ...
    static Color4b ColorWhiteBalance(Color4b c, Color4b unbalancedWhite)
    {
        ...
        if (unbalancedWhite[0] == 0)
            unbalancedWhite[0] = 1;
        if (unbalancedWhite[1] == 0)
            unbalancedWhite[1] = 1;
        if (unbalancedWhite[2] == 0)
            unbalancedWhite[2] = 1;

        return Color4b(math::Clamp<int>((int)(c[0] * (255.0f / unbalancedWhite[0])), 0, 255),
                       math::Clamp<int>((int)(c[1] * (255.0f / unbalancedWhite[1])), 0, 255),
                       math::Clamp<int>((int)(c[2] * (255.0f / unbalancedWhite[2])), 0, 255), 255);
    }
    ...
    static void PerlinColor(MeshType &m, Box3f bbox, float freq, Point3i channelOffsets)
    {
        ...
        for (VertexIterator vi = m.vert.begin(); vi != m.vert.end(); ++vi)
        {
            if (!(*vi).IsD())
            {
                ...
                (*vi).C() = Color4b(
                  int(255 * math::Perlin::Noise(channelOffsets[0] + p[0] * freq, channelOffsets[0] + p[1] * freq,
                                                channelOffsets[0] + p[2] * freq)),
                  int(255 * math::Perlin::Noise(channelOffsets[1] + p[0] * freq, channelOffsets[1] + p[1] * freq,
                                                channelOffsets[1] + p[2] * freq)),
                  int(255 * math::Perlin::Noise(channelOffsets[2] + p[0] * freq, channelOffsets[2] + p[1] * freq,
                                                channelOffsets[2] + p[2] * freq)),
                  255);
            }
        }
    }
    ...
    static void ColorNoise(MeshType &m, int noiseBits)
    {
        ...
        for (VertexIterator vi = m.vert.begin(); vi != m.vert.end(); ++vi)
        {
            if (!(*vi).IsD())
            {
                (*vi).C()[0] = math::Clamp<int>(
                  (*vi).C()[0] + randomGen.generate(int(2 * pow(2.0f, noiseBits))) - int(pow(2.0f, noiseBits)), 0, 255);
                (*vi).C()[1] = math::Clamp<int>(
                  (*vi).C()[1] + randomGen.generate(int(2 * pow(2.0f, noiseBits))) - int(pow(2.0f, noiseBits)), 0, 255);
                (*vi).C()[2] = math::Clamp<int>(
                  (*vi).C()[2] + randomGen.generate(int(2 * pow(2.0f, noiseBits))) - int(pow(2.0f, noiseBits)), 0, 255);
            }
        }
    }
};
