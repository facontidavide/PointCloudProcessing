...
/**
 It works according to various strategy, like geodesic distance from the border
(UpdateQuality::VertexGeodesicFromBorder) or curvature ecc.
  ...
  public:
    ...
    class VQualityHeap
    {
      public:
        ...
        inline bool operator<(const VQualityHeap &vq) const
        {
            return q > vq.q;
        }
        inline bool operator==(const VQualityHeap &vq) const
    ...
    \warning VF topology, Per Vertex Quality and border flags already computed (see UpdateFlags::FaceBorderFromVF and
    UpdateTopology::VertexFace);
    ...
    */
    ...
    /*
      Saturate the vertex quality so that for each vertex the gradient of the quality is lower than the given threshold
        ...
        while (!st.empty())
        {
            ...
            for (vvi = star.begin(); vvi != star.end(); ++vvi)
            {
                ...
                if (distGeom < fabs(qi - vc->Q()))
                {
                    ...
                    // center = 10 other=0
                    if (vc->Q() >
                        qi)  // first case: the center of the star has to be lowered (and re-inserted in the queue).
                    {
                    ...
                    else
                    {
                        ...
                        //             printf("distGeom %f, qi %f, vc->Q() %f, fabs(qi - vc->Q())
