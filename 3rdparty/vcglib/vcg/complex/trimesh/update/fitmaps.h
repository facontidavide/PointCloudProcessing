  ...
  public:
    ...
    class Bicubic
    {
      public:
        ...
        {
            return a() * u * u * u * v * v * v + b() * u * u * u * v * v + c() * u * u * u * v + d() * u * u * u +
                   e() * u * u * v * v * v + f() * u * u * v * v + g() * u * u * v + h() * u * u + i() * u * v * v * v +
                   l() * u * v * v + m() * u * v + n() * u + o() * v * v * v + p() * v * v + q() * v + r();
        }
    ...
    static bool fitBicubicPoints(VertexType* v, std::vector<CoordType>& ref, Bicubic& ret,
                                 std::vector<CoordType>& points, std::vector<VertexType*>& ring)
    {
        points.clear();
    ...
    static void computeSFitmap(MeshType& m)  //, float target = 1000)
    {
        ...
        for (VertexIterator it = m.vert.begin(); it != m.vert.end(); ++it)
        {
            ...
            for (int iteration = 0; iteration < iteraz; ++iteration)
            {
                ...
                for (typename std::vector<VertexType*>::iterator it2 = pointsGlobal.begin(); it2 != pointsGlobal.end();
                     ++it2)
                {
