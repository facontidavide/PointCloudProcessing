...
added three general-utility global functions for texture coordinates: SmoothTextureCoords, IsFoldFree, MarkFolds (see
descriptions)
...
#define v2 (f->V2(i)->T().P())
        ...
        for (FaceIterator f = Super::m.face.begin(); f != Super::m.face.end(); f++)
        {
            ...
            for (i = 0; i < 3; i++)
            {
                ScalarType a = (v1 - v0).Norm(), b = ((v1 - v0) * (v2 - v0)) / a, c = area2 / a,

                           m0 = data[f][i] / area2, m1 = data[f][(i + 1) % 3] / area2,
                  ...
                  M1 = mA + 1.0 / mA, M2 = mA - 1.0 / mA, px = e * my, py = -e * mx, qx = m1 * b + m2 * a, qy = m1 * c,
                  ...
                  dx =  // M1
                  //*M1 // ^ theta-1
                  pow(M1, theta - 1) * (px * (M1 + theta * M2) - 2.0 * qx * M1),
                           dy =  // M1
                           //*M1 // ^ theta-1
                           pow(M1, theta - 1) * (py * (M1 + theta * M2) - 2.0 * qy * M1),

                           gy = dy / c, gx = (dx - gy * b) / a;
