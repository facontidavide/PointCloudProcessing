  ...
  public:
    ...
    static std::pair<float, float> ComputePerVertexQualityMinMax(MeshType &m)  // V1.0
    {
        std::pair<float, float> minmax =
          std::make_pair(std::numeric_limits<float>::max(), -std::numeric_limits<float>::max());

        VertexIterator vi;
        for (vi = m.vert.begin(); vi != m.vert.end(); ++vi)
            if (!(*vi).IsD())
            {
                if ((*vi).Q() < minmax.first)
                    minmax.first = (*vi).Q();
                if ((*vi).Q() > minmax.second)
                    minmax.second = (*vi).Q();
            }
        return minmax;
    }
    ...
    static std::pair<float, float> ComputePerFaceQualityMinMax(MeshType &m)  // V1.0
    {
        ...
        for (fi = m.face.begin(); fi != m.face.end(); ++fi)
            if (!(*fi).IsD())
            {
                if ((*fi).Q() < minmax.first)
                    minmax.first = (*fi).Q();
                if ((*fi).Q() > minmax.second)
                    minmax.second = (*fi).Q();
            }
        return minmax;
    }
    ...
    /**
      ...
      E.g. it assume a 'empty' model where all the mass is located on the surface and compute the barycenter of that
      thinshell.
    ...
    static void ComputePerVertexQualityDistribution(MeshType &m, Distribution<float> &h,
                                                    bool selectionOnly = false)  // V1.0
    {
        ...
        for (vi = m.vert.begin(); vi != m.vert.end(); ++vi)
            ...
            {
                assert(!math::IsNAN((*vi).Q()) && "You should never try to compute Histogram with Invalid Floating "
                                                  "points numbers (NaN)");
                h.Add((*vi).Q());
            }
    }

    static void ComputePerFaceQualityDistribution(MeshType &m, Distribution<float> &h,
                                                  bool selectionOnly = false)  // V1.0
    {
        FaceIterator fi;
        for (fi = m.face.begin(); fi != m.face.end(); ++fi)
            ...
            {
                assert(!math::IsNAN((*fi).Q()) && "You should never try to compute Histogram with Invalid Floating "
                                                  "points numbers (NaN)");
                h.Add((*fi).Q());
            }
    }
