    ...
    for (fi = m.face.begin(); fi != m.face.end(); ++fi)
        for (int j = 0; j < 3; ++j)
            if (!(*fi).V(j)->IsV())  // foreach unvisited vertex we loop around it searching for creases.
            {
                ...
                if (locCreaseCounter > 0 && (!isBorderVertex))
                    newVertexCounter--;
            }

    // A questo punto ho un vettore che mi direbbe per ogni faccia quale vertice devo mettere. Dopo che ho aggiunto i
    // vertici necessari,
    ...
    for (fi = m.face.begin(); fi != m.face.end(); ++fi)
        for (int j = 0; j < 3; ++j)  // foreach unvisited vertex
        {
            ...
            if (curVertexInd < startVn)
                assert(size_t(curVertexInd) == vertInd);
            if (curVertexInd >= startVn)
            {
