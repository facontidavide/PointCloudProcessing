...
class Smooth
{
  public:
    ...
    // In the latter case 0 means that the vertex is not moved and 1 means that the vertex is moved onto the computed
    // position.
    //
    ...
    static void VertexCoordPlanarLaplacian(MeshType &m, int step, float AngleThrRad = math::ToRad(1.0),
                                           bool SmoothSelected = false, vcg::CallBackPos *cb = 0)
    {
        VertexIterator vi;
        FaceIterator fi;
        LaplacianInfo lpz(CoordType(0, 0, 0), 0);
    ...
    */
    ...
    static void VertexCoordViewDepth(MeshType &m, const CoordType &viewpoint, const ScalarType alpha, int step,
    ...
    void FaceNormalFuzzyVectorSB(MeshType &m, SimpleTempData<typename MeshType::FaceContainer, PDFaceInfo> &TD,
                                 ScalarType sigma)
    {
        int i;
    ...
    static void FaceNormalAngleThreshold(MeshType &m, SimpleTempData<typename MeshType::FaceContainer, PDFaceInfo> &TD,
                                         ScalarType sigma)
    {
        int i;
        ...
        for (fi = m.face.begin(); fi != m.face.end(); ++fi)
            if (!(*fi).IsD())
            {
                ...
                // sigma == 0 All the normals are averaged
                // sigma == 1 Nothing is averaged.
                // The averaging is weighted with the difference between normals. more similar the normal more important
                // they are.
                ...
                for (i = 0; i < 3; ++i)
                {
                    ...
                    for (; !ep.End(); ++ep)
                    {
                        if (!(*ep.f).IsV())
                        {
                            ...
                            // Note that if two faces form an angle larger than 90 deg, their contribution should be
                            // very very small.
                            ...
                            if (cosang >= sigma)
                            {
                                ...
                                normalSum += ep.f->N() * (w * w);  // similar normals have a cosang very close to 1 so
                                                                   // cosang - sigma is maximized
                            }
                            (*ep.f).SetV();
                        }
                    }
    ...
    // Nota che dovrebbe essere sempre un vettore che giace nel piano del triangolo e perpendicolare al lato opposto al
    // vertice p.
    ...
    */
    ...
    void FitMesh(MeshType &m, SimpleTempData<typename MeshType::VertContainer, PDVertInfo> &TDV,
                 SimpleTempData<typename MeshType::FaceContainer, PDFaceInfo> &TDF, float lambda)
    {
    ...
    static void FastFitMesh(MeshType &m, SimpleTempData<typename MeshType::VertContainer, PDVertInfo> &TDV,
    ...
    static void VertexCoordPasoDoble(MeshType &m, int step, typename MeshType::ScalarType Sigma = 0, int FitStep = 10,
    ...
    static void VertexCoordPasoDobleFast(MeshType &m, int NormalSmoothStep, typename MeshType::ScalarType Sigma = 0,
                                         int FitStep = 50, bool SmoothSelected = false)
    {
        PDVertInfo lpzv;
        lpzv.np = CoordType(0, 0, 0);
        PDFaceInfo lpzf;
                                              ...
                                              typename MeshType::VertexType *vp)
    {
        ...
        for (unsigned int i = 0; i < faces2.size(); i++)
            if (faces2[i])
            {
                ...
                avgn +=
                  vcg::Normal<typename MeshType::CoordType>(vertices[0]->cP(), vertices[1]->cP(), vertices[2]->cP());
                avgn +=
                  vcg::Normal<typename MeshType::CoordType>(vertices[2]->cP(), vertices[3]->cP(), vertices[0]->cP());
            }
        avgn.Normalize();
