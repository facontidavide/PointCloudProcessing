...
class Allocator
{
  public:
        ...
        if (pu.NeedUpdate())
        {
...

            {
                ...
                while (ii < m.hn - n)  // cycle on all the faces except the new ones
                {
                    if (!(*hi).IsD())
                    {
                        if (HasHNextAdjacency(m))
                            pu.Update((*hi).HNp());
                        if (HasHPrevAdjacency(m))
                            pu.Update((*hi).HPp());
                        if (HasHOppAdjacency(m))
                            pu.Update((*hi).HOp());
                        ++ii;
                    }
    ...
    */
    ...
    /** Function to add n faces to the mesh.
This is the only full featured function that is able to manage correctly all the internal pointers of the mesh (ff and
vf relations).
  ...
  public:
        ...
        if (i != m.vert_attr.end())
            if ((*i)._sizeof == sizeof(ATTR_TYPE))
            {
                if ((*i)._padding != 0)
                {
                    ...
                    std::pair<AttrIterator, bool> new_i =
                      m.vert_attr.insert(attr);  // insert the modified PointerToAttribute
                    assert(new_i.second);
                    i = new_i.first;
                }
    ...
    template <class ATTR_TYPE>
    static void DeletePerVertexAttribute(MeshType &m,
    ...
    template <class ATTR_TYPE>
    static typename MeshType::template PerEdgeAttributeHandle<ATTR_TYPE> AddPerEdgeAttribute(MeshType &m,
                                                                                             std::string name)
    {
        PAIte i;
        PointerToAttribute h;
    ...
    template <class ATTR_TYPE>
    static typename MeshType::template PerVertexAttributeHandle<ATTR_TYPE> AddPerEdgeAttribute(MeshType &m)
    {
        return AddPerEdgeAttribute<ATTR_TYPE>(m, std::string(""));
    }

    template <class ATTR_TYPE>
    static typename MeshType::template PerEdgeAttributeHandle<ATTR_TYPE> GetPerEdgeAttribute(const MeshType &m,
    ...
    template <class ATTR_TYPE>
    static typename MeshType::template PerFaceAttributeHandle<ATTR_TYPE> AddPerFaceAttribute(MeshType &m,
                                                                                             std::string name)
    {
        PAIte i;
        PointerToAttribute h;
    ...
    template <class ATTR_TYPE>
    static typename MeshType::template PerFaceAttributeHandle<ATTR_TYPE> GetPerFaceAttribute(MeshType &m,
                                                                                             const std::string &name)
    {
        assert(!name.empty());
        PointerToAttribute h1;
        ...
        if (i != m.face_attr.end())
            if ((*i)._sizeof == sizeof(ATTR_TYPE))
            {
                if ((*i)._padding != 0)
                {
                    ...
                    std::pair<AttrIterator, bool> new_i =
                      m.face_attr.insert(attr);  // insert the modified PointerToAttribute
                    assert(new_i.second);
                    i = new_i.first;
                }
    ...
    template <class ATTR_TYPE>
    static typename MeshType::template PerMeshAttributeHandle<ATTR_TYPE> AddPerMeshAttribute(MeshType &m,
                                                                                             std::string name)
    {
        PAIte i;
        PointerToAttribute h;
    ...
    template <class ATTR_TYPE>
    static typename MeshType::template PerMeshAttributeHandle<ATTR_TYPE> GetPerMeshAttribute(MeshType &m,
                                                                                             const std::string &name)
    {
        assert(!name.empty());
        PointerToAttribute h1;
        ...
        if (i != m.mesh_attr.end())
            if ((*i)._sizeof == sizeof(ATTR_TYPE))
            {
                if ((*i)._padding != 0)
                {
                    ...
                    std::pair<AttrIterator, bool> new_i =
                      m.mesh_attr.insert(attr);  // insert the modified PointerToAttribute
                    assert(new_i.second);
                    i = new_i.first;
                }
    ...
    static void FixPaddedPerVertexAttribute(MeshType &m, PointerToAttribute &pa)
    {
        ...
        for (unsigned int i = 0; i < m.vert.size(); ++i)
        {
            ...
            memcpy((void *)dest, (void *)&(ptr[i * pa._sizeof]), sizeof(ATTR_TYPE));
        }
    ...
    */
    ...
    struct NameTypeBound : public NameTypeBound_Base
    {
        ...
        {
            Allocator::template AddPerVertexAttribute<TYPE>(m, _name);
        }
        void AddPerFaceAttribute(MeshType &m)
        {
            Allocator::template AddPerFaceAttribute<TYPE>(m, _name);
        }
        void AddPerEdgeAttribute(MeshType &m)
        {
            Allocator::template AddPerEdgeAttribute<TYPE>(m, _name);
        }
        void AddPerMeshAttribute(MeshType &m)
        {
            Allocator::template AddPerMeshAttribute<TYPE>(m, _name);
        }
    ...
    {
        return (binders.find(name) != binders.end());
    }
    ...
    static void AddNameTypeBound(NameTypeScope &binders, std::string name)
    {
        ...
        if (bi != binders.end())
            assert(typeid(TYPE).name() ==
                   ((*bi).second)->TypeID());  // the name was previously bound to a dirrefent type
        else
        {
