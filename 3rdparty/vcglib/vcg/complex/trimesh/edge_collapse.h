  ...
  public:
    ...
    void FindSets(EdgeType &p)
    {
        ...
        for (x.f = v0->VFp(), x.z = v0->VFi(); x.f != 0; ++x)
        {
            ...
            if (zv1 == -1)
                AV0().push_back(x);  // la faccia x.f non ha il vertice v1 => e' incidente solo in v0
            else
                AV01().push_back(x);
        }
        ...
        for (x.f = v1->VFp(), x.z = v1->VFi(); x.f != 0; ++x)
        {
            ...
            if (zv0 == -1)
                AV1().push_back(x);  // la faccia x.f non ha il vertice v1 => e' incidente solo in v0
        }
    }
    /*
    ...
    */
    bool LinkConditions(EdgeType pos)
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            for (; !vfi.End(); ++vfi)
            {
                ...
                ++VertCnt[vfi.V2()];
                if (vfi.V1() < vfi.V2())
                    ++EdgeCnt[std::make_pair(vfi.V1(), vfi.V2())];
                else
                    ++EdgeCnt[std::make_pair(vfi.V2(), vfi.V1())];
            }
        ...
        {
            if (vfi.V1() == pos.V(1))
                LkEdge.push_back(vfi.V2());
            if (vfi.V2() == pos.V(1))
                LkEdge.push_back(vfi.V1());
        }
    ...
    bool LinkConditionsOld(EdgeType pos)
    {
        ...
        for (x.f = pos.V(1)->VFp(), x.z = pos.V(1)->VFi(); x.f != 0; ++x)
        {
            if (x.f->V1(x.z) == pos.V(0))
                x.f->V2(x.z)->Flags() |= ADJ_E | ADJ_1;
            else
                x.f->V2(x.z)->Flags() |= ADJ_1;
            if (x.f->V2(x.z) == pos.V(0))
                x.f->V1(x.z)->Flags() |= ADJ_E | ADJ_1;
            else
                x.f->V1(x.z)->Flags() |= ADJ_1;
        }
        ...
        for (x.f = pos.V(0)->VFp(), x.z = pos.V(0)->VFi(); x.f != 0; ++x)
        {
            ...
            {
                x.f->V1(x.z)->SetV();
                if (x.f->V1(x.z)->Flags() & ADJ_1)
                    ++adj01;
                if (x.f->V1(x.z)->Flags() & ADJ_E)
                    ++adje;
            }
            if (!x.f->V2(x.z)->IsV())
            {
                x.f->V2(x.z)->SetV();
                if (x.f->V2(x.z)->Flags() & ADJ_1)
                    ++adj01;
                if (x.f->V2(x.z)->Flags() & ADJ_E)
                    ++adje;
            }
        }
    ...
    int DoCollapse(TriMeshType &m, EdgeType &c, const Point3<ScalarType> &p)
    {
        ...
        for (i = AV0().begin(); i != AV0().end(); ++i)
        {
            ...
            (*i).f->VFp((*i).z) =
              (*i).f->V((*i).z)->VFp();  // e appendo la lista di facce incidenti in v1 a questa faccia
