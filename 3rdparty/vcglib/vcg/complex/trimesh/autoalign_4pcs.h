...
class FourPCS
{
  public:
    ...
    class PUsedTypes
      : public vcg::UsedTypes<vcg::Use<PVertex>::template AsVertexType, vcg::Use<PFace>::template AsFaceType>
    {
    };
  ...
  private:
    ...
    struct CandiType
    {
        ...
        inline bool operator<(const CandiType &o) const
        {
...
void FourPCS<MeshType>::Init(MeshType &_P, MeshType &_Q)
{
    ...
    for (int i = 0; i < 100; ++i)
    {
        ...
        vcg::tri::GetKClosestVertex<MeshType, vcg::GridStaticPtr<typename MeshType::VertexType, ScalarType>,
                                    std::vector<VertexType *>, std::vector<ScalarType>, std::vector<CoordType> >(
          *Q, ugridQ, 2, Q->vert[ri].cP(), Q->bbox.Diag(), ress, dists, samples);
        assert(ress.size() == 2);
        avD += dists[1];
    }
...
bool FourPCS<MeshType>::SelectCoplanarBase()
{
    ...
    int best = -1;
    ScalarType bestv = std::numeric_limits<float>::max();
    for (i = 0; i < P->vert.size(); ++i)
    {
    ...
    vcg::tri::GetInSphereVertex<MeshType, vcg::GridStaticPtr<typename MeshType::VertexType, ScalarType>,
                                std::vector<typename MeshType::VertexType *>, std::vector<ScalarType>,
                                std::vector<CoordType> >(*P, ugridP, B4, radius, closests, distances, points);
    ...
    for (int i = 0; i < 4; ++i)
    {
        vcg::tri::GetKClosestVertex<MeshType, vcg::GridStaticPtr<typename MeshType::VertexType, ScalarType>,
                                    std::vector<VertexType *>, std::vector<ScalarType>, std::vector<CoordType> >(
          *P, ugridP, prs.feetsize, B[i], radius, ExtB[i], dists, samples);
    }
...
bool FourPCS<MeshType>::IsTransfCongruent(FourPoints fp, vcg::Matrix44<ScalarType> &mat, float &trerr)
{
    ...
    ScalarType err = 0.0;
    for (int i = 0; i < 4; ++i)
        err += (mat * mov[i] - fix[i]).SquaredNorm();
...
template <class MeshType>
void FourPCS<MeshType>::ComputeR1R2(ScalarType d1, ScalarType d2)
{
    int vi, vj;
    R1.clear();
    // R2.clear();
    int start = clock();
    for (vi = 0; vi < mapsub.size(); ++vi)
        for (vj = vi; vj < mapsub.size(); ++vj)
        {
...
template <class MeshType>
bool FourPCS<MeshType>::FindCongruent()
{  // of base B, on Q, with approximation delta
    bool done = false;
    ...
    bR1 = std::lower_bound<typename std::vector<Couple>::iterator, Couple>(R1.begin(), R1.end(),
                                                                           Couple(d1 - prs.delta * 2.0));
    eR1 = std::lower_bound<typename std::vector<Couple>::iterator, Couple>(R1.begin(), R1.end(),
                                                                           Couple(d1 + prs.delta * 2.0));
    bR2 = std::lower_bound<typename std::vector<Couple>::iterator, Couple>(R1.begin(), R1.end(),
                                                                           Couple(d2 - prs.delta * 2.0));
    eR2 = std::lower_bound<typename std::vector<Couple>::iterator, Couple>(R1.begin(), R1.end(),
                                                                           Couple(d2 + prs.delta * 2.0));
    ...
    if (bR1 == R1.end())
        return false;  // if there are no such pairs return
    if (bR2 == R1.end())
        return false;  // if there are no such pairs return
    ...
    typename PMesh::template PerVertexAttributeHandle<int> id =
      vcg::tri::Allocator<PMesh>::template AddPerVertexAttribute<int>(Invr, std::string("index"));
    i = &(*bR1) - &(*R1.begin());
    for (vii = Invr.vert.begin(); vii != Invr.vert.end(); ++vii, ++i)
        id[vii] = i;
    ...
    for (uint i = 0; i < R2inv.size(); ++i)
    {
        ...
        vcg::tri::GetInBoxVertex<PMesh, GridType, std::vector<typename PMesh::VertexType *> >(Invr, *ugrid, bb,
                                                                                              closests);
...
int FourPCS<MeshType>::EvaluateSample(CandiType &fp, CoordType &tp, CoordType &np, const float &angle)
{
    ...
    vq.P() = tp;
    vq.N() = np;
    v = vcg::tri::GetClosestVertexNormal<MeshType, vcg::GridStaticPtr<typename MeshType::VertexType, ScalarType> >(
      *Q, ugridQ, vq, radius, dist);

    if (v != 0)
        if (v->N().dot(np) - angle > 0)
            return 1;
        else
            return -1;
}
...
template <class MeshType>
void FourPCS<MeshType>::EvaluateAlignment(CandiType &fp)
{
    int n_delta_close = 0;
    for (int i = 0; i < 4; ++i)
    {
...
template <class MeshType>
void FourPCS<MeshType>::TestAlignment(CandiType &fp)
{
    radius = prs.delta;
    int n_delta_close = 0;
...
template <class MeshType>
bool FourPCS<MeshType>::Align(int L, vcg::Matrix44f &result, AACb *cb)
{  // main loop
