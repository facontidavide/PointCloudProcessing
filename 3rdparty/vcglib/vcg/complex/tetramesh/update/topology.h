...
This is class for Topology of a tetrahedralmesh.
       @param STL_VERT_CONT (Template Parameter) Specifies the type of the vertices container any the vertex type.
               @param STL_TETRA_CONT (Template Parameter) Specifies the type of the tetrahedrons container any the
tetrahedrons type.
*/
...
class UpdateTopologyBase
{
  ...
  public:
    ...
    **/
    ...
    {
        for (VertexIterator v = vert.begin(); v != vert.end(); ++v)
        {
            v->VTp() = 0;
            v->VTi() = 0;
        }
  ...
  public:
    ...
    static bool IsExternEdge(TetraType *t, int edge)
    {
        ...
        if (t->HasTTAdjacency())
        {
            ...
            while ((!pl.LoopEnd()) && (!pl.T()->IsBorderF(Tetra::FofE(pl.E(), 0))) &&
                   (!pl.T()->IsBorderF(Tetra::FofE(pl.E(), 1))))
                pl.NextT();
            if (pl.LoopEnd())
                return false;
            else
...
class UpdateTopology
  : public UpdateTopologyBase<typename TetraMeshType::VertexContainer, typename TetraMeshType::TetraContainer>
{
  public:
    static void TTTopology(TetraMeshType &tmesh)
    {
        UpdateTopologyBase<typename TetraMeshType::VertexContainer, typename TetraMeshType::TetraContainer>::TTTopology(
          tmesh.vert, tmesh.tetra);
    }
    static void VTTopology(TetraMeshType &tmesh)
    {
        UpdateTopologyBase<typename TetraMeshType::VertexContainer, typename TetraMeshType::TetraContainer>::VTTopology(
          tmesh.vert, tmesh.tetra);
    }
};
