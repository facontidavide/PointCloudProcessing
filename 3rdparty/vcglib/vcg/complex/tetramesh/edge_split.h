  ...
  private:
    ...
    static int _GetMapVertEdgeRot(const int &indexE, const int &indexV)
    {
        static int mapvertedgerot[12][4] = {
            { 0, 3, 1, 2 }, { 0, 1, 2, 3 }, { 0, 2, 3, 1 }, { 1, 3, 2, 0 }, { 1, 0, 3, 2 }, { 2, 1, 3, 0 },

            { 1, 2, 0, 3 }, { 2, 3, 0, 1 }, { 3, 1, 0, 2 }, { 2, 0, 1, 3 }, { 3, 2, 1, 0 }, { 3, 0, 2, 1 },
        };
    ...
    static int _GetMapFaceEdgeRot(const int &indexE, const int &indexF)
    {
        static int mapfaceedgerot[12][4] = {
            { 1, 2, 0, 3 }, { 0, 1, 2, 3 }, { 2, 0, 1, 3 }, { 3, 1, 0, 2 }, { 1, 0, 3, 2 }, { 3, 0, 2, 1 },

            { 0, 3, 1, 2 }, { 2, 3, 0, 1 }, { 1, 3, 2, 0 }, { 0, 2, 3, 1 }, { 3, 2, 1, 0 }, { 2, 1, 3, 0 },
        };
    ...
    {
        static int mapfaceedgerot[12][4] = {
            { 2, 0, -1, -1 }, { 0, -1, 2, -1 }, { -1, 2, 0, -1 }, { 2, -1, -1, 0 }, { -1, 0, -1, 2 }, { -1, -1, 2, 0 },

            { 0, 2, -1, -1 }, { 2, -1, 0, -1 }, { -1, 0, 2, -1 }, { 0, -1, -1, 2 }, { -1, 2, -1, 0 }, { -1, -1, 0, 2 },
        };
...
// end asserts
    ...
    void _AddNewTetrahedrons(TetraMeshType &tm, PosType pos, VertexType *vnew)
    {
        ...
        if (!(pos.Jump()) && (direction == 0) && (firsttp0 != NULL) && (firsttp1 != NULL) && (oldtp0 != NULL) &&
            (oldtp1 != NULL))
        {
            _ConnectTTTopology(oldtp0, 0, firsttp0, 2);
            _ConnectTTTopology(oldtp1, 0, firsttp1, 2);
        }
        else if (!(pos.Jump()) && (direction == 2) && (firsttp0 != NULL) && (firsttp1 != NULL) && (oldtp0 != NULL) &&
                 (oldtp1 != NULL))
        {
