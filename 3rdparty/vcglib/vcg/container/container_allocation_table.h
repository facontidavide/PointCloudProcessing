...
class CATEntry : public CATBase<STL_CONT>
{
  ...
  protected:
    ...
    static typename std::list<ENTRY_TYPE>::iterator &Curr()
    {  // container that was last accessed
  ...
  public:
    static int &Id()
    {                       // unique identifier of the istance
...
template <typename STL_CONT, class ENTRY_TYPE>
void CATEntry<STL_CONT, ENTRY_TYPE>::Resort(ValueType *old_start, ValueType *new_start)
{
    AT().sort();
    UTD() = false;
}
...
template <typename STL_CONT, class ENTRY_TYPE>
ENTRY_TYPE &CATEntry<STL_CONT, ENTRY_TYPE>::GetEntry(typename STL_CONT::value_type *pt)
{
    Update(pt);
    return *Curr();
}
...
class CAT : public CATEntry<STL_CONT, EntryCAT<STL_CONT, ATTR_TYPE> >
{
  ...
  public:
    ...
    {
        static CAT<STL_CONT, ATTR_TYPE> *instance = NULL;
        return instance;
    }
};
//---------------------- CAT: implementation---------------------------------------------------
...
struct Env
{
    ...
    {
        static std::map<std::string, CATBaseBase *> ntb;
        return ntb;
    }
    ...
    static CAT<TYPE_1, TYPE_2> *newCAT()
    {
        ...
        if (ti == TypeNameBounds().end())
        {
            ...
            (CAT<TYPE_1, TYPE_2> *)(*ti).second;
    }
};

template <typename STL_CONT, class ATTR_TYPE>
CAT<STL_CONT, ATTR_TYPE> *CAT<STL_CONT, ATTR_TYPE>::New()
{
    if (Instance() == NULL)
    {
