...
#define DECL_GSSVX(NAMESPACE, FNAME, FLOATTYPE, KEYTYPE)                                                               \
    ...
    {                                                                                                                  \
        ...
        NAMESPACE::FNAME(options, A, perm_c, perm_r, etree, equed, R, C, L, U, work, lwork, B, X, recip_pivot_growth,  \
...
class SparseLU<MatrixType, SuperLU> : public SparseLU<MatrixType>
{
  ...
  public:
    SparseLU(int flags = NaturalOrdering) : Base(flags)
    {
    }

    SparseLU(const MatrixType &matrix, int flags = NaturalOrdering) : Base(flags)
    {
        compute(matrix);
    }
...
void SparseLU<MatrixType, SuperLU>::compute(const MatrixType &a)
{
    ...
    switch (Base::orderingMethod())
    {
        ...
        case ColApproxMinimumDegree:
            ...
            break;
        default:
            std::cerr << "Eigen: ordering method \"" << Base::orderingMethod() << "\" not supported by the SuperLU "
                                                                                  "backend\n";
            m_sluOptions.ColPerm = NATURAL;
    };
    ...
    SuperLU_gssvx(&m_sluOptions, &m_sluA, m_q.data(), m_p.data(), &m_sluEtree[0], &m_sluEqued, &m_sluRscale[0],
                  &m_sluCscale[0], &m_sluL, &m_sluU, NULL, 0, &m_sluB, &m_sluX, &recip_pivot_gross, &rcond, &ferr,
...
bool SparseLU<MatrixType, SuperLU>::solve(const MatrixBase<BDerived> &b, MatrixBase<XDerived> *x) const
{
    ...
    RealScalar recip_pivot_gross, rcond;
    SuperLU_gssvx(&m_sluOptions, &m_sluA, m_q.data(), m_p.data(), &m_sluEtree[0], &m_sluEqued, &m_sluRscale[0],
                  &m_sluCscale[0], &m_sluL, &m_sluU, NULL, 0, &m_sluB, &m_sluX, &recip_pivot_gross, &rcond,
