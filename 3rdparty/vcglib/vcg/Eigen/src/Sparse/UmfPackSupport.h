...
class SparseLU<MatrixType, UmfPack> : public SparseLU<MatrixType>
{
  ...
  public:
    SparseLU(int flags = NaturalOrdering) : Base(flags), m_numeric(0)
    {
    }

    SparseLU(const MatrixType &matrix, int flags = NaturalOrdering) : Base(flags), m_numeric(0)
    {
        compute(matrix);
    }
...
void SparseLU<MatrixType, UmfPack>::compute(const MatrixType &a)
{
    ...
    void *symbolic;
    int errorCode = 0;
    errorCode = umfpack_symbolic(rows, cols, a._outerIndexPtr(), a._innerIndexPtr(), a._valuePtr(), &symbolic, 0, 0);
    if (errorCode == 0)
        errorCode = umfpack_numeric(a._outerIndexPtr(), a._innerIndexPtr(), a._valuePtr(), symbolic, &m_numeric, 0, 0);
...
void SparseLU<MatrixType, UmfPack>::extractData() const
{
    if (m_extractedDataAreDirty)
    {
        ...
        // extract
        umfpack_get_numeric(m_l._outerIndexPtr(), m_l._innerIndexPtr(), m_l._valuePtr(), m_u._outerIndexPtr(),
                            m_u._innerIndexPtr(), m_u._valuePtr(), m_p.data(), m_q.data(), 0, 0, 0, m_numeric);
...
bool SparseLU<MatrixType, UmfPack>::solve(const MatrixBase<BDerived> &b, MatrixBase<XDerived> *x) const
{
    ...
    {
        errorCode = umfpack_solve(UMFPACK_A, m_matrixRef->_outerIndexPtr(), m_matrixRef->_innerIndexPtr(),
                                  m_matrixRef->_valuePtr(), &x->col(j).coeffRef(0),
                                  &b.const_cast_derived().col(j).coeffRef(0), m_numeric, 0, 0);
        if (errorCode != 0)
            return false;
    }
