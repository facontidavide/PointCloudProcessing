...
class SparseLLT<MatrixType, Cholmod> : public SparseLLT<MatrixType>
{
  ...
  public:
    SparseLLT(int flags = 0) : Base(flags), m_cholmodFactor(0)
    {
        cholmod_start(&m_cholmod);
    }

    SparseLLT(const MatrixType& matrix, int flags = 0) : Base(flags), m_cholmodFactor(0)
    {
        cholmod_start(&m_cholmod);
        compute(matrix);
    }
...
template <typename MatrixType>
inline const typename SparseLLT<MatrixType>::CholMatrixType& SparseLLT<MatrixType, Cholmod>::matrixL() const
{
