...
struct ei_sparse_product_mode
{
    enum
    {
        ...
        value = ((Lhs::Flags & Diagonal) == Diagonal || (Rhs::Flags & Diagonal) == Diagonal) ?
                  ...
                  (Lhs::Flags & SparseBit) == SparseBit ? SparseTimeDenseProduct : DenseTimeSparseProduct
    };
};
...
struct SparseProductReturnType<Lhs, Rhs, SparseTimeSparseProduct>
{
    ...
    typedef typename ei_meta_if<TransposeLhs, SparseMatrix<Scalar, 0>,
                                const typename ei_nested<Lhs, Rhs::RowsAtCompileTime>::type>::ret LhsNested;

    typedef typename ei_meta_if<TransposeRhs, SparseMatrix<Scalar, 0>,
...
struct ei_traits<SparseProduct<LhsNested, RhsNested, ProductMode> >
{
    ...
    typedef typename ei_meta_if<ResultIsSparse, SparseMatrixBase<SparseProduct<LhsNested, RhsNested, ProductMode> >,
                      ...
                      public ei_traits<SparseProduct<LhsNested, RhsNested, ProductMode> >::Base
{
  ...
  public:
    template <typename Lhs, typename Rhs>
    EIGEN_STRONG_INLINE SparseProduct(const Lhs& lhs, const Rhs& rhs) : m_lhs(lhs), m_rhs(rhs)
    {
...
template <typename Lhs, typename Rhs, typename ResultType, int LhsStorageOrder = ei_traits<Lhs>::Flags& RowMajorBit,
...
{
    ei_sparse_product_selector<typename ei_cleantype<Lhs>::type, typename ei_cleantype<Rhs>::type, Derived>::run(
      product.lhs(), product.rhs(), derived());
    return derived();
}
...
Derived& MatrixBase<Derived>::lazyAssign(const SparseProduct<Lhs, Rhs, DenseTimeSparseProduct>& product)
{
    ...
    enum
    {
        RhsIsRowMajor = (_Rhs::Flags & RowMajorBit) == RowMajorBit
    };
    derived().setZero();
    for (int j = 0; j < product.rhs().outerSize(); ++j)
        for (RhsInnerIterator i(product.rhs(), j); i; ++i)
            derived().col(RhsIsRowMajor ? i.index() : j) +=
              i.value() * product.lhs().col(RhsIsRowMajor ? j : i.index());
    return derived();
}
...
EIGEN_STRONG_INLINE const typename SparseProductReturnType<Derived, OtherDerived>::Type SparseMatrixBase<Derived>::
