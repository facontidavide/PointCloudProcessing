    ...
    for (k = 0; k < std::max(nct, nrt); ++k)
    {
        ...
        for (j = k + 1; j < n; ++j)
        {
            if ((k < nct) && (m_sigma[k] != 0.0))
            {
                ...
                Scalar t =
                  matA.col(k).end(m - k).dot(matA.col(j).end(m - k));  // FIXME dot product or cwise prod + .sum() ??
                t = -t / matA(k, k);
    ...
    if (wantu)
    {
        ...
        for (k = nct - 1; k >= 0; k--)
        {
            if (m_sigma[k] != 0.0)
            {
                for (j = k + 1; j < nu; ++j)
                {
                    Scalar t = m_matU.col(k).end(m - k).dot(
    ...
    if (wantv)
    {
        for (k = n - 1; k >= 0; k--)
        {
            if ((k < nrt) & (e[k] != 0.0))
            {
                for (j = k + 1; j < nu; ++j)
                {
                    Scalar t = m_matV.col(k).end(n - k - 1).dot(
        ...
        switch (kase)
        {
            ...
            case 3:
            {
                ...
                Scalar scale = std::max(
                  std::max(std::max(std::max(ei_abs(m_sigma[p - 1]), ei_abs(m_sigma[p - 2])), ei_abs(e[p - 2])),
                           ei_abs(m_sigma[k])),
                  ei_abs(e[k]));
...
inline SVD<typename MatrixBase<Derived>::PlainMatrixType> MatrixBase<Derived>::svd() const
{
