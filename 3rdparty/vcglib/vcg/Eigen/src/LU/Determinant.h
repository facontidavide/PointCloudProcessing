...
inline const typename Derived::Scalar ei_bruteforce_det3_helper(const MatrixBase<Derived>& matrix, int a, int b, int c)
{
    return matrix.coeff(0, a) * (matrix.coeff(1, b) * matrix.coeff(2, c) - matrix.coeff(1, c) * matrix.coeff(2, b));
}

template <typename Derived>
const typename Derived::Scalar ei_bruteforce_det4_helper(const MatrixBase<Derived>& matrix, int j, int k, int m, int n)
{
...
template <typename Derived, int DeterminantType = (Derived::Flags & (UpperTriangularBit | LowerTriangularBit)) ?
                                                    TriangularDeterminant :
...
struct ei_determinant_impl<Derived, 3>
{
    ...
    {
        return ei_bruteforce_det3_helper(m, 0, 1, 2) - ei_bruteforce_det3_helper(m, 1, 0, 2) +
...
struct ei_determinant_impl<Derived, 4>
{
    static typename ei_traits<Derived>::Scalar run(const Derived& m)
    {
        ...
        return ei_bruteforce_det4_helper(m, 0, 1, 2, 3) - ei_bruteforce_det4_helper(m, 0, 2, 1, 3) +
               ei_bruteforce_det4_helper(m, 0, 3, 1, 2) + ei_bruteforce_det4_helper(m, 1, 2, 0, 3) -
               ei_bruteforce_det4_helper(m, 1, 3, 0, 2) + ei_bruteforce_det4_helper(m, 2, 3, 0, 1);
    }
};
