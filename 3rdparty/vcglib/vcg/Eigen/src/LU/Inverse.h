...
void ei_compute_inverse_in_size3_case(const MatrixType& matrix, MatrixType* result)
{
    ...
    const Scalar invdet = Scalar(1) / (det_minor00 * matrix.coeff(0, 0) - det_minor10 * matrix.coeff(1, 0) +
...
void ei_compute_inverse_in_size4_case(const MatrixType& matrix, MatrixType* result)
{
    ...
    else
    {
        ...
        else
        {
            ...
            int swap0with =
              ei_abs(m.coeff(0, 0)) + ei_abs(m.coeff(0, 1)) > ei_abs(m.coeff(1, 0)) + ei_abs(m.coeff(1, 1)) ? 0 : 1;
            m.row(0).swap(m.row(swap0with));
            // swap row 1 with the the row among 2 and 3 that has the biggest 2 first coeffs
            int swap1with =
              ei_abs(m.coeff(2, 0)) + ei_abs(m.coeff(2, 1)) > ei_abs(m.coeff(3, 0)) + ei_abs(m.coeff(3, 1)) ? 2 : 3;
