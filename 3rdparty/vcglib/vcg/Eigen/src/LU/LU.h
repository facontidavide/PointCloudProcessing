...
class LU
{
  public:
    ...
    enum
    {
        MaxSmallDimAtCompileTime = EIGEN_ENUM_MIN(MatrixType::MaxColsAtCompileTime, MatrixType::MaxRowsAtCompileTime)
    };

    typedef Matrix<typename MatrixType::Scalar,
                   MatrixType::ColsAtCompileTime,  // the number of rows in the "kernel matrix" is the number of cols of
                                                   // the original matrix
                                                   // so that the product "matrix * kernel = zero" makes sense
                   Dynamic,                        // we don't know at compile-time the dimension of the kernel
                   MatrixType::Options,
                   MatrixType::MaxColsAtCompileTime,  // see explanation for 2nd template parameter
                   MatrixType::MaxColsAtCompileTime  // the kernel is a subspace of the domain space, whose dimension is
                                                     // the number
...
LU<MatrixType>::LU(const MatrixType& matrix)
    : m_originalMatrix(matrix), m_lu(matrix), m_p(matrix.rows()), m_q(matrix.cols())
{
...
void LU<MatrixType>::computeKernel(KernelMatrixType* result) const
{
    ...
    Matrix<Scalar, Dynamic, Dynamic, MatrixType::Options, MatrixType::MaxColsAtCompileTime,
           MatrixType::MaxColsAtCompileTime>
      y(-m_lu.corner(TopRight, m_rank, dimker));

    m_lu.corner(TopLeft, m_rank, m_rank).template marked<UpperTriangular>().solveTriangularInPlace(y);
...
template <typename MatrixType>
const typename LU<MatrixType>::KernelResultType LU<MatrixType>::kernel() const
{
...
template <typename MatrixType>
const typename LU<MatrixType>::ImageResultType LU<MatrixType>::image() const
{
...
template <typename MatrixType>
template <typename OtherDerived, typename ResultType>
bool LU<MatrixType>::solve(const MatrixBase<OtherDerived>& b, ResultType* result) const
{
    ...
    m_lu.corner(Eigen::TopLeft, smalldim, smalldim)
      .template marked<UnitLowerTriangular>()
      .solveTriangularInPlace(c.corner(Eigen::TopLeft, smalldim, c.cols()));
    if (rows > cols)
    {
...
inline const LU<typename MatrixBase<Derived>::PlainMatrixType> MatrixBase<Derived>::lu() const
{
