...
MatrixBase<Derived>::cross(const MatrixBase<OtherDerived>& other) const
{
    ...
    return typename ei_plain_matrix_type<Derived>::type(lhs.coeff(1) * rhs.coeff(2) - lhs.coeff(2) * rhs.coeff(1),
...
struct ei_unitOrthogonal_selector
{
    ...
    inline static VectorType run(const Derived& src)
    {
         ...
         */
        if ((!ei_isMuchSmallerThan(src.x(), src.z())) || (!ei_isMuchSmallerThan(src.y(), src.z())))
        {
...
/** \returns a unit vector which is orthogonal to \c *this
  ...
  */
template <typename Derived>
typename MatrixBase<Derived>::PlainMatrixType MatrixBase<Derived>::unitOrthogonal() const
{
