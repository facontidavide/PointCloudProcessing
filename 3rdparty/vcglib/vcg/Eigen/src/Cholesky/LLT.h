 ...
 * definite matrices,
  * use LDLT instead for the semidefinite case. Also, do not use a Cholesky decomposition to determine whether a system
 * of equations
  * has a solution.
  *
...
void LLT<MatrixType>::compute(const MatrixType &a)
{
    ...
    for (int j = 1; j < size; ++j)
    {
        ...
        if (endSize > 0)
        {
            ...
            m_matrix.col(j).end(endSize) = (a.row(j).end(endSize).adjoint() - m_matrix.col(j).end(endSize)) / x;
        }
    }
...
/** Computes the solution x of \f$ A x = b \f$ using the current decomposition of A.
  ...
  *
  * \returns true always! If you need to check for existence of solutions, use another decomposition like LU, QR, or
 * SVD.
  *
...
/** \cholesky_module
  ...
  */
template <typename Derived>
inline const LLT<typename MatrixBase<Derived>::PlainMatrixType> MatrixBase<Derived>::llt() const
{
