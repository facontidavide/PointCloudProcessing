...
class LDLT
{
  public:
    ...
    LDLT(const MatrixType &matrix) : m_matrix(matrix.rows(), matrix.cols())
    {
        compute(matrix);
    }
...
void LDLT<MatrixType>::compute(const MatrixType &a)
{
    ...
    {
        RealScalar tmp =
          ei_real(a.coeff(j, j) - (m_matrix.row(j).start(j) * m_matrix.col(j).start(j).conjugate()).coeff(0, 0));
        ...
        if (endSize > 0)
        {
            _temporary.end(endSize) =
              (m_matrix.block(j + 1, 0, endSize, j) * m_matrix.col(j).start(j).conjugate()).lazy();

            m_matrix.row(j).end(endSize) = a.row(j).end(endSize).conjugate() - _temporary.end(endSize).transpose();
...
template <typename MatrixType>
template <typename RhsDerived, typename ResultType>
bool LDLT<MatrixType>::solve(const MatrixBase<RhsDerived> &b, ResultType *result) const
{
...
/** \cholesky_module
  ...
  */
template <typename Derived>
inline const LDLT<typename MatrixBase<Derived>::PlainMatrixType> MatrixBase<Derived>::ldlt() const
{
    return derived();
}
