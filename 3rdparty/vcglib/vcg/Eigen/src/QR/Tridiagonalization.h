...
class Tridiagonalization
{
  public:
    ...
    enum
    {
        ...
        SizeMinusOne = MatrixType::RowsAtCompileTime == Dynamic ? Dynamic : MatrixType::RowsAtCompileTime - 1,
    ...
    typedef typename NestByValue<DiagonalCoeffs<NestByValue<Block<MatrixType, SizeMinusOne, SizeMinusOne> > > >::
    ...
    /** This constructor initializes a Tridiagonalization object for
      ...
      */
    Tridiagonalization(int size = Size == Dynamic ? 2 : Size) : m_matrix(size, size), m_hCoeffs(size - 1)
    {
    }

    Tridiagonalization(const MatrixType& matrix) : m_matrix(matrix), m_hCoeffs(matrix.cols() - 1)
    {
  ...
  private:
    ...
    static void _decomposeInPlace3x3(MatrixType& mat, DiagonalType& diag, SubDiagonalType& subdiag,
                                     bool extractQ = true);
...
const typename Tridiagonalization<MatrixType>::DiagonalReturnType Tridiagonalization<MatrixType>::diagonal(void) const
{
...
/** constructs and returns the tridiagonal matrix T.
  ...
  */
template <typename MatrixType>
typename Tridiagonalization<MatrixType>::MatrixType Tridiagonalization<MatrixType>::matrixT(void) const
{
 ...
 * using Part<LowerTriangular|Selfadjoint>  is very very slow */
            ...
            hCoeffs.end(n - i - 1) =
              (matA.corner(BottomRight, n - i - 1, n - i - 1).template part<LowerTriangular | SelfAdjoint>() *
               (h * matA.col(i).end(n - i - 1)))
                .lazy();
            // simple axpy
            hCoeffs.end(n - i - 1) +=
              (h * Scalar(-0.5) * matA.col(i).end(n - i - 1).dot(hCoeffs.end(n - i - 1))) * matA.col(i).end(n - i - 1);
            // rank-2 update
 ...
 * small matrices). The algo performs block matrix-vector and transposed matrix vector products. */
            ...
            for (int b = i + 1; b < n4; b += 4)
            {
                ...
                Block<CoeffVectorType, 4, 1>(hCoeffs, b, 0, 4, 1) +=
                  Block<MatrixType, Dynamic, 4>(matA, b + 4, b, n - b - 4, 4).adjoint() *
                  Block<MatrixType, Dynamic, 1>(matA, b + 4, i, n - b - 4, 1);
                // the 4x4 block diagonal:
                Block<CoeffVectorType, 4, 1>(hCoeffs, b, 0, 4, 1) +=
                  (Block<MatrixType, 4, 4>(matA, b, b, 4, 4).template part<LowerTriangular | SelfAdjoint>() *
                   (h * Block<MatrixType, 4, 1>(matA, b, i, 4, 1)))
                    .lazy();
            }
#endif
 ...
 * if we remove the specialization of Block for Matrix then it is even worse, much worse ! */
            ...
            hCoeffs.end(n - i - 1) =
              (matA.corner(BottomRight, n - i - 1, n - i - 1) * (h * matA.col(i).end(n - i - 1))).lazy();
            ...
            for (int j1 = i + 1; j1 < n; ++j1)
            {
                ...
                if (PacketSize > 1)
                {
                    ...
                    for (int i1 = alignedStart; i1 < alignedEnd; i1 += PacketSize)
                        ei_pstore(pc + i1, ei_psub(ei_pload(pc + i1), ei_padd(ei_pmul(tmp0, ei_ploadu(pb + i1)),
...
template <typename MatrixType>
typename Tridiagonalization<MatrixType>::MatrixType Tridiagonalization<MatrixType>::matrixQ(void) const
{
    int n = m_matrix.rows();
    ...
    for (int i = n - 2; i >= 0; i--)
    {
        ...
        matQ.corner(BottomRight, n - i - 1, n - i - 1) -=
          ((m_hCoeffs.coeff(i) * m_matrix.col(i).end(n - i - 1)) *
           (m_matrix.col(i).end(n - i - 1).adjoint() * matQ.corner(BottomRight, n - i - 1, n - i - 1)).lazy())
            .lazy();
...
template <typename MatrixType>
void Tridiagonalization<MatrixType>::decomposeInPlace(MatrixType& mat, DiagonalType& diag, SubDiagonalType& subdiag,
                                                      bool extractQ)
{
    int n = mat.rows();
...
/** \internal
  ...
  */
template <typename MatrixType>
void Tridiagonalization<MatrixType>::_decomposeInPlace3x3(MatrixType& mat, DiagonalType& diag, SubDiagonalType& subdiag,
                                                          bool extractQ)
{
