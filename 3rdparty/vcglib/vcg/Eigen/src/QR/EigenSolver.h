...
void EigenSolver<MatrixType>::orthes(MatrixType& matH, RealVectorType& ort)
{
    ...
    for (int m = low + 1; m <= high - 1; ++m)
    {
        ...
        if (scale != 0.0)
        {
            ...
            // H = (I-u*u'/h)*H*(I-u*u')/h)
            int bSize = high - m + 1;
            matH.block(m, m, bSize, n - m) -=
              ((ort.segment(m, bSize) / h) *
    ...
    for (int m = high - 1; m >= low + 1; m--)
    {
        if (matH.coeff(m, m - 1) != 0.0)
        {
            ...
            int bSize = high - m + 1;
            m_eivec.block(m, m, bSize, bSize) +=
              ((ort.segment(m, bSize) / (matH.coeff(m, m - 1) * ort.coeff(m))) *
        ...
        else  // No convergence yet
        {
            ...
            while (m >= l)
            {
                ...
                if (ei_abs(matH.coeff(m, m - 1)) * (ei_abs(q) + ei_abs(r)) <
                    eps *
                      (ei_abs(p) * (ei_abs(matH.coeff(m - 1, m - 1)) + ei_abs(z) + ei_abs(matH.coeff(m + 1, m + 1)))))
                {
                    break;
                }
                m--;
            ...
            for (int i = n - 1; i >= 0; i--)
            {
                ...
                else
                {
                    ...
                    else  // Solve real equations
                    {
                        x = matH.coeff(i, i + 1);
                        y = matH.coeff(i + 1, i);
                        q = (m_eivalues.coeff(i).real() - p) * (m_eivalues.coeff(i).real() - p) +
            ...
            for (int i = n - 2; i >= 0; i--)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        x = matH.coeff(i, i + 1);
                        y = matH.coeff(i + 1, i);
                        vr = (m_eivalues.coeff(i).real() - p) * (m_eivalues.coeff(i).real() - p) +
