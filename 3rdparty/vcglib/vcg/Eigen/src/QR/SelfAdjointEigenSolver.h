...
class SelfAdjointEigenSolver
{
  public:
    enum
    {
        Size = _MatrixType::RowsAtCompileTime
    };
    ...
    SelfAdjointEigenSolver() : m_eivec(int(Size), int(Size)), m_eivalues(int(Size))
    {
        ei_assert(Size != Dynamic);
    }

    SelfAdjointEigenSolver(int size) : m_eivec(size, size), m_eivalues(size)
    {
    }
    ...
    SelfAdjointEigenSolver(const MatrixType& matA, const MatrixType& matB, bool computeEigenvectors = true)
        : m_eivec(matA.rows(), matA.cols()), m_eivalues(matA.cols())
    {
...
void SelfAdjointEigenSolver<MatrixType>::compute(const MatrixType& matrix, bool computeEigenvectors)
{
    ...
    while (end > 0)
    {
        ...
        ei_tridiagonal_qr_step(diag.data(), subdiag.data(), start, end,
...
/** Computes the eigenvalues of the generalized eigen problem
  ...
  */
template <typename MatrixType>
void SelfAdjointEigenSolver<MatrixType>::compute(const MatrixType& matA, const MatrixType& matB,
...
template <typename Derived>
struct ei_operatorNorm_selector<Derived, false>
{
    static inline typename NumTraits<typename ei_traits<Derived>::Scalar>::Real
    operatorNorm(const MatrixBase<Derived>& m)
    {
        ...
        // FIXME if it is really guaranteed that the eigenvalues are already sorted,
        // then we don't need to compute a maxCoeff() here, comparing the 1st and last ones is enough.
        return ei_sqrt((m_eval * m_eval.adjoint()).template marked<SelfAdjoint>().eigenvalues().maxCoeff());
    }
};
...
/** \qr_module
  ...
  */
template <typename Derived>
inline typename NumTraits<typename ei_traits<Derived>::Scalar>::Real MatrixBase<Derived>::operatorNorm() const
{
    return ei_operatorNorm_selector<Derived, Flags & SelfAdjointBit>::operatorNorm(derived());
}
