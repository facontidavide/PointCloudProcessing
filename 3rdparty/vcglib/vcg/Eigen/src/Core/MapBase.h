...
class MapBase : public MatrixBase<Derived>
{
  public:
    ...
    template <bool IsForceAligned, typename Dummy>
    ...
    {
        return ei_ploadt < Scalar, int(PacketAccess) == ForceAligned ?
                                     Aligned :
                                     LoadMode > (m_data + (IsRowMajor ? col + row * stride() : row + col * stride()));
    }
    ...
    inline void writePacket(int row, int col, const PacketScalar& x)
    {
        ei_pstoret<Scalar, PacketScalar, int(PacketAccess) == ForceAligned ? Aligned : StoreMode>(
          const_cast<Scalar*>(m_data) + (IsRowMajor ? col + row * stride() : row + col * stride()), x);
    }
    ...
    inline MapBase(const Scalar* data, int rows, int cols) : m_data(data), m_rows(rows), m_cols(cols)
    {
        ei_assert((data == 0) || (rows > 0 && (RowsAtCompileTime == Dynamic || RowsAtCompileTime == rows) && cols > 0 &&
