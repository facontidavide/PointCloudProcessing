...
struct ei_functor_traits<ei_scalar_cast_op<Scalar, NewType> >
{
    enum
    {
        Cost = ei_is_same_type<Scalar, NewType>::ret ? 0 : NumTraits<NewType>::AddCost,
        PacketAccess = false
    };
};
...
template <typename Scalar>
struct ei_scalar_identity_op EIGEN_EMPTY_STRUCT
{
...
// this macro is really needed because ei_functor_traits must be specialized after it is declared but before it is
// used...
...
struct ei_functor_has_linear_access
{
    enum
    {
        ret = 1
...
template <typename Scalar>
struct ei_functor_has_linear_access<ei_scalar_identity_op<Scalar> >
{
    enum
    {
        ret = 0
...
template <typename Functor>
struct ei_functor_allows_mixing_real_and_complex
{
    enum
    {
        ret = 0
...
template <typename Scalar>
struct ei_functor_allows_mixing_real_and_complex<ei_scalar_product_op<Scalar> >
{
    enum
    {
        ret = 1
