...
template <typename XprType, unsigned int Mode>
...
template <typename Lhs, typename Rhs,
          int TriangularPart = (int(Lhs::Flags) & LowerTriangularBit) ?
                                 LowerTriangular :
                                 (int(Lhs::Flags) & UpperTriangularBit) ? UpperTriangular : -1,
...
struct ei_solve_triangular_selector<Lhs, Rhs, UpLo, RowMajor | IsDense>
{
    ...
    static void run(const Lhs& lhs, Rhs& other)
    {
        ...
        for (int c = 0; c < other.cols(); ++c)
        {
            ...
            for (int i = (IsLowerTriangular ? 1 : size - 2); IsLowerTriangular ? i < blockyStart : i > blockyStart;
                 i += (IsLowerTriangular ? 1 : -1))
            {
                Scalar tmp =
                  other.coeff(i, c) - (IsLowerTriangular ?
                                         ((lhs.row(i).start(i)) * other.col(c).start(i)).coeff(0, 0) :
            ...
            for (int i = blockyStart; IsLowerTriangular ? i < size : i > 0;)
            {
                ...
                for (; IsLowerTriangular ? i < endBlock : i > endBlock; i += IsLowerTriangular ? 1 : -1)
                {
                    ...
                    Scalar tmp = other.coeff(i, c) - btmp.coeff(IsLowerTriangular ? remainingSize : 3 - remainingSize) -
                                 (lhs.row(i).segment(IsLowerTriangular ? startBlock : i + 1, remainingSize) *
                                  other.col(c).segment(IsLowerTriangular ? startBlock : i + 1, remainingSize))
                                   .coeff(0, 0);
    ...
    static void run(const Lhs& lhs, Rhs& other)
    {
        ...
        for (int c = 0; c < other.cols(); ++c)
        {
            ...
            for (int i = IsLowerTriangular ? 0 : size - 1; IsLowerTriangular ? i < blockyEnd : i > blockyEnd;)
            {
                 ...
                 *   other.col(c).end(size-endBlock) += (lhs.block(endBlock, startBlock, size-endBlock,
                 * endBlock-startBlock)
                ...
                ei_cache_friendly_product_colmajor_times_vector(
                  ...
                  &(lhs.const_cast_derived().coeffRef(IsLowerTriangular ? endBlock : 0,
                                                      IsLowerTriangular ? startBlock : endBlock + 1)),
                  lhs.stride(), btmp, &(other.coeffRef(IsLowerTriangular ? endBlock : 0, c)));
                // 				if (IsLowerTriangular)
                //           other.col(c).end(size-endBlock) += (lhs.block(endBlock, startBlock, size-endBlock,
                //           endBlock-startBlock)
                //                                           *
            ...
            for (i = blockyEnd; IsLowerTriangular ? i < size - 1 : i > 0; i += (IsLowerTriangular ? 1 : -1))
            {
                 ...
                 */
                if (IsLowerTriangular)
                    other.col(c).end(size - i - 1) -=
                      other.coeffRef(i, c) * Block<Lhs, Dynamic, 1>(lhs, i + 1, i, size - i - 1, 1);
                else
...
void MatrixBase<Derived>::solveTriangularInPlace(const MatrixBase<OtherDerived>& _other) const
{
    ...
    enum
    {
        copy = ei_traits<OtherDerived>::Flags & RowMajorBit
    };

    typedef typename ei_meta_if<copy, typename ei_plain_matrix_type_column_major<OtherDerived>::type,
                                OtherDerived&>::ret OtherCopy;
...
/** \returns the product of the inverse of \c *this with \a other, \a *this being triangular.
  ...
  * by extract().
  *
  * \addexample SolveTriangular \label How to solve a triangular system (aka. how to multiply the inverse of a
 * triangular matrix by another one)
  *
