...
struct ei_L2_block_traits
{
    enum
    {
        width = 8 * ei_meta_sqrt<L2MemorySize / (64 * sizeof(Scalar))>::ret
    };
};
...
static void ei_cache_friendly_product(int _rows, int _cols, int depth, bool _lhsRowMajor, const Scalar* _lhs,
                                      int _lhsStride, bool _rhsRowMajor, const Scalar* _rhs, int _rhsStride,
                                      bool resRowMajor, Scalar* res, int resStride)
{
    ...
    Scalar* EIGEN_RESTRICT rhsCopy = ei_aligned_stack_new(Scalar, l2BlockSizeAligned * l2BlockSizeAligned);
        ...
        for (int l2j = 0; l2j < cols; l2j += l2BlockCols)
        {
            ...
            for (int l2k = 0; l2k < size; l2k += l2BlockSize)
            {
                ...
                if (needRhsCopy)
                    ...
                    {
                        ei_internal_assert(l2BlockSizeAligned * (l1j - l2j) + (l2blockSizeEnd - l2k) <
                                           l2BlockSizeAligned * l2BlockSizeAligned);
                        memcpy(rhsCopy + l2BlockSizeAligned * (l1j - l2j), &(rhs[l1j * rhsStride + l2k]),
                               (l2blockSizeEnd - l2k) * sizeof(Scalar));
                    }
                ...
                {
                    int offsetblock =
                      l2k * (l2blockRowEnd - l2i) + (l1i - l2i) * (l2blockSizeEnd - l2k) - l2k * MaxBlockRows;
                ...
                if (l2blockRemainingRows > 0)
                {
                    int offsetblock = l2k * (l2blockRowEnd - l2i) + (l2blockRowEndBW - l2i) * (l2blockSizeEnd - l2k) -
                                      l2k * l2blockRemainingRows;
                    ...
                    for (int l1j = l2j; l1j < l2blockColEnd; l1j += 1)
                    {
                        ...
                        for (int k = l2k; k < l2blockSizeEnd; k += PacketSize)
                        {
                            ...
                            if (l2blockRemainingRows >= 2)
                                dst[1] =
                                  ei_pmadd(tmp, ei_pload(&(localB[k * l2blockRemainingRows + PacketSize])), dst[1]);
                            if (l2blockRemainingRows >= 3)
                                dst[2] =
                                  ei_pmadd(tmp, ei_pload(&(localB[k * l2blockRemainingRows + 2 * PacketSize])), dst[2]);
                            if (l2blockRemainingRows >= 4)
                                dst[3] =
                                  ei_pmadd(tmp, ei_pload(&(localB[k * l2blockRemainingRows + 3 * PacketSize])), dst[3]);
                            if (MaxBlockRows == 8)
                            {
                                if (l2blockRemainingRows >= 5)
                                    dst[4] = ei_pmadd(
                                      tmp, ei_pload(&(localB[k * l2blockRemainingRows + 4 * PacketSize])), dst[4]);
                                if (l2blockRemainingRows >= 6)
                                    dst[5] = ei_pmadd(
                                      tmp, ei_pload(&(localB[k * l2blockRemainingRows + 5 * PacketSize])), dst[5]);
                                if (l2blockRemainingRows >= 7)
                                    dst[6] = ei_pmadd(
                                      tmp, ei_pload(&(localB[k * l2blockRemainingRows + 6 * PacketSize])), dst[6]);
                                if (l2blockRemainingRows >= 8)
                                    dst[7] = ei_pmadd(
                                      tmp, ei_pload(&(localB[k * l2blockRemainingRows + 7 * PacketSize])), dst[7]);
                            }
                        }
...
static EIGEN_DONT_INLINE void ei_cache_friendly_product_colmajor_times_vector(int size, const Scalar* lhs,
                                                                              int lhsStride, const RhsType& rhs,
                                                                              Scalar* res)
{
...
#define _EIGEN_ACCUMULATE_PACKETS(A0, A13, A2)                                                                         \
    ei_pstore(&res[j],                                                                                                 \
              ei_padd(ei_pload(&res[j]), ei_padd(ei_padd(ei_pmul(ptmp0, EIGEN_CAT(ei_ploa, A0)(&lhs0[j])),             \
    ...
    int alignmentPattern =
      alignmentStep == 0 ? AllAligned : alignmentStep == (PacketSize / 2) ? EvenAligned : FirstAligned;
    ...
    {
        Packet ptmp0 = ei_pset1(rhs[i]), ptmp1 = ei_pset1(rhs[i + offset1]), ptmp2 = ei_pset1(rhs[i + 2]),
               ptmp3 = ei_pset1(rhs[i + offset3]);
        ...
        if (PacketSize > 1)
        {
            ...
            for (int j = 0; j < alignedStart; ++j)
                res[j] += ei_pfirst(ptmp0) * lhs0[j] + ei_pfirst(ptmp1) * lhs1[j] + ei_pfirst(ptmp2) * lhs2[j] +
                          ei_pfirst(ptmp3) * lhs3[j];
...
static EIGEN_DONT_INLINE void ei_cache_friendly_product_rowmajor_times_vector(const Scalar* lhs, int lhsStride,
...
#define _EIGEN_ACCUMULATE_PACKETS(A0, A13, A2)                                                                         \
    ...
    if (PacketSize > 1)
    {
        ...
        while (skipRows < PacketSize && alignedStart != ((lhsAlignmentOffset + alignmentStep * skipRows) % PacketSize))
            ++skipRows;
    ...
    for (int i = skipRows; i < rowBound; i += rowsAtOnce)
    {
        ...
        {
            /* explicit vectorization */
            Packet ptmp0 = ei_pset1(Scalar(0)), ptmp1 = ei_pset1(Scalar(0)), ptmp2 = ei_pset1(Scalar(0)),
                   ptmp3 = ei_pset1(Scalar(0));
