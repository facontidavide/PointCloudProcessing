...
struct CommaInitializer
{
    ...
    inline CommaInitializer(MatrixType& mat, const Scalar& s) : m_matrix(mat), m_row(0), m_col(1), m_currentBlockRows(1)
    {
    ...
    CommaInitializer& operator,(const Scalar& s)
    {
        if (m_col == m_matrix.cols())
        {
            ...
            ei_assert(m_row < m_matrix.rows() && "Too many rows passed to comma initializer (operator<<)");
        }
        ei_assert(m_col < m_matrix.cols() && "Too many coefficients passed to comma initializer (operator<<)");
    ...
    CommaInitializer& operator,(const MatrixBase<OtherDerived>& other)
    {
        if (m_col == m_matrix.cols())
        {
            ...
            ei_assert(m_row + m_currentBlockRows <= m_matrix.rows() && "Too many rows passed to comma initializer "
                                                                       "(operator<<)");
        }
        ei_assert(m_col < m_matrix.cols() && "Too many coefficients passed to comma initializer (operator<<)");
    ...
    {
        ei_assert((m_row + m_currentBlockRows) == m_matrix.rows() && m_col == m_matrix.cols() &&
...
template <typename Derived>
template <typename OtherDerived>
inline CommaInitializer<Derived> MatrixBase<Derived>::operator<<(const MatrixBase<OtherDerived>& other)
{
