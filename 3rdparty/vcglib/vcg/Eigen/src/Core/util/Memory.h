...
/** \internal allocates \a size bytes. The returned pointer is guaranteed to have 16 bytes alignment.
  * On allocation error, the returned pointer is undefined, but if exceptions are enabled then a std::bad_alloc is
 * thrown.
  */
...
template <>
inline void* ei_conditional_aligned_malloc<false>(size_t size)
{
#ifdef EIGEN_NO_MALLOC
    ei_assert(false && "heap allocation is forbidden (EIGEN_NO_MALLOC is defined)");
#endif
...
inline T* ei_conditional_aligned_new(size_t size)
{
...
template <>
inline void ei_conditional_aligned_free<false>(void* ptr)
{
    free(ptr);
}
...
/** \internal delete the elements of an array.
  ...
  */
template <typename T>
inline void ei_delete_elements_of_array(T* ptr, size_t size)
{
...
/** \internal delete objects constructed with ei_conditional_aligned_new
  ...
  */
template <typename T, bool Align>
inline void ei_conditional_aligned_delete(T* ptr, size_t size)
{
    ei_delete_elements_of_array<T>(ptr, size);
...
inline static int ei_alignmentOffset(const Scalar* ptr, int maxOffset)
{
    ...
    return Vectorized ?
             std::min<int>((PacketSize - (int((size_t(ptr) / sizeof(Scalar))) & PacketAlignedMask)) & PacketAlignedMask,
                           maxOffset) :
             0;
}
...
/** \internal
  ...
  */
#ifdef __linux__
#define ei_aligned_stack_alloc(SIZE) (SIZE <= EIGEN_STACK_ALLOCATION_LIMIT) ? alloca(SIZE) : ei_aligned_malloc(SIZE)
#define ei_aligned_stack_free(PTR, SIZE)                                                                               \
    if (SIZE > EIGEN_STACK_ALLOCATION_LIMIT)                                                                           \
    ei_aligned_free(PTR)
#else
