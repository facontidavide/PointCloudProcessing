...
struct ei_assign_traits
{
  ...
  private:
    enum
    {
        InnerSize = int(Derived::Flags) & RowMajorBit ? Derived::ColsAtCompileTime : Derived::RowsAtCompileTime,
        InnerMaxSize =
          int(Derived::Flags) & RowMajorBit ? Derived::MaxColsAtCompileTime : Derived::MaxRowsAtCompileTime,
    ...
    enum
    {
        ...
        MaySliceVectorize =
          MightVectorize && int(InnerMaxSize) >= 3 * PacketSize /* slice vectorization can be slow, so we only
...
of a dynamic block in a fixed-size matrix */
  ...
  public:
    enum
    {
        Vectorization = int(MayInnerVectorize) ? int(InnerVectorization) : int(MayLinearVectorize) ?
                                                 int(LinearVectorization) :
                                                 int(MaySliceVectorize) ? int(SliceVectorization) : int(NoVectorization)
    };
  ...
  public:
    enum
    {
        Unrolling = (int(Vectorization) == int(InnerVectorization) || int(Vectorization) == int(NoVectorization)) ?
                      (int(MayUnrollCompletely) ? int(CompleteUnrolling) : int(MayUnrollInner) ? int(InnerUnrolling) :
                                                                                                 int(NoUnrolling)) :
...
struct ei_assign_novec_CompleteUnrolling
{
    enum
    {
        row = int(Derived1::Flags) & RowMajorBit ? Index / int(Derived1::ColsAtCompileTime) :
                                                   Index % Derived1::RowsAtCompileTime,
        col = int(Derived1::Flags) & RowMajorBit ? Index % int(Derived1::ColsAtCompileTime) :
                                                   Index / Derived1::RowsAtCompileTime
    };
...
struct ei_assign_innervec_CompleteUnrolling
{
    ...
    EIGEN_STRONG_INLINE static void run(Derived1 &dst, const Derived2 &src)
    {
        ...
        ei_assign_innervec_CompleteUnrolling<Derived1, Derived2,
                                             Index + ei_packet_traits<typename Derived1::Scalar>::size, Stop>::run(dst,
                                                                                                                   src);
    }
};
...
struct ei_assign_innervec_InnerUnrolling
{
    EIGEN_STRONG_INLINE static void run(Derived1 &dst, const Derived2 &src, int row_or_col)
    {
        ...
        ei_assign_innervec_InnerUnrolling<Derived1, Derived2, Index + ei_packet_traits<typename Derived1::Scalar>::size,
                                          Stop>::run(dst, src, row_or_col);
    }
};
...
template <typename Derived1, typename Derived2, int Vectorization = ei_assign_traits<Derived1, Derived2>::Vectorization,
...
struct ei_assign_impl<Derived1, Derived2, NoVectorization, CompleteUnrolling>
{
    EIGEN_STRONG_INLINE static void run(Derived1 &dst, const Derived2 &src)
    {
        ei_assign_novec_CompleteUnrolling<Derived1, Derived2, 0, Derived1::SizeAtCompileTime>::run(dst, src);
    }
};
...
struct ei_assign_impl<Derived1, Derived2, InnerVectorization, CompleteUnrolling>
{
    EIGEN_STRONG_INLINE static void run(Derived1 &dst, const Derived2 &src)
    {
        ei_assign_innervec_CompleteUnrolling<Derived1, Derived2, 0, Derived1::SizeAtCompileTime>::run(dst, src);
    }
};
...
struct ei_assign_impl<Derived1, Derived2, InnerVectorization, InnerUnrolling>
{
    EIGEN_STRONG_INLINE static void run(Derived1 &dst, const Derived2 &src)
    {
        ...
        const int outerSize = dst.outerSize();
        for (int j = 0; j < outerSize; ++j)
            ei_assign_innervec_InnerUnrolling<Derived1, Derived2, 0, innerSize>::run(dst, src, j);
    }
};
...
struct ei_assign_impl<Derived1, Derived2, LinearVectorization, NoUnrolling>
{
    inline static void run(Derived1 &dst, const Derived2 &src)
    {
        ...
        const int alignedStart =
          ei_assign_traits<Derived1, Derived2>::DstIsAligned ? 0 : ei_alignmentOffset(&dst.coeffRef(0), size);
...
struct ei_assign_impl<Derived1, Derived2, SliceVectorization, NoUnrolling>
{
    inline static void run(Derived1 &dst, const Derived2 &src)
    {
        ...
        int alignedStart =
          ei_assign_traits<Derived1, Derived2>::DstIsAligned ? 0 : ei_alignmentOffset(&dst.coeffRef(0, 0), innerSize);
...
EIGEN_STRONG_INLINE Derived &MatrixBase<Derived>::lazyAssign(const MatrixBase<OtherDerived> &other)
{
    EIGEN_STATIC_ASSERT_SAME_MATRIX_SIZE(Derived, OtherDerived)
    EIGEN_STATIC_ASSERT(
      (ei_is_same_type<typename Derived::Scalar, typename OtherDerived::Scalar>::ret),
...
template <typename Derived, typename OtherDerived,
          ...
          bool NeedToTranspose = Derived::IsVectorAtCompileTime &&OtherDerived::IsVectorAtCompileTime &&int(
                                   Derived::RowsAtCompileTime) == int(OtherDerived::ColsAtCompileTime) &&
