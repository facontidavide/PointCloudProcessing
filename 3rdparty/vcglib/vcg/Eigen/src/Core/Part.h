...
/** \nonstableyet
  ...
  *             UnitUpperTriangular, LowerTriangular, StrictlyLowerTriangular, UnitLowerTriangular. This is in fact a
 * bit field; it must have either
...
struct ei_traits<Part<MatrixType, Mode> > : ei_traits<MatrixType>
{
    ...
    {
        Flags =
          (_MatrixTypeNested::Flags & (HereditaryBits) & (~(PacketAccessBit | DirectAccessBit | LinearAccessBit))) |
          Mode,
...
template <typename MatrixType, unsigned int Mode>
class Part : public MatrixBase<Part<MatrixType, Mode> >
{
  public:
    ...
    inline Scalar& coeffRef(int row, int col)
    {
        ...
        ei_assert((Mode == UpperTriangular && col >= row) || (Mode == LowerTriangular && col <= row) ||
                  (Mode == StrictlyUpperTriangular && col > row) || (Mode == StrictlyLowerTriangular && col < row));
...
/** \nonstableyet
  ...
  *
  * The parameter \a Mode can have the following values: \c UpperTriangular, \c StrictlyUpperTriangular, \c
 * UnitUpperTriangular,
...
struct ei_part_assignment_impl
{
    ...
    inline static void run(Derived1& dst, const Derived2& src)
    {
        ...
        {
            ei_assert(Mode == UpperTriangular || Mode == LowerTriangular || Mode == StrictlyUpperTriangular ||
                      Mode == StrictlyLowerTriangular);
            if ((Mode == UpperTriangular && row <= col) || (Mode == LowerTriangular && row >= col) ||
                (Mode == StrictlyUpperTriangular && row < col) || (Mode == StrictlyLowerTriangular && row > col))
...
void Part<MatrixType, Mode>::lazyAssign(const Other& other)
{
    ...
    ei_part_assignment_impl<MatrixType, Other, Mode, unroll ? int(MatrixType::SizeAtCompileTime) : Dynamic>::run(
      m_matrix.const_cast_derived(), other.derived());
}

/** \nonstableyet
  * \returns a lvalue pseudo-expression allowing to perform special operations on \c *this.
  *
  * The \a Mode parameter can have the following values: \c UpperTriangular, \c StrictlyUpperTriangular, \c
 * LowerTriangular,
...
template <typename MatrixType, unsigned int Mode>
inline Part<MatrixType, Mode>& Part<MatrixType, Mode>::operator*=(const typename ei_traits<MatrixType>::Scalar& other)
{
    return *this = m_matrix * other;
}
