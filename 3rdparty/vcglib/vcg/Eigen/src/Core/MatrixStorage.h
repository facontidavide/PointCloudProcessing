...
/** \internal
  * Static array automatically aligned if the total byte size is a multiple of 16 and the matrix options require auto
 * alignment
  */
...
struct ei_matrix_array
{
    ...
    ei_matrix_array()
    {
        ...
        ei_assert((reinterpret_cast<size_t>(array) & 0xf) == 0 &&
                  "this assertion is explained here: http://eigen.tuxfamily.org/dox/UnalignedArrayAssert.html  **** "
                  "READ THIS WEB PAGE !!! ****");
#endif
    }
...
class ei_matrix_storage<T, Dynamic, Dynamic, Dynamic, _Options>
{
  ...
  public:
    ...
    inline ei_matrix_storage(ei_constructor_without_unaligned_array_assert) : m_data(0), m_rows(0), m_cols(0)
    {
    }
    inline ei_matrix_storage(int size, int rows, int cols) : m_data(ei_aligned_new<T>(size)), m_rows(rows), m_cols(cols)
    {
    }
    inline ~ei_matrix_storage()
    {
