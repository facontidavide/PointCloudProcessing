...
struct ei_dot_traits
{
  ...
  private:
    ...
    enum
    {
        ...
        Cost = Derived1::SizeAtCompileTime *
                 (Derived1::CoeffReadCost + Derived2::CoeffReadCost + NumTraits<Scalar>::MulCost) +
...
struct ei_dot_vec_unroller
{
    ...
    {
        return ei_pmadd(
          v1.template packet<Aligned>(row1, col1), v2.template packet<Aligned>(row2, col2),
...
template <typename Derived1, typename Derived2, int Vectorization = ei_dot_traits<Derived1, Derived2>::Vectorization,
...
struct ei_dot_impl<Derived1, Derived2, LinearVectorization, NoUnrolling>
{
    ...
    static Scalar run(const Derived1& v1, const Derived2& v2)
    {
        ...
        if (size >= packetSize)
        {
            PacketScalar packet_res = ei_pmul(v1.template packet<alignment1>(0), v2.template packet<alignment2>(0));
            for (int index = packetSize; index < alignedSize; index += packetSize)
            {
                packet_res =
                  ei_pmadd(v1.template packet<alignment1>(index), v2.template packet<alignment2>(index), packet_res);
            }
...
typename ei_traits<Derived>::Scalar MatrixBase<Derived>::dot(const MatrixBase<OtherDerived>& other) const
{
...
/** \returns an expression of the quotient of *this by its own norm.
  ...
  */
template <typename Derived>
inline const typename MatrixBase<Derived>::PlainMatrixType MatrixBase<Derived>::normalized() const
{
...
template <typename Derived>
template <typename OtherDerived>
bool MatrixBase<Derived>::isOrthogonal(const MatrixBase<OtherDerived>& other, RealScalar prec) const
{
