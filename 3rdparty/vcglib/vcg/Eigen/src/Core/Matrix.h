...
/** \class Matrix
  ...
  *                 The former controls storage order, and defaults to column-major. The latter controls alignment,
 * which is required
  *                 for vectorization. It defaults to aligning matrices except for fixed sizes that aren't a multiple of
 * the packet size.
 ...
 * the array
  * of coefficients as a fixed-size array, as a class member. This makes sense for very small matrices, typically up to
 * 4x4, sometimes up
 ...
 * cannot
  * exceed a certain value. This happens when taking dynamic-size blocks inside fixed-size matrices: in this case
...
template <typename _Scalar, int _Rows, int _Cols, int _Options, int _MaxRows, int _MaxCols>
class Matrix : public MatrixBase<Matrix<_Scalar, _Rows, _Cols, _Options, _MaxRows, _MaxCols> >
{
  public:
  ...
  public:
    enum
    {
        NeedsToAlign = (Options & AutoAlign) == AutoAlign && SizeAtCompileTime != Dynamic &&
                       ((sizeof(Scalar) * SizeAtCompileTime) % 16) == 0
    };
    ...
    EIGEN_STRONG_INLINE PacketScalar packet(int row, int col) const
    {
        return ei_ploadt<Scalar, LoadMode>(
          m_storage.data() + (Flags & RowMajorBit ? col + row * m_storage.cols() : row + col * m_storage.rows()));
    }
    ...
    EIGEN_STRONG_INLINE void writePacket(int row, int col, const PacketScalar& x)
    {
        ei_pstoret<Scalar, PacketScalar, StoreMode>(
          m_storage.data() + (Flags & RowMajorBit ? col + row * m_storage.cols() : row + col * m_storage.rows()), x);
    }
    ...
    /** \internal */
    Matrix(ei_constructor_without_unaligned_array_assert) : m_storage(ei_constructor_without_unaligned_array_assert())
    {
    }
#endif
    ...
    {
        _check_template_params();
        if ((RowsAtCompileTime == 1 && ColsAtCompileTime == 2) || (RowsAtCompileTime == 2 && ColsAtCompileTime == 1))
        {
        ...
        {
            ei_assert(x > 0 && (RowsAtCompileTime == Dynamic || RowsAtCompileTime == x) && y > 0 &&
                      (ColsAtCompileTime == Dynamic || ColsAtCompileTime == y));
        }
    }
...
#include EIGEN_MATRIX_PLUGIN
  ...
  private:
    ...
    {
        EIGEN_STATIC_ASSERT((_Rows > 0 && _Cols > 0 && _MaxRows <= _Rows && _MaxCols <= _Cols &&
...
template <typename _Scalar, int _Rows, int _Cols, int _Options, int _MaxRows, int _MaxCols>
template <typename MatrixType, typename OtherDerived>
struct Matrix<_Scalar, _Rows, _Cols, _Options, _MaxRows, _MaxCols>::ei_matrix_set_selector<MatrixType, OtherDerived,
                                                                                           true>
{
    static void run(MatrixType& dst, const OtherDerived& src)
    {
        dst._set_noalias(src.eval());
    }
};
