...
struct ProductReturnType<Lhs, Rhs, CacheFriendlyProduct>
{
    ...
    typedef typename ei_nested<Rhs, Lhs::RowsAtCompileTime, typename ei_plain_matrix_type_column_major<Rhs>::type>::type
      RhsNested;
 ...
 */
template <typename Lhs, typename Rhs>
struct ei_product_mode
{
    enum
    {

        value =
          ((Rhs::Flags & Diagonal) == Diagonal) || ((Lhs::Flags & Diagonal) == Diagonal) ?
            DiagonalProduct :
            Lhs::MaxColsAtCompileTime == Dynamic &&
                (Lhs::MaxRowsAtCompileTime == Dynamic || Rhs::MaxColsAtCompileTime == Dynamic) &&
...
struct ei_traits<Product<LhsNested, RhsNested, ProductMode> >
{
    ...
    typedef
      typename ei_scalar_product_traits<typename _LhsNested::Scalar, typename _RhsNested::Scalar>::ReturnType Scalar;
    ...
    enum
    {
        ...
        CanVectorizeRhs =
          RhsRowMajor && (RhsFlags & PacketAccessBit) && (ColsAtCompileTime % ei_packet_traits<Scalar>::size == 0),

        CanVectorizeLhs =
          (!LhsRowMajor) && (LhsFlags & PacketAccessBit) && (RowsAtCompileTime % ei_packet_traits<Scalar>::size == 0),

        EvalToRowMajor = RhsRowMajor && (ProductMode == (int)CacheFriendlyProduct ? LhsRowMajor : (!CanVectorizeLhs)),

        RemovedBits = ~(EvalToRowMajor ? 0 : RowMajorBit),

        Flags = ((unsigned int)(LhsFlags | RhsFlags) & HereditaryBits & RemovedBits) | EvalBeforeAssigningBit |
                EvalBeforeNestingBit |
                (CanVectorizeLhs || CanVectorizeRhs ? PacketAccessBit : 0) |
                (LhsFlags & RhsFlags & AlignedBit),

        CoeffReadCost = InnerSize == Dynamic ? Dynamic : InnerSize * (NumTraits<Scalar>::MulCost + LhsCoeffReadCost +
                                                                      RhsCoeffReadCost) +
...
template <typename LhsNested, typename RhsNested, int ProductMode>
class Product : ei_no_assignment_operator, public MatrixBase<Product<LhsNested, RhsNested, ProductMode> >
{
  public:
  ...
  public:
    ...
    inline Product(const Lhs& lhs, const Rhs& rhs) : m_lhs(lhs), m_rhs(rhs)
    {
        ...
        EIGEN_STATIC_ASSERT(
          (ei_is_same_type<typename Lhs::RealScalar, typename Rhs::RealScalar>::ret),
          YOU_MIXED_DIFFERENT_NUMERIC_TYPES__YOU_NEED_TO_USE_THE_CAST_METHOD_OF_MATRIXBASE_TO_CAST_NUMERIC_TYPES_EXPLICITLY)
        ei_assert(lhs.cols() == rhs.rows() && "invalid matrix product" && "if you wanted a coeff-wise or a dot product "
                                                                          "use the respective explicit functions");
    }
    ...
    {
        PacketScalar res;
        ei_product_packet_impl<Flags & RowMajorBit ? RowMajor : ColMajor, Unroll ? InnerSize - 1 : Dynamic, _LhsNested,
...
inline const typename ProductReturnType<Derived, OtherDerived>::Type MatrixBase<Derived>::
...
struct ei_product_coeff_vectorized_unroller
{
    enum
    {
        PacketSize = ei_packet_traits<typename Lhs::Scalar>::size
    };
    EIGEN_STRONG_INLINE static void run(int row, int col, const Lhs& lhs, const Rhs& rhs,
                                        typename Lhs::PacketScalar& pres)
    {
        ...
        pres =
          ei_padd(pres, ei_pmul(lhs.template packet<Aligned>(row, Index), rhs.template packet<Aligned>(Index, col)));
    }
};
...
struct ei_product_coeff_impl<InnerVectorization, Index, Lhs, Rhs, RetScalar>
{
    ...
    {
        PacketScalar pres;
        ei_product_coeff_vectorized_unroller<Index + 1 - PacketSize, Lhs, Rhs, PacketScalar>::run(row, col, lhs, rhs,
                                                                                                  pres);
...
struct ei_product_coeff_vectorized_dyn_selector
{
    ...
    {
        res = ei_dot_impl<Block<Lhs, 1, ei_traits<Lhs>::ColsAtCompileTime>,
                          Block<Rhs, ei_traits<Rhs>::RowsAtCompileTime, 1>, LinearVectorization,
                          NoUnrolling>::run(lhs.row(row), rhs.col(col));
    }
};
...
struct ei_product_coeff_vectorized_dyn_selector<Lhs, Rhs, 1, RhsCols>
{
    EIGEN_STRONG_INLINE static void run(int /*row*/, int col, const Lhs& lhs, const Rhs& rhs, typename Lhs::Scalar& res)
    {
        res = ei_dot_impl<Lhs, Block<Rhs, ei_traits<Rhs>::RowsAtCompileTime, 1>, LinearVectorization, NoUnrolling>::run(
          lhs, rhs.col(col));
    }
};
...
struct ei_product_coeff_vectorized_dyn_selector<Lhs, Rhs, LhsRows, 1>
{
    EIGEN_STRONG_INLINE static void run(int row, int /*col*/, const Lhs& lhs, const Rhs& rhs, typename Lhs::Scalar& res)
    {
        res = ei_dot_impl<Block<Lhs, 1, ei_traits<Lhs>::ColsAtCompileTime>, Rhs, LinearVectorization, NoUnrolling>::run(
          lhs.row(row), rhs);
    }
};
...
struct ei_product_coeff_vectorized_dyn_selector<Lhs, Rhs, 1, 1>
{
    ...
    {
        res = ei_dot_impl<Lhs, Rhs, LinearVectorization, NoUnrolling>::run(lhs, rhs);
    }
};
...
static void ei_cache_friendly_product_colmajor_times_vector(int size, const Scalar* lhs, int lhsStride,
                                                            const RhsType& rhs, Scalar* res);

template <typename Scalar, typename ResType>
static void ei_cache_friendly_product_rowmajor_times_vector(const Scalar* lhs, int lhsStride, const Scalar* rhs,
                                                            int rhsSize, ResType& res);

template <
  typename ProductType, int LhsRows = ei_traits<ProductType>::RowsAtCompileTime,
...
struct ei_cache_friendly_product_selector<ProductType, LhsRows, ColMajor, HasDirectAccess, 1, RhsOrder, RhsAccess>
{
    ...
    inline static void run(DestDerived& res, const ProductType& product)
    {
        ...
        ei_cache_friendly_product_colmajor_times_vector(res.size(), &product.lhs().const_cast_derived().coeffRef(0, 0),
                                                        product.lhs().stride(), product.rhs(), _res);
...
struct ei_cache_friendly_product_selector<ProductType, 1, LhsOrder, LhsAccess, RhsCols, RowMajor, HasDirectAccess>
{
    ...
    inline static void run(DestDerived& res, const ProductType& product)
    {
        ...
        ei_cache_friendly_product_colmajor_times_vector(res.size(), &product.rhs().const_cast_derived().coeffRef(0, 0),
                                                        product.rhs().stride(), product.lhs().transpose(), _res);
...
struct ei_cache_friendly_product_selector<ProductType, LhsRows, RowMajor, HasDirectAccess, 1, RhsOrder, RhsAccess>
{
    ...
    inline static void run(DestDerived& res, const ProductType& product)
    {
        ...
        ei_cache_friendly_product_rowmajor_times_vector(&product.lhs().const_cast_derived().coeffRef(0, 0),
                                                        product.lhs().stride(), _rhs, product.rhs().size(), res);
...
struct ei_cache_friendly_product_selector<ProductType, 1, LhsOrder, LhsAccess, RhsCols, ColMajor, HasDirectAccess>
{
    ...
    {
        UseLhsDirectly =
          ((ei_packet_traits<Scalar>::size == 1) || (Lhs::Flags & ActualPacketAccessBit)) && (Lhs::Flags & RowMajorBit)
    };
    ...
    inline static void run(DestDerived& res, const ProductType& product)
    {
        ...
        ei_cache_friendly_product_rowmajor_times_vector(&product.rhs().const_cast_derived().coeffRef(0, 0),
                                                        product.rhs().stride(), _lhs, product.lhs().size(), res);
...
inline Derived& MatrixBase<Derived>::operator+=(
  const Flagged<Product<Lhs, Rhs, CacheFriendlyProduct>, 0, EvalBeforeNestingBit | EvalBeforeAssigningBit>& other)
{
    if (other._expression()._useCacheFriendlyProduct())
        ei_cache_friendly_product_selector<Product<Lhs, Rhs, CacheFriendlyProduct> >::run(const_cast_derived(),
                                                                                          other._expression());
    else
...
template <typename T>
struct ei_product_copy_rhs
{
    typedef typename ei_meta_if<(ei_traits<T>::Flags & RowMajorBit) || (!(ei_traits<T>::Flags & DirectAccessBit)),
                                typename ei_plain_matrix_type_column_major<T>::type, const T&>::ret type;
};

template <typename T>
struct ei_product_copy_lhs
{
    typedef typename ei_meta_if<(!(int(ei_traits<T>::Flags) & DirectAccessBit)), typename ei_plain_matrix_type<T>::type,
                                const T&>::ret type;
};
...
inline void Product<Lhs, Rhs, ProductMode>::_cacheFriendlyEvalAndAdd(DestDerived& res) const
{
    ...
    LhsCopy lhs(m_lhs);
    RhsCopy rhs(m_rhs);
    ei_cache_friendly_product<Scalar>(rows(), cols(), lhs.cols(), _LhsCopy::Flags & RowMajorBit,
                                      (const Scalar*)&(lhs.const_cast_derived().coeffRef(0, 0)), lhs.stride(),
                                      _RhsCopy::Flags & RowMajorBit,
