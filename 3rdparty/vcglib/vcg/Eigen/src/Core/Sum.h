...
struct ei_sum_traits
{
  ...
  public:
    enum
    {
        Vectorization = (int(Derived::Flags) & ActualPacketAccessBit) && (int(Derived::Flags) & LinearAccessBit) ?
                          LinearVectorization :
                          NoVectorization
    };
...
struct ei_sum_vec_unroller
{
    ...
    {
        return ei_padd(ei_sum_vec_unroller<Derived, Start, HalfLength>::run(mat),
...
struct ei_sum_vec_unroller<Derived, Start, 1>
{
    enum
    {
        ...
        row = int(Derived::Flags) & RowMajorBit ? index / int(Derived::ColsAtCompileTime) :
                                                  index % Derived::RowsAtCompileTime,
        col = int(Derived::Flags) & RowMajorBit ? index % int(Derived::ColsAtCompileTime) :
                                                  index / Derived::RowsAtCompileTime,
...
template <typename Derived, int Vectorization = ei_sum_traits<Derived>::Vectorization,
...
struct ei_sum_impl<Derived, LinearVectorization, NoUnrolling>
{
    ...
    static Scalar run(const Derived &mat)
    {
        ...
        const int alignedStart = (Derived::Flags & AlignedBit) || !(Derived::Flags & DirectAccessBit) ?
                                   0 :
                                   ei_alignmentOffset(&mat.const_cast_derived().coeffRef(0), size);
        enum
        {
            alignment = (Derived::Flags & DirectAccessBit) || (Derived::Flags & AlignedBit) ? Aligned : Unaligned
        };
...
/** \returns the sum of all coefficients of *this
  ...
  */
template <typename Derived>
inline typename ei_traits<Derived>::Scalar MatrixBase<Derived>::sum() const
{
...
/** \returns the trace of \c *this, i.e. the sum of the coefficients on the main diagonal.
  ...
  */
template <typename Derived>
inline typename ei_traits<Derived>::Scalar MatrixBase<Derived>::trace() const
{
    return diagonal().sum();
}
